# arwebui — AgentRunner Web UI（Codex 交互模型）
...
它只通过公开的 `ar` CLI 契约(`os/exec`)与系统交互，是 AgentRunner 的
正式本机产品面；功能定义进入 `docs/` 三层体系，并参与
`scripts/check.sh` 闸门。`web/` 继续保留为极简 vanilla QA 驾驶舱。
...
> 产品 QA 一律连接共享的 `~/.local/share/agentrunner/` daemon/store；不要
> 通过改 `HOME`/`XDG_DATA_HOME` 隔离真实历史。破坏性 daemon 测试需另行确认。
