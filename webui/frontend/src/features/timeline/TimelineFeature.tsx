...
import { useTimelineCopyFeedback } from "./useTimelineCopyFeedback";
...
export function MsgActions({ text, ts, onContinue }: { text: string; ts?: string; onContinue?: () => Promise<void> }) {
  const { copied, copy } = useTimelineCopyFeedback(text);
...
export function ShellDetail({ t }: { t: ToolItem }) {
  ...
  const { copied, copy } = useTimelineCopyFeedback(copyBody);
  return (
