...
import { ChangesOutcome } from "../../components/ChangesOutcome";
import { DaemonAlert } from "../../components/DaemonAlert";
import { SessionNotFound } from "../../components/NotFound";
...
import {
  ...
  TerminalAlert,
  TurnFailureCard,
} from "../../components/SessionChrome";
import { useSessionDataController } from "./useSessionDataController";
import {
  isSessionNotFound,
  isValidSessionId,
  ...
  if (notFound) {
    return (
      <SessionView
        ...
        notFound={<SessionNotFound sid={sid} onBack={() => select(null)} />}
      />
    );
  }

  // INC-41 RD-B · the Environment rail is a floating card, so opening it
  // doesn't shift the conversation column. Changes remains a real split view.
  return (
    <SessionView
      daemonAlert={<DaemonAlert />}
      topbar={(
      ...
      findBar={findOpen ? (
        <FindBar
          scope={() => document.querySelector<HTMLElement>(".timeline")}
