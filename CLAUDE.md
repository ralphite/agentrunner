...
- `docs/LOG.md` — 决策台账：理由、坑、接受不修的偏差（只追加）。
- `docs/PROCESS.md` — 以上一切的流程：三层模型、双闸门测试纪律、
  执行协议。**改任何文档前先读它。**

硬性规则：
- **想清楚就做，不设前置文档闸。** 改动直接写代码，`check.sh` 全绿
  即提交推送。文档描述**已成立的事实**——改动若改变了某层文档的
  事实，同一个 commit 更新它；没改变就别动。
- **不产生纯仪式的提交。** 工作纸、收口件、评审件、"我做了什么"的
  台账条目一律不写；每个提交要能独立说明自己的价值。
  （`docs/increments/` 已冻结为历史，不再新增。）
- 动 `docs/DESIGN.md` 不变量必须走 PROCESS.md 的"不变量变更流程"
  （停下、写清冲突、单独 review），禁止代码里先绕。这是唯一的硬闸。
- 新增/改变 user journey 时先在 `docs/JOURNEYS.md` 落一两句，避免做
  出没人要的功能。
