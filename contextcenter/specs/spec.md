# Context Center — 产品 Spec v0.7

上承 `../reference/product-record.html` 中的 v0.6 Spec；本版并入其后定稿
的两项结构性决策（任务即文档、文件即真相），并钉死随之而来的实现边界。
与 `constitution.md` 冲突时以 constitution 为准。文末附 v0.6 → v0.7
变更清单。
...
没有预设的文档类型集合；Bug Backlog、Workstream、Plan 都只是起了这些名
字的普通文档。

- `kind`（可选 frontmatter 字段）标记语义：`project` / `workstream` /
  `task` / `bug` / `loop` / `lesson` / `plan` / `research` …，只影响默认
  icon、默认字段与可用 Action，不改变"它是文档"这一本质。
- Icon 解析顺序：用户手动设置 > `kind` 预设 > 按名字猜测的预设（如名为
  "Bugs" 的文档给 bug icon）> 默认页面 icon。
...
导航。紧凑、接近 Notion：展开/折叠、hover 出现 `+`（新建子文档）与
`⋯`（更多操作）、新增、移动、重命名、删除、切换 Project、搜索、快速创建。

**树只显示章节级文档**：物化出来的行级 Task 文档默认不出现在树里（防
止几百个微文档把树炸掉），它们经由父页列表、Loop 引用和搜索到达。
...
### 4.3 右侧：Contextual Inspector

默认关闭；选中 Task 行、Attempt、Loop、Bug 等结构化对象时出现。只显示
该对象必要的状态、关联与 Action（如 Task 的状态/Attempts/关联文档/
Mark as done；Attempt 的 outcome/Lesson/Use this result；Loop 的当前
项/队列/stop condition；Bug 的 priority/fix attempts/最佳结果）。不重
复整篇文档，不承担 Session/Diff 管理。

### 4.4 Composer / Artifact Comment（核心交互）

1. 用户选择任意文本、Block、Task 行或 Widget；
2. 出现浮动入口，选区成为 Composer 中的引用 Chip；
3. 用户附加 Comment/Instruction，选择绑定 Agent（Codex/Claude Code/内
   置轻量模型）；
4. 消息携带 Project、文档路径、选区内容与周边上下文；
5. Agent 回复，或对原位置**提议局部更新**（Proposed update 预览 +
   Copy / Apply update to document 一键应用）。

同一套交互适用于正文、Task、Attempt、Loop 和整页。
...
4. 子文档可作为紧凑的一行链接插入正文。
5. 用户可选择任意文字、Block 或 Task 行，并在 Composer 中引用。
...
4. 澄清：没有预设文档类型集合，icon 只是名字/kind 的默认装饰（§3.1）；
   Bug 就是 `kind: bug` 的 Task（§3.3）。
5. 验收标准新增 13–16 条。
