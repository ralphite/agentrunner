...
用 `v2/qa/ws.sh`，全部 repo 按 **SHA 钉死**（与上游漂移无关）：

```
v2/qa/ws.sh prepare <profile> <dir>    # color | cobra | cobra-broken | gin | blank
v2/qa/ws.sh cleanup <dir>              # 用完即删
```
...
`v2/qa/fixtures/build-error.png`（一张含编译错误的"CI 截图"）。

| # | 动作 | 验证 |
|---|---|---|
| 1 | `ar send $sid --image v2/qa/fixtures/build-error.png "这是 CI 的截图。哪个文件哪一行报了什么错？"` | 回答准确说出 `command.go` / `1234` / `EnableTraverseRunHooks2`（图片真被模型读到）|
...
workspace diff 归档到 `v2/qa/runs/<日期>-<QA号>/`；FAIL 必须先归因
