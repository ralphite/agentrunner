...
**G1 多模态用户输入（图片/PDF/附件/长贴折叠） — ❌ 设计缺失 · 高**
...
**G2 并行/后台子 agent（spawn background + 可杀） — ⚠️ 设计欠定 · 高**
...
**G6 会话续聊形态 — ❌ 设计缺失 · 高（单条缺口卡死最多 journey）**
...
**G8 运行中 spec 变更（换模型/角色切换）"变更即事件"路径 — ❌ 设计缺失 · 中**
spec 冻结于 RunStarted（对的），但缺一个显式变更事件族（如 ModeChanged
之于 mode）承载换模型、权限面切换。
→ UJ-11（评审→动手的角色切换）
...
**G21 自定义命令 / slash 面 — ⚠️ 设计欠定 · 低**
协议一行预留；命令的定义位（repo 内文件？）、展开语义（进 prompt 还是
进工具）、与 skills 的边界未设计。
→ UJ-19

### 工具与检索面

**G18 内置工具面完整性 — ⚠️ 设计欠定 + 🔧 实现缺口 · 中**
DESIGN 列名"file read/**write**/edit、bash、**glob/grep**、**web
fetch/search**"，其中：write_file 实现缺失（起项目/建文件借 bash
heredoc）；glob/grep 无独立工具（借 bash）；web fetch/search 完全未
spec（还牵动 network 资源类与注入面）。
→ UJ-01, UJ-05
...
唤醒、task_output 进度 tail、shadow repo 并发 flock、overlap:interrupt。
