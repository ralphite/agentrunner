#!/usr/bin/env bash
# QA-54 real gate (INC-53, HANDA #24): project overlay + launcher. The
...
#
#   qa/run-qa54.sh <arwebui-binary> <ar-binary>
set -euo pipefail

WEBUI="${1:?usage: run-qa54.sh <arwebui-binary> <ar-binary>}"
AR="${2:?usage: run-qa54.sh <arwebui-binary> <ar-binary>}"
...
curl -sf "http://127.0.0.1:$port/api/health" >/dev/null || { echo "QA-54: arwebui never came up" >&2; cat "$rt/webui.log" >&2; exit 1; }
...
")"
[ -n "$WS" ] || { echo "QA-54: no real workspace in store" >&2; exit 1; }
...
[ "$c1" = "400" ] || { echo "QA-54 FAIL: off-whitelist app → $c1, want 400" >&2; exit 1; }
...
[ "$c2" = "400" ] || { echo "QA-54 FAIL: unknown workspace → $c2, want 400" >&2; exit 1; }
...
[ "$c3" = "200" ] || { echo "QA-54 FAIL: project overlay update → $c3, want 200" >&2; exit 1; }
...
")"
echo "$got" | grep -q "QA54 Project" || { echo "QA-54 FAIL: overlay name not surfaced: $got" >&2; exit 1; }
meta="$rt/webui-meta.json"
[ -f "$meta" ] && grep -q "QA54 Project" "$meta" \
  || { echo "QA-54 FAIL: overlay not persisted to webui-meta.json" >&2; ls "$rt" >&2; exit 1; }
...
{ echo "QA-54 project launcher — $(date)"; echo "reject off-whitelist app + unknown workspace; overlay persist OK"; \
  echo "note: real open -a not exercised (side-effectful); argv covered by Go twins"; } > "$run_dir/notes.md"
echo "QA-54: all green (security reject paths + overlay persist). archived at $run_dir"
