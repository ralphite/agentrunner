...
| 16b | 内置 agent 默认全自动可用（不列 `agents:` 也可 spawn；白名单封闭性讨论） | #78 余项 | S | 📐 awaiting-review (INC-34) | 变更单 INC-34:三选(推荐 B——已声明 spawn 面则内置隐式在册,opt-out 者硬边界守住);触多 agent 面永不静默变宽,待用户裁 |
...
| 7b | 2026-07-09 | #7b skill context:fork (INC-31) | ✅ 双闸门全绿（3 孪生:改写成形+四不改写例/默认 prompt/全链 SpawnRequested 冻结 RoleSpec 载正文 + QA-37 真 Gemini 七红线:展开入 journal、子会话跑出 WIDGET-COUNT: 4、receipt 回父）；ingest 展开复用动态角色全链零 spawn 机制改动；INC-30 号让路并发 G24/G25(改 31),QA 让号至 37 | (见 push) |
