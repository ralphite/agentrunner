...
- **闸门 A（孪生，进 check.sh）**：
  ...
  - `TestInSessionGoalClaimWithVerifierStillVerifier`：有 verifier +
    模型声明但 verifier miss → 不达成（claim 不越权），feedback 记
    claim rejected。
  - `TestInSessionGoalNoVerifierBudget`：无 verifier、无声明 → 逐界
    计 check，预算尽 `GoalAchieved{budget}`（决策 #31 仍成立）。
  - `TestGoalControlRevive`（daemon）：goal-attach 到非 hosted 会话 →
    revive + 落账开工。
