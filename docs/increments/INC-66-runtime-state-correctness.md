...
- PASS：真实 Gemini Goal 在 check 1 fail 后进入 `goal_exhausted`，update 扩预算后
  同一 session 的 check 2 pass，且只在此时 `goal_achieved{satisfied}`。
- PASS：20k tree budget 下同一 batch 的 3 个 sibling 各获 5366 reservation，均
  completed；lead 最终 billed 16705，无 `limit_exceeded`、无 running 残影。
- PASS：1s fixed-rate slow interval 正确 journal missed tick；Scheduled Retry 通过
  API 新建 driver series，journal 从 `driver_started` 开始，无 conversation input；
  terminal `nextRunAt` 为空，inspect raw/cache/billed usage 完整。
- PASS：空 journal 不可 list/send；真实 daemon/webui 重启后 session detail、
  `#run:run1` deep link、API status 均恢复，browser console warning/error 为 0。
- 测试 session/journal/workspace 全保留，事件、inspect、health 与截图归档在
...
两个独立复核 Agent 对原始 finding 逐条确认；实现后完成 correctness、concurrency、
contract 三视角 review，原 19 条全部关闭。真实环境另发现并关闭 deploy liveness、
旧 launchd 进程替换、driver usage 投影三条状态缺陷。A/B 双闸均通过；#11 Ctrl-C
按用户要求不在本增量范围。
