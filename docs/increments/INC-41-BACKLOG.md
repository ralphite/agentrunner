...
- ☐ **HM-6(P3,低置信)建议卡 hover 描边跳到 `--ink-2`**(`styles.home.css:121-124`):从 `--line`(`#e7e7e7`)
...
- ☐ **SC-20(P3,部分 ✂)Create 按钮是全屏对比度最高的元素**:`.scheduled-create .menu-trigger`
...
- ☐ **DF-D7(P2)binary 文件仍会去 fetch `/blob` 并吃一条 400**:轮26 复验 Changes 屏见 1 条
...
- ☐ **DF-D8(P2)blob 预取对 binary/超大文件必然吃 400/413**:`AR.blob` 对 untracked 文件做预取,
