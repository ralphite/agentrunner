...
retry 按钮。零后端改动。
轮17 f36cf5e 已修:`timeline.ts` 新增 `explainFailure()` 把 errs.Class 全表(provider_rate_limit/
...
轮17 f36cf5e 已修:`isSessionNotFound` 增设一条**窄**的 400 判据(须匹配 server `api.go sid()` 的
