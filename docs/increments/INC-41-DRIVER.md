...
## ⏱ 速度硬规则(idle 即失败,与 ⛔ 并列最高优先级)

**空转就是失败——更新之间的长间隔不可接受(用户裁决)。** 循环必须【连续】干活:
- runner 已改成 while-true + 几秒 guard,**没有 30min sleep**;一轮完立即下一轮。
- **每一轮内也必须最大化并发**:finder **一批多发**(不同屏/组件并行取证);
  implementer **一批多发**(touches 白名单两两无交集、worktree 隔离、并发跑)。
- **一轮如果串行派活(一次只跑一个子 agent),这一轮算失败轮。**
- git 冲突用 **rebase/retry** 解决、只在最后 push 那步串行;**绝不靠暂停循环换省事**
  ——绝不 bootout/停循环来避免 git 碰撞。
