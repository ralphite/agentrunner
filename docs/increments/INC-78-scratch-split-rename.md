...
用户进一步质疑 "Scratch" 概念本身(历史考古见 LOG 同日条目)。本增量
落地的是方案 A(每 workspace 独立成组,可改名);两个更激进方向已呈报
待裁决,A 为呈现层改动、可逆,不阻塞:
- B(Codex 原教旨):无项目会话完全不进项目分组,平铺呈现;
- C(砍桥接):取消"无项目时静默铸造 ws-目录",composer 强制显式选择
  ——动 composer 合同与 daemon 契约,量级另计。
