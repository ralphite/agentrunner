# INC-54 durable Last turn diff（Codex Changes 范围）
...
1. `INC-54.0`：工作纸 + 独立契约 review，裁决通过后才写代码。
2. `INC-54.1`：snapshot 只读 comparison + CLI + DESIGN 同 commit；A 闸核心
   测试与 `./scripts/check.sh` 全绿，立即 push `origin/main`。
3. `INC-54.2`：Web API + frontend scope UX + scripted UI tests；build/check
   全绿，立即 push。
4. `INC-54.3`：部署 live 8809，共享数据 QA-54 全景浏览器验收；三层/支撑件
