...
- [ ] AskForm 自由文本输入 `ask-free`(AskForm.tsx:136):裸 input,
  无边框/焦点态,与 composer 输入不同族。
- [ ] turn-error 卡按钮 `turn-error-toggle`(SessionView.tsx:950)、
  `turn-error-action`(:960):错误卡的 Technical details 开关与
  Retry 按钮零样式。
- [ ] terminal-alert 按钮 `terminal-alert-action`(SessionView.tsx:1017)
  与 `tam-label`(:982,:1013 goal 元信息标签)。
- [ ] ENVIRONMENT rail worktree 按钮组 `env-wt-actions/env-wt-action/
  env-wt-danger`(SupervisionPanel.tsx:822-856):Apply back/Remove
  等动作按钮裸奔。
- [ ] Scheduled `scheduled-create`(Scheduled.tsx:523,Create 按钮
  ghost 化曾有专样式,现无规则)。

## P1 · 高频可见明细/状态面裸奔

- [ ] **Timeline 工具明细全家**(最大簇,Timeline.tsx 344-582,
  ...
  ± 着色,与 dsk shell 块同族。
- [ ] Timeline 活动行 `cx-activity-row/label`(:952-954)与
  `act-caret/act-cat`(:717-718):Worked 折叠内的活动分类行。
- [ ] markdown 表格 `cx-table`(Markdown.tsx:264):**assistant 回答
  里的表格零样式**(用户实机截图 #1 diff 表格烂即含此因)。
- [ ] ATTENTION 圆点 `attention-dot`(SupervisionPanel.tsx:142-166
  ×4):注意力行无状态色点。
- [ ] 加载骨架:`tl-skeleton/tl-skel-row`(Timeline.tsx:1152-1160)、
  `changes-outcome-skel`(ChangesOutcome.tsx:436)——加载期白板。
- [ ] 滚底浮钮 `tl-jump`(Timeline.tsx:1233)。
- [ ] ENVIRONMENT 行标签 `env-row-label`(×5)与路径明细
  `env-detail/env-detail-path/env-path/env-path-copy`。
- [ ] goal 文案 `goal-copy/goal-settled-copy`(SupervisionPanel
  321-364)与 `msg-goal-verdict`(Timeline.tsx:209)、
  `cx-goal-note`(:913)、`gbar-checks`(SessionView.tsx:1224)。
- [ ] shell 块状态徽标 `shell-status`(Timeline.tsx:289)。

## P2 · 次级/低频面

- [ ] Modals 等宽字 `code`(×7)与行布局 `row-flex`(×4)、
  `confirm-copy`(:182)、`fork-empty`(:664)——fork/rewind/confirm
  模态内部排版。
- [ ] approval Details 内部 `approval-gates/approval-readonly`
  (ApprovalCard.tsx:88,97)。
- [ ] supervision 折叠明细 `supervision-details/label/caret`
  (SupervisionPanel.tsx:475-478)。
- [ ] Scheduled 杂项 `sched-project-chip`(:712)`sched-pinned`(:719)。
- [ ] Settings:`rs-archive-empty`(×2)`rs-archive-note`
  `rs-sc-grouptitle`;快捷键面板 `sc-group`(Shortcuts.tsx:45)。
- [ ] `tl-notfound-id`(NotFound.tsx:14)`pop-section`(Popover.tsx:253)
