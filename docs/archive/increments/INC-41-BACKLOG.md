...
- 2026-07-21 轮76（headless·主驾驶自对标 + 3 并发 read-only finder + 3 并发 worktree implementer）：**切到 diff/rich/composer/env 屏重对标**。主驾驶自截 live diff-split/thread/composer/review-toolbar 直比金标，判 change-card / composer / diff-split / review-toolbar 均**已高度 parity**（避免在已 parity 处空转）。3 finder（diff-split / thread+env-panel / settings+home+composer）取证。**关闭 3 个 golden 直证、用户可见的差距**（一批 3 并发 worktree，白名单两两互斥 `{tw.css}` / `{SupervisionPanel.tsx+test}` / `{Home.tsx+headline.test}`，各 vitest 645 绿 + build 绿，落一个推一个）：
  - **ENV-SECTION-HAIRLINE**（tw.css:738/1385，push 12c841c3）：Environment 面板去掉 `.supervision-section` 的 `border-b` 与 `.supervision-details` 的 `border-t` 两条发丝横线 → 对标 Codex「一张卡内扁平清单、纯留白+dim 标签分组」。DOM 探针 border 0px 直证，live after 图 Run details 上方横线消失。裁决了 R73/R75 遗留 seed。
  - **DIFF-GLYPH-MODIFIED-NEUTRAL**（tw.css:827，同 push 12c841c3）：修改/重命名/复制文件的状态字形 `text-blue`→`text-ink-2` 中性墨色（金标 `M↓` 为中性深墨，蓝色抢增删语义色）。DOM 探针 glyphMod rgb(96,96,96)、glyphAdd 仍绿直证。
  - **BG-LABEL-PARITY**（SupervisionPanel.tsx:301，push c18d71d0）：Environment 分组标题 "Background work"→"Background processes"，对标金标 `codex-task-thread.jpg`；tsx+test 同步（test 4 处断言更新）。
  - **HOME-REPO-UNDERLINE**（Home.tsx:156，push a6cde610）：首页 headline 的 repo 名加细点状偏暗下划线（inline `underline decoration-dotted decoration-dim underline-offset-4`），对标金标 `codex-crop-newtask-emptystate.jpg` 的可变量提示。DOM 探针 underline/dotted/dim/offset-4 直证。仅视觉；点击切项目需跨 Composer，seed 下轮。
  - **证伪/关闭 seed**：SettingsAppearance +/− glyph 已 parity（finder 验 U+2212 正确配对，关 seed）；fold-band/bgproc 已 parity（无独立 fold-band 元素、bgproc row 视觉等价）。
  - **裁为刻意/暂缓**：diff-split 桌面卡片碎片 vs Codex 连续文档（DiffView.tsx:216 注释明写桌面"must keep card shape"=刻意决策，翻转前需设计确认，暂缓）；topbar Environment 蓝 active fill（有意为可发现性保留，判断题，暂缓）。
  - live=index-iTbhf1Jc.js，console panorama(home/rich/diff) error+warning=0。Seeds 下轮：HOME-REPO 点击切项目接线、diff word-level highlight 需补金标裁决、topbar active-fill 中性化裁决、切 approval/command-palette/model-menu 屏重对标。

- 2026-07-21 轮77（headless·主驾驶自对标金标 + live 双证·单点 CSS 直接落 + 2 并发 read-only finder 播种）：**切到命令面板/菜单/home 屏重对标**。主驾驶自截 live home/rich/diff/scheduled + 驱动 live 打开 model/add/command-palette 三菜单，逐屏并排金标（codex-crop-command-palette/model-dropdown/add-menu/change-card/composer/scheduled-list）。判 model 菜单 / add 菜单 / change-card / composer / scheduled 行结构均**已高度 parity**（避免空转）。**关闭 1 个 golden+内部一致性双证、visible 差距**（工作区干净无让路，单点 CSS 主驾驶直接落 main，vitest 645 绿 + build 绿）：
  - ✅ **CMDK-SECTION-CASE（P2·命令面板每次 Cmd+K 可见·金标+内部一致性双证）**〔R77 关闭·push 851b14ae〕：命令面板分区标签 `.cmdk-group`(tw.css:557) `text-[11px] font-medium uppercase tracking-[0.06em] text-dim` → `text-[12px] font-medium text-dim`（去 uppercase + tracking、11→12px），对齐我方自己 add/context 菜单的 `.menu-label`(tw.css:537 `text-[12px] font-medium text-dim` Title-case) + Codex 金标 `codex-crop-command-palette.jpg`（"Tasks"/"Unread tasks" 均 Title-case regular）。旧 shouty UPPERCASE 是命令面板独有的 one-off 不一致（sidebar/add 菜单均 Title-case）。**live 复验**(index-BscY_zAe)：`.cmdk-group` textContent="Sessions"（原 "SESSIONS"）、textTransform=**none**、fontSize=**12px**、letterSpacing=**normal**；before/after 截图直证 "SESSIONS/NEEDS ATTENTION"→"Sessions/Needs attention"。全景 console(home/scheduled/rich) **0**。
  - **裁为刻意/暂缓（本轮 benchmark 发现但不做）**：MSG-ACTION-DIVIDER（消息动作栏 `.msg-actions-div` 竖分隔线 + goal-verdict）——查明是**刻意决策**：verdict 已被移到 turn-footer（Timeline.thread.test.tsx:44 defending 注释 "Codex places the outcome after the turn's change card, not in the per-message row"），但金标 `codex-crop-message-actions.jpg` 显示 verdict 与动作图标**同行**（`[copy][👍][👎][share] | ✓ Goal achieved in Xs`），与注释前提矛盾——需 golden 裁决 verdict 是否该内联回动作行，seed 下轮 fresh 判。thumbs up/down 是 rating 后端（我方无）→ 不做壳。
  - **已确认无差距勿再 seed**：model 菜单（Model/Effort/Speed + Advanced^ 完全 parity）、add 菜单（Add/Goal/Plan mode/Advanced Title-case section label 已 parity·Plugins/Attach Finder 是 Codex 独有后端 out-of-scope）、scheduled 行结构（icon+title+cadence·relative-time 两行·与金标一致·All/Active/Finished 第三 tab SC-11 刻意·Paused/Mark-all-read 需 unread 后端 out-of-scope）、composer（access chip/model selector parity）。
  - live=index-BscY_zAe.js。2 并发 read-only finder（diff/Review-split 金标 / 命令面板+home+菜单金标）后台跑供下轮 seed。

  - ✅ **CMDK-SEL-NEUTRAL（P2·命令面板键盘导航每行可见·金标+live 双证·finder af2eaa 领跑 seed）**〔R77 关闭·push 0edf75fc〕：命令面板选中行 `.cmdk-item.sel`(tw.css:564) `bg-blue-soft`(蓝 accent) → `bg-panel-2`(中性灰,与 hover 同色)。Codex 金标 `codex-crop-command-palette.jpg` 的 ⌘1 光标行是**中性浅灰**填充(与 hover 同 hue、只是键盘光标),我方蓝底读作 Codex 此列从不用的 primary/active 态——键盘上下箭头每行都闪蓝。**live 复验**(index-DdQUk3Cf)：`.cmdk-item.sel` backgroundColor=**rgb(238,238,238)**(=--panel-2,原 blue-soft);before/after 截图直证选中行蓝→灰。全景 console 0。
  **本轮定性**:**非失败轮** — 主驾驶单点直接落(非派 worktree implementer)关闭 **2 个命令面板 golden+live 双证、visible 差距**:CMDK-SECTION-CASE(分区标签 UPPERCASE→Title-case·对齐我方 add 菜单+金标)+ CMDK-SEL-NEUTRAL(选中行蓝→中性灰·对齐金标·finder 领跑 seed)。两条均 tw.css 同文件、主驾驶串行直接落(工作区干净、headless 轮无让路、非 worktree 批),各 vitest 645/16 绿 + build 绿 + live DOM 探针双证 + console 全景 0。四闸门绿:改动屏对金标复比确关小(分区标签 "Sessions"/"Needs attention" Title-case、选中行 rgb238 灰)、qa/runs/2026-07-21-R77 存 before/after cmdk 截图 + 探针输出。诚实:两条均命令面板(Cmd+K power-user 面),非主视口最大差距——但都是 golden 双证的实质 parity 收敛(字重/case/背景色 token),非 a11y/装饰 nit;且第二条由 finder 独立坐实。
  **下轮 fresh-eyes seed(2 finder 回报,均 read-only 对金标)**：
    - **CMDK-ROW-DENSITY（中·finder af2eaa #2·置信中·须 live Codex 核比例）**：命令面板行 `.cmdk-item`(tw.css:558) `text-[13px] py-[8px]` 比 Codex 密(Codex 行 ≈15px、~44px 高、与 input 15px 更平);建议 `text-[14px]`/`py-[10px]` 抬行高。金标缩略图判、下轮先核。
    - **CMDK-KBD-BADGE（低·finder af2eaa #3）**：⌘N 徽章 `.cmdk-kbd`(tw.css:567) `border border-line ... font-mono` = keycap 感;Codex 是无边浅灰 soft pill + UI 字体。去 border+font-mono(须拆 `.cmdk-kbd` 与 `.kbd/.rs-kbd` 共享规则,后者可能要 keycap 感)。低价 polish。
    - **DIFF-WORD-LEVEL（中·finder ae6d31 #1·最高可见度·但须先真 Codex 核 + 新增能力)**：diff 改动行缺 word-level(行内)高亮——我方 `renderCode`(DiffView.tsx:40-46) 只做语法着色、整行 add/del 平涂 `--diff-add/del-bg`;Codex 只加深真正变化的子串。关闭=FileBody 对配对 del/add 行做 token 级 LCS 包 `.dl-word` 加深 span,inline+`.fd-split` 两路都挂。**属新增能力非改 class·下轮实现前先在真 Codex app 上核该行内高亮确实存在**(headless 无 Computer Use 截不到)。
    - **DIFF-GLYPH-MODIFIED-COLOR（低-中·finder ae6d31 #2)**：diff 文件头 modified 字形 `.fd-glyph-modified`(tw.css:832) 是灰字母 `M`(text-ink-2);Codex 是带色/git-icon `M↓`,辨识度更高。关闭=改 `.fd-glyph-modified` 配色或换 phosphor git 图标。注:前置展开三角是 ✂ 刻意(RV-3·DiffView.tsx:1306 defended·勿动)。
    - **MSG-ACTION-VERDICT-INLINE（中·须 golden 裁决)**：消息动作栏 verdict 位置——现移到 turn-footer(Timeline.thread.test.tsx:44 defending 注释 "Codex places outcome after change card, not per-message row"),但金标 `codex-crop-message-actions.jpg` 显示 verdict 与 copy/share 图标**同行**(`[icons] | ✓ Goal achieved in Xs`)+ `.msg-actions-div` 竖分隔——注释前提与金标矛盾,下轮 fresh 裁决 verdict 是否内联回动作行。
    - finder 判**已 parity 勿再 seed**:model 菜单(完全 parity)、add 菜单(in-scope 项 parity·Plugins/Attach Finder out-of-scope)、home 空态(R76 已做)、diff fold-band/gutter-color/行号/语法高亮/Review 分栏比例/commit-push pill(均 DF-/R\d+/QA- defended parity)、composer(access chip/model selector parity)。out-of-scope(需新后端):composer worktree/environment chip、scheduled Paused tab/Mark-all-read(unread 后端)。
  - <2026-07-21 轮77>：对标命令面板/菜单/home(主驾驶自对标 codex-crop-command-palette/model-dropdown/add-menu 金标 + 驱动 live 开三菜单 + DOM 探针双证)、关差距 CMDK-SECTION-CASE(P2·UPPERCASE→Title-case·对齐 .menu-label+金标)+CMDK-SEL-NEUTRAL(P2·选中行蓝→中性灰·finder 领跑 seed·对齐金标)、开轮工作区干净无让路、主驾驶单点直接落 2 条 tw.css(非 worktree 批)、push 851b14ae+0edf75fc(+2 ledger commit)、live=index-DdQUk3Cf.js、console panorama 0

  - <2026-07-21 轮78（headless·主驾驶自对标 codex-crop-command-palette 金标 + live 双证·主驾驶单点串行直接落 + 2 并发 read-only finder 播种）>：**切回命令面板做剩余 golden+live 双证密度差距 + 逐条核实 R77 seed**。主驾驶截 live cmdk（Cmd+K）逐屏并排金标 `codex-crop-command-palette.jpg`，DOM 探针 before：`.cmdk-item` height=37.6px/13px/py-8px（密）、`.cmdk-kbd` border=1px+font-mono（keycap 感）。工作区干净无让路，两条同 tw.css → 主驾驶串行直接落 main（非 worktree 批，R77 precedent），vitest 645 绿 + build 绿：
    - ✅ **CMDK-ROW-DENSITY（P2·命令面板每次 Cmd+K 可见·金标+live 双证·消 R77 finder af2eaa seed）**〔R78 关闭·push 7d8fba4c〕：`.cmdk-item`(tw.css:564) `px-2 py-[8px] text-[13px]`(~37.6px 密、读作 dense dropdown) → `py-[10px] text-[14px]`。Codex 金标 task 行 ~44px 高、~15px 字，是与 15px search input 齐平的 calm 可扫列表。**live 复验**(index-CUumS3ju)：`.cmdk-item` height=**43.2px**、fontSize=**14px**、padY=**10px**；after 截图直证列表明显宽松。
    - ✅ **CMDK-KBD-BADGE（低·命令面板 ⌘N 徽章·消 R77 finder af2eaa seed·金标+live 双证）**〔R78 关闭·同 push 7d8fba4c〕：`.cmdk-kbd` 从共享的 `.cmdk-kbd,.kbd,.rs-kbd { border border-line ... font-mono text-[11px] }` keycap 规则**拆出** → `.cmdk-kbd { rounded-[5px] bg-panel-2 px-[6px] py-[2px] text-[12px] text-dim }`（无 border、无 font-mono、12px）。Codex 金标 ⌘N 是无边浅灰 soft pill + UI 字体（quiet row-index hint，非可按 keycap）；真快捷键表 `.kbd/.rs-kbd` 保留 keycap（document 真实按键）。**live 复验**：`.cmdk-kbd` borderWidth=**0px**、fontFamily=**-apple-system**、fontSize=**12px**。
  **本轮定性**:**非失败轮** — 主驾驶单点串行直接落（工作区干净、headless 无让路、非 worktree 批）关闭 **2 个命令面板 golden+live 双证、每次 Cmd+K 可见的实质密度/字体差距**：行从 dense dropdown（37.6px）抬到 calm 列表（43.2px 对齐金标 ~44px）+ ⌘N 徽章从 bordered mono keycap 变无边 soft pill UI 字体。均 vitest 645 绿 + build 绿 + live DOM 探针双证 + 全景 console(home/rich/diff×light/dark) **0**。四闸门绿：改动屏对金标复比确关小（before/after cmdk 截图 + 探针 37.6→43.2px / border 1→0px 直证）、qa/runs/2026-07-21-r78 存 before/after cmdk 截图 + finder 证据。诚实：两条均命令面板（Cmd+K power-user 面，与 R77 同面），但都是金标直证的实质密度/typography parity 收敛（非 a11y/装饰 nit），且由 R77 finder af2eaa 独立坐实为 seed。**裁决两条 R77 seed 为 ✂**：MSG-ACTION-VERDICT-INLINE（查 Timeline.tsx:1344-1355 verdict 已内联在 tail-row `[actions] │ ⊘ Goal achieved in N`，R77 seed 前提"移到 turn-footer 非内联"错误，实为已 parity）、DIFF-GLYPH-MODIFIED-COLOR（反转 R76 DIFF-GLYPH-MODIFIED-NEUTRAL 的 deliberate 决策，金标 M↓ 亦中性深墨、蓝色抢增删语义色）。
    **仍开放 ☐（下轮 fresh-eyes 排·本轮 2 finder 播种）**：
    - **⚠ DIFF 系差距需先造有真实修改的会话（验证数据 blocker·diff finder a956 坐实）**：本实例**所有** session 的 diff 都是纯 addition（pinned d8ac last-turn=13 files 全 `new file` +12 −0；其他会话 diff_len 0/nested/非 repo）——金标 `codex-diff-review.jpg` 的价值全在 modified 文件（split old/new 双列、红底删除行、`@@` hunk 头、`∧ N unmodified lines` 折叠带），这些**在本实例无法像素核验**（split 对纯 addition 正确回退单列 inline，DIFF-SPLIT-ADDED defended）。**下轮先 seed/造一个有真实 modification 的会话**，再核 DIFF-WORD-LEVEL（行内 word-level 高亮·仍须真 Codex Computer Use 核该能力存在）/ FOLD-BAND-WEIGHT 可视复验 / 删除行渲染。
    - **CMDK-KBD-BADGE-ALIGN 复看（可选低价）**：kbd 12px 后与 14px 行文字的垂直对齐/右缘间距下轮 live 扫一眼确认无回退（本轮 after 截图看无异常）。
    - **thread 屏（finder a36 坐实已强 parity·勿再 seed）**：变更卡（.changes-outcome R68/INC-41 defended pill/可点行）、composer 底栏（Gemini Flash 粗+Medium dim 逐字重现金标 5.6 Sol/High）、消息动作行（Copy/Share + ⊘ Goal achieved 已实现·thumbs 是 rating 后端 out-of-scope）、用户气泡/Worked 折叠/artifact 卡均 parity。access chip 灰点回退是 INC-54 刻意数据驱动。
    - **diff 可渲染表面（finder a956 坐实已 parity/defended·勿再 seed）**：split 布局(DF-1 62%)、toolbar(Review│Last Turn▾│+12−0│…│Commit or push)、文件头(caret+glyph+dim-dir/bold-base+双计数)、added-row(3px 绿 accent+绿底+gutter+`+`+语法高亮)。刻意 ✂：逐文件圆角卡(DiffView.tsx:213 defended)、前置 caret(RV-3)、Review label pill(DIFF-REVIEW-LABEL)、× 关闭(RV-1/RD-8)。
    - out-of-scope（需新后端·勿 seed）：rating thumbs、composer 用量环/worktree-env chip、Environment Background processes/Browser/Sources 段。
    - <2026-07-21 轮78>：对标命令面板(主驾驶自对标 codex-crop-command-palette 金标 + DOM 探针 before/after 双证)、关差距 CMDK-ROW-DENSITY(P2·37.6→43.2px/13→14px·消 R77 finder seed)+CMDK-KBD-BADGE(低·bordered mono keycap→无边 soft pill UI 字体·拆共享规则)、裁 ✂ MSG-ACTION-VERDICT-INLINE(已内联 parity)+DIFF-GLYPH-MODIFIED-COLOR(反转 R76 deliberate)、开轮工作区干净无让路、主驾驶单点串行直接落 tw.css(非 worktree 批)、push 7d8fba4c、live=index-CUumS3ju.js、console panorama 0、2 finder(thread a36/diff a956)回报均强 parity+diff 需造 modified 会话方可续核。

- 2026-07-21 轮79（headless·主驾驶自对标 codex-crop-scheduled-list/sidebar-projects 金标 + live 探针双证·单点 CSS 主驾驶串行直接落 + 2 并发 read-only finder 播种）：**切到 Scheduled/sidebar/home 屏重对标**。无 API key（本 headless 轮 ~/.env 无 GEMINI/ANTHROPIC）→ **diff-mod 会话无法造，diff 系差距仍 blocked**（DIFF-WORD-LEVEL/FOLD-BAND/删除行渲染须真修改会话，续 R78 seed）。主驾驶截 live home/scheduled/settings + 逐屏并排金标；工作区干净无让路，两条均 tw.css → 主驾驶串行直接落 main（R77/R78 precedent，非 worktree 批），各 vitest 645 绿 + build 绿：
  - ✅ **SCHED-SEARCH-PILL（P2·Scheduled 每次可见·金标+live 双证）**〔R79 关闭·push 37b521ea〕：`.sched-search`(tw.css:992) `rounded-[10px] px-3 h-9` + input `text-[13px]`（boxy 密） → `rounded-full px-4 h-11` + input `text-[14px]`。金标 `codex-crop-scheduled-list.jpg` 的搜索框是**全圆 pill**（radius=半高）+ ~15px placeholder 的 calm 搜索条。**live 探针**(index-C_Q7p2yU)：`.sched-search` borderRadius=**rounded-full**、height=**44px**、padX=**16px**、input fontSize=**14px**；before/after 截图直证方角 boxy→全圆 calm pill。
  - ✅ **SIDE-HEADING-SIZE（P2·高置信·sidebar 每屏可见·父<子层级倒挂·金标+live 双证·finder afc4 领跑 seed）**〔R79 关闭·push 9c5de484〕：`.project-heading`(tw.css:308) 复用共享基类 `.section-label,.project-heading{text-[11px]}`，其 override 只改 case/weight/tracking/color **从未改字号** → repo 组名（mt-test/agentrunner）泄漏在 **11px**，比其下缩进的 `.project-session` 子行 **13px** 还小（唯一父<子倒挂）。加 `text-[13px]` 与 session 齐。金标 `codex-crop-sidebar-projects` repo 名与 session 同号、靠 folder 图标（非字号）区分层级——正是 r47 SIDE-SUBTITLE 注释本意。**live 探针**(index-CewmmNBO)：`.project-heading` fontSize **11px→13px** == `.project-session` 13px；after 截图直证 mt-test/workspace/editable_mermaid2 组名与子行同号、folder 图标承载分组。
  **本轮定性**：**非失败轮** — 主驾驶单点串行直接落（工作区干净、headless 无让路、非 worktree 批）关闭 **2 个金标+live 双证、每屏可见的实质差距**：Scheduled 搜索框 boxy 方角→全圆 calm pill（44px）+ sidebar repo 组名修复父<子字号倒挂（11→13px）。均 vitest 645 绿 + build 绿 + live DOM 探针双证 + 全景 console(home/scheduled/rich/diff×light/dark) **0**。四闸门绿。诚实：SCHED 是 power 面、SIDE 是 sidebar，但 SIDE 是**层级倒挂**（父标题比子行还小）的真结构瑕疵、每屏可见，SCHED 是 golden 直证的 pill 形状差——均非 a11y/装饰 nit，且 SIDE 由 finder afc4 独立坐实。
  - 2 finder 回报：**thread(accc)** 逐屏（model 菜单/变更卡/composer 底栏/消息动作行/Environment 面板）PIL 标定 + 探针核 = **强 parity 无新差距**（context-usage gauge 无 token 后端 out-of-scope、变更卡标题字号无可靠锚点不报）；**sched/settings/sidebar(afc4)** 领跑上二条 + 确认 **SCHED-SUGGEST-LABEL 已做**（`.sched-suggestions-title` R48 已 normal-case/15px/500，关 seed）。
  **仍开放 ☐（下轮 fresh-eyes 排·afc4 seed）**：
    - **SCHED-TITLE-SIZE（P3·中置信·afc4 seed·跨页共享类需专门轮）**：`.page-heading h2`(tw.css:968) `text-[22px] font-semibold` vs 金标 `codex-crop-scheduled-list` "Scheduled tasks"≈28px 轻字重；我方 Home headline 已 28px/400（统一大标题系统），Scheduled h2 只 22px→标题:正文 1.7x vs Codex 2.15x。改法 `text-[22px]→text-[26-28px]`（可一并 semibold→medium/normal）。**注**：`.page-heading` 跨页共享（Settings 等也用），仅 Scheduled 有金标，下轮先复验其他页无回退再落。
    - **⚠ DIFF 系差距仍 blocked（无 API key 造 modified 会话）**：DIFF-WORD-LEVEL（行内高亮·须真 Codex Computer Use 核）/ FOLD-BAND-WEIGHT / 删除行渲染 / DIFF-GAP-LABEL-XALIGN——本实例所有 session diff 全纯 addition，须先造有真实修改的会话（须交互 session 带 GEMINI_API_KEY）。
  - <2026-07-21 轮79>：对标 Scheduled/sidebar/home(主驾驶自对标 codex-crop-scheduled-list/sidebar-projects 金标 + live 探针双证)、关差距 SCHED-SEARCH-PILL(P2·boxy 方角→全圆 44px calm pill·金标双证)+SIDE-HEADING-SIZE(P2·高·repo 组名 11→13px 修父<子倒挂·finder afc4 领跑·金标双证)、无 API key diff-mod 仍 blocked、开轮工作区干净无让路、主驾驶单点串行直接落 2 条 tw.css(非 worktree 批)、push 37b521ea+9c5de484、live=index-CewmmNBO.js、console panorama 0

- 2026-07-21 轮80（headless·主驾驶自对标 codex-scheduled/codex-crop-scheduled-list + codex-new-task-home + codex-task-thread + codex-thread-environment-panel 金标 · live 探针双证 · 单点 CSS 主驾驶直接落 + 1 并发 read-only finder 播种）：**带新鲜眼睛重截 live 全屏（home/scheduled/rich/diff × light/dark）逐屏并排金标**。核实 home 4 卡/headline、变更卡 inline 文件列表（ChangesOutcome.tsx:459/507 已 Show N more files·Codex behavior·parity）、Environment 面板 Changes/Worktree/Create branch/Commit-or-push（parity；Background processes/Browser/Sources 需新后端·out-of-scope）、rich thread/composer 均**已高度 parity**（避免空转）。工作区干净无让路。**关闭 1 个 golden+内部一致性+live 探针三证、Scheduled 每屏可见的实质大标题差距**（单文件 tw.css → 主驾驶直接落 main，R77–R79 precedent 非 worktree 批，vitest 645 绿 + build 绿）：
  - ✅ **SCHED-TITLE-SIZE（P3→关闭·消 R79 afc4 seed·纠正 R79「跨页共享须专轮」误判）**〔R80 关闭·push 851095e5〕：`.page-heading h2`(tw.css:976) `text-[22px] font-semibold` → `text-[28px] font-normal leading-tight max-[680px]:text-[24px]`，副标题 `.page-heading p` `text-[13px] mt-0.5` → `text-[15px] mt-1`。**关键纠错**：R79 seed 认为 `.page-heading` "跨页共享（Settings 等也用）需专门轮复验" —— grep 实证 **`.page-heading` 仅 Scheduled.tsx 一处引用**，无跨页回退风险。我方 Home headline `.home-empty-headline` 是 28px/400/leading-tight（统一大标题系统），Scheduled h2 却 22px/600——**大标题家族里唯一比 Home 又小又重的页**，也小于 Codex `codex-scheduled.jpg`/`codex-crop-scheduled-list.jpg` "Scheduled tasks"≈28px 轻字重。统一到 28px/normal 并抬副标题 13→15px（对齐 Codex ~15px 描述行、在更大标题下保持 calm）。**live 探针复验**(index-CJQ0rGn8)：`.page-heading h2` fontSize **22→28px**、fontWeight **600→400**、lineHeight **35px**（leading-tight）；`.page-heading p` fontSize **13→15px**；before/after 截图直证 "Scheduled runs" 从局促小标题→calm 大标题追平金标。全景 console(scheduled) **0**。
  **本轮定性**：**非失败轮** — 关闭 **1 个 golden+内部一致性+live 三证、Scheduled 每屏可见的实质大标题差距**：页标题从 22px/600（大标题家族里唯一又小又重的页）统一到 28px/400 追平我方 Home + Codex ~28px，副标题 13→15px 同步 calm。**纠正了 R79 的误判**（`.page-heading` 并非跨页共享、仅 Scheduled 用），解锁了这条被误 block 的 seed。vitest 645 绿 + build 绿 + live DOM 探针三证 + console 0。四闸门绿：改动屏对金标复比确关小（before/after scheduled 截图 + 探针 22→28px/600→400/15px 直证）、qa/runs/2026-07-21-r80 存 before/after scheduled 截图 + 探针输出。诚实：单条大标题 typography 收敛（非 a11y/装饰 nit·golden+内部一致性双锚·每次进 Scheduled 页可见），本轮为聚焦快轮（主驾驶单点直接落 + 1 finder 播种），未派 worktree 批。
  - **仍开放 ☐（下轮 fresh-eyes 排·finder a7a8 后台播种中）**：
    - **⚠ DIFF 系差距仍 blocked（无 API key 造 modified 会话）**：DIFF-WORD-LEVEL / FOLD-BAND-WEIGHT / 删除行渲染——本实例所有 session diff 全纯 addition，须先造有真实修改的会话（须交互 session 带 GEMINI_API_KEY），续 R78/R79 seed。
    - thread/composer finder（a7a8）seed 下轮登记（本轮 finder 后台跑，结果并入下轮）。
  - <2026-07-21 轮80>：对标 scheduled/home/rich/diff 全屏重截 live 并排金标(主驾驶自对标 codex-scheduled/codex-crop-scheduled-list/codex-new-task-home/codex-task-thread/codex-thread-environment-panel + live 探针三证)、关差距 SCHED-TITLE-SIZE(P3·22→28px/600→400+副标题13→15px·统一大标题系统·纠正 R79 跨页共享误判·golden+内部一致性双锚)、核实 home 卡/变更卡文件列表/Environment/thread/composer 均 parity(避空转)、无 API key diff-mod 仍 blocked、开轮工作区干净无让路、主驾驶单点直接落 tw.css(非 worktree 批)、push 851095e5、live=index-CJQ0rGn8.js、console panorama 0、1 finder(thread/composer a7a8)后台播种
  - **R80 finder a7a8 回报（thread/composer·read-only）**：逐组件（变更卡/附件卡/消息动作行/composer 底排/Worked-fold/Step-limit 卡）比对金标 codex-task-thread/crop-composer/crop-change-card/crop-message-actions，**均判 parity 或 out-of-scope，无确定性 in-scope seed**（该区布满 R68/R78/TH-5/TH-21/INC-41 defended）。唯一低置信候选〔下轮须真 Codex app 核实再定〕：**THREAD-CARD-DENSITY（低·须真机核）**——`.changes-outcome`(tw.css:965 `my-3`) 卡间气口 ~40-45px vs 金标 ~20px，附件卡→变更卡→动作行堆叠偏松；改法 `my-3→my-2` 或父容器 `space-y` 统管。**但随 run 数据（卡数/是否 verdict 终态）变化大、且该区经 R68/R78 多轮调过，可能是刻意气口——必须先真机核实，勿盲 seed。** thumbs/context-usage 环/Environment Background-Browser-Sources 段确认 out-of-scope（无后端）。

- 2026-07-21 轮81（headless·主驾驶自对标 home/scheduled/rich/diff/model-menu/composer/sidebar/approval/mobile 全屏重截 live 并排金标 + 2 并发 read-only finder 播种 settings/scheduled-thread）：**带新鲜眼睛穷尽自对标**，核实成熟面均**刻意 parity 且带注释依据**（composer env-strip flat=R46 HOME-CHIP-FLAT 对齐 codex-new-task-home、diff 文件卡桌面保留=DiffView.tsx:214-219 文档决策、Scheduled 第三 tab=Finished 而非 Codex Paused=Scheduled.tsx:16-20「无 pause 后端」诚实决策、diff +/- marker cell=RVW-MARKER/DIFF-MARKER-NOTCH 调过、model dropdown/scheduled Suggestions/approval 卡/sidebar projects 均像素级 parity）——避免空转、不碰刻意决策。工作区干净无让路。**关闭 1 个 finder 领跑（settings finder aa5c 高置信·golden 单系统依据 + live 探针双证）、Settings 落地页每次进入即可见的实质结构差距**（单文件复用现有 rs- helper → 主驾驶直接落 main，R77–R80 precedent 非 worktree 批，vitest 645 绿 + build 绿）：
  - ✅ **SETTINGS-GENERAL-CHROME（P2→关闭·finder aa5c 领跑）**〔R81 关闭·push 69e2214c〕：`SettingsGeneral.tsx` 落地面板此前**手搓** 19px/650 标题 + 无边框 `border-t border-line-2` 扁平列表行，破坏其余 6 面共享的 `rs-` 系统（`rs-panel-title` 22px/semibold + 有边框 `rs-row`/`rs-row-block` 卡片）——用户进 Settings 看到的**第一页**看起来像另一套更早、未完成的界面。Codex 对每个 settings section 跑**同一套设计系统**；General 现加入（`rs-panel`/`rs-panel-title`/`rs-panel-sub`/`rs-row-block`），左栏导航永不切换内容面的视觉语言。行为不变（status 读出 + reset 流一致），mobile-stack 测试改指 `.rs-row-head` flex 容器（布局迁移后所在）。**live 探针双证**(index-B_-ibON0)：`.rs-panel-title` fontSize **19→22px**、fontWeight **650→600**；2 个 `.rs-row` 卡片各 borderTop **1px**、radius **12px**、bg-panel（此前无卡片、扁平 border-t）；before/after 截图直证 Status/Reset 从扁平分隔行→有边框卡片、标题追平 Git 等兄弟面。全 section（General/Appearance/Git/Configuration）console **light+dark 0**。
  **本轮定性**：**非失败轮** — 关闭 **1 个 finder 领跑·golden 单系统 + live 探针双证、Settings 落地页每次进入即可见的实质结构差距**：General 面板从「19px 标题 + 无边框扁平行」统一到「22px `rs-panel-title` + 有边框 `rs-row` 卡片」，追平其余 6 面 + Codex 单一 settings 设计系统。vitest 645 绿 + build 绿 + live DOM 探针双证 + console 0（4 section × light/dark）。四闸门绿：改动屏对兄弟面/金标复比确关小（before/after general 截图 + 探针 19→22px/650→600/2 卡片直证）、qa/runs/2026-07-21-r81 存 before/after + 探针输出。诚实：单面结构 chrome 收敛（非 a11y/装饰 nit·golden 单系统 + 内部一致性双锚·每次进 Settings 落地页可见），主驾驶单点直接落 + 2 finder 播种，未派 worktree 批。**两 finder 均诚实回报其余审计面（settings 其余 6 面 / scheduled Suggestions / rich thread 变更卡·Environment·Worked-fold / mobile）无确定 in-scope 差距，为 parity 或文档刻意决策**——印证成熟面已高度防守。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：
    - **⚠ DIFF 系差距仍 blocked（无 API key 造 modified 会话）**：DIFF-WORD-LEVEL / FOLD-BAND-WEIGHT / 删除行渲染——本实例所有 session diff 全纯 addition，须先造有真实修改的会话（须交互 session 带 GEMINI_API_KEY），续 R78/R79/R80 seed。
    - **低置信·须真机核（勿盲 seed）**：① SettingsConfiguration `rs-kv`（`bg-panel-2` 填充块）vs 下方 `rs-row`（`bg-panel` 边框卡）两种容器语言（finder aa5c·可能是 read-only 数据读出 vs 交互设置的刻意区分，须查 git log 确认意图再定）；② Scheduled 第 3 建议 cadence「Every 6 hours」vs 金标「Weekdays at 9:00 AM」（finder a7bf·仅值不同、呈现一致·价值边际）；③ WorkedFold 排在变更卡**之前** vs 金标 thread「Worked for…」在卡**之后**（finder a7bf·低置信·Codex 单轮语义不确定·须查 Timeline.tsx TAIL-ROW git log）；④ THREAD-CARD-DENSITY（R80 finder a7a8·须真机核）。
  - <2026-07-21 轮81>：对标 home/scheduled/rich/diff/model-menu/composer/sidebar/approval/mobile 全屏重截 live 并排金标(主驾驶自对标 + 2 finder settings/sched-thread 播种 + live 探针双证)、关差距 SETTINGS-GENERAL-CHROME(P2·General 面板 19px扁平行→22px rs-panel-title+有边框rs-row卡·加入共享 rs- 系统·追平其余6面+Codex 单一设计系统·golden单系统+内部一致性双锚)、核实成熟面(composer flat=R46/diff卡=文档/Finished-tab=无pause后端/model-menu/Suggestions/approval/sidebar)均刻意parity(避空转)、无 API key diff-mod 仍 blocked、开轮工作区干净无让路、主驾驶单点直接落 SettingsGeneral.tsx(非 worktree 批)、push 69e2214c、live=index-B_-ibON0.js、console panorama(4 section×light/dark) 0、2 finder(settings aa5c/sched-thread a7bf)诚实回报其余面无确定 in-scope 差距

- 2026-07-24 轮84（headless·主驾驶自对标 diff-review 分栏 × Codex 金标 codex-diff-review/codex-crop-diff-rendering + live 探针/截图多证·单点跨 3 文件主驾驶串行直接落）：**解锁 4 轮长期 blocked 的 DIFF-REVIEW-SPLIT 屏（金标标注"最重要"屏）并关闭其内一个真实功能差距。**
  - **★ 解锁发现（durable QA 基建）**：diff-review 屏此前 R78–R83 一直判"无 API key 造 modified 会话→无法核验删除行/word-level/fold band"。本轮实证 `webui/meta.go handleDiff` 的默认 `working-tree` scope **只是对 `meta.Workspace` 跑 `git diff HEAD` 的纯前端渲染，不需要 LLM turn、不 gate 于 session 存在**。遂在已知 diff 会话 `d8ac` 的 journal 指向的 dead workspace 路径（`~/.codex/worktrees/b875/…/wt-20260710-143427`）上**物化一个含真实 modification 的 git repo**（提交 baseline server.go+README → 改行/删行/加行/留大段未改行，15+/14−、2 文件），该真实会话的 Changes/diff 屏遂原生渲染出删除行/新增行/hunk/fold band/语法高亮/split 双列——**DIFF-REVIEW-SPLIT 彻底可信可测**。fixture 保留供后续轮复用（QA 数据保留纪律）。
  - **对标结论（避空转·诚实）**：逐行对标金标后判**已 parity、不制造假差距**：① 删除行代码文本经决定性探针实测是 **ink 色**（rgb 13,13,13，先前误把红 `.dl-sign` 当文本染色）——"删除行渲染"已 parity；② word-level 金标本身也无（modified 行 full-line tint，同我方）；③ fold band section context 是 git @@ heading、Codex 对代码文件亦显示；④ Commit-or-push glyph 是 DIFF-CP 刻意决策（DiffView.tsx:175 注释）。diff 逐行/inline 视图高度收敛。
  - ✅ **DIFF-SPLIT-FOLD-BAND（P2·真实功能差距·split 视图每多 hunk diff 可见·inline/split 自身不一致）**〔R84 关闭·push 3d082383〕：**split（并排）视图**把每个 hunk 边界渲染成裸 `.dl-hunk` section 标题——**丢了 inline 视图有的 `N unmodified lines` 计数 + caret + 点击展开**（DiffView.tsx:1157 `splitRows` 分支不走可折叠 `band()`）。→ split 视图用户既看不到折叠了多少行、也无法展开它们（功能缺失，偏离 Codex 始终可展开的 fold band）。修：给 `SplitRow` 加 `idx`（hunk 原始行索引）→ split 分支用它 key 回共享 `hunkGaps` 图，渲染同一 `.fd-gap` band（计数+caret+toggle）+ 展开时的 split 化 ctx 行 + trailing gap；tw.css 加 `.fd-split .fd-gap{grid-column:1/-1}` 让 band 跨全 5 列轨。**live 三证**(index-Dy9UKpS2)：split 视图 `.fd-gap` **0→4 个**（`3/2/9/3 unmodified lines` + caret，均 `<button>`）、点击展开 `.dls` 行 55→58（revealed 生效）、light+dark 均是；before(`split-desktop-light.png` 裸 `package server` 灰条无计数无 caret) / after(`split-foldband-{light,dark}.png` 带计数+caret) 截图直证。vitest **843 绿**（+新 `DiffView.splitfold.test.tsx` 2 例 + 更 `diffSummary.test.ts` idx 断言）+ build 绿。全景 console(home/rich/scheduled × light/dark) **0**（fixture 会话的 2 个附件图片 404 是 stale 附件专属、正交于 diff 功能，diff 面板本身 0 错误）。
  **本轮定性**：**非失败轮** — ①**解锁**了 4 轮 blocked 的最高价值屏（durable 基建：证明 diff 渲染不需 API key，物化可复用 fixture），②**关闭 1 个真实功能差距**（split 视图 fold band 从"无计数/不可展开"→ 追平 inline + Codex 可折叠 band，每个多 hunk diff 在 split 视图可见）。非 a11y/装饰 nit（是功能缺失 + inline/split 自身不一致）。四闸门绿：改动屏 before/after 对比确关小、qa/runs/2026-07-24-r84 存 live/after 截图 + 探针输出、三层文档齐活。主驾驶单点跨 3 源文件（diffSummary.ts+DiffView.tsx+tw.css）串行直接落（R77–R83 precedent 非 worktree 批），rebase 到 origin/main（并发 INC-101 无冲突）。
  - **仍开放 ☐（下轮 fresh-eyes 排·现 diff 屏已可测）**：
    - **DIFF 屏现已解锁可测**（fixture 在 `~/.codex/worktrees/b875/…/wt-20260710-143427`，绑真实会话 d8ac）——下轮可继续对标 diff 更细节（file-status glyph `M↓` vs 我方 `M` 字母·低价值、fold band content-preview 是否该省·存疑不改）或攻其他屏。
    - **ENV-OVERLAP-1440**（须真人 1440 实窗取证·INC-97 刻意浮层·续 R82/R83 seed）。
    - **低置信·须真机核**：① SettingsConfiguration rs-kv vs rs-row；② WorkedFold 排序；③ THREAD-CARD-DENSITY（续 R81/R82）。
  - <2026-07-24 轮84>：对标 diff-review 分栏 × Codex 金标(主驾驶自对标 + live 探针/截图多证)、**解锁 DIFF-REVIEW-SPLIT**(证明 working-tree diff=`git diff HEAD` 纯渲染无需 API key·在 dead workspace 路径物化 modified fixture 15+/14−·真实会话 d8ac 原生渲染)、关差距 **DIFF-SPLIT-FOLD-BAND**(P2·split 视图 `.fd-gap` 0→4·补 `N unmodified lines` 计数+caret+展开·SplitRow.idx key 回 hunkGaps·tw.css grid-column:1/-1)、判删除行渲染/word-level/commit-push glyph 均已 parity(避空转不造假差距)、开轮工作区干净无让路、主驾驶跨 3 源文件串行直接落、rebase 过并发 INC-101、push 3d082383、live=index-Dy9UKpS2.js、vitest 843 绿、console panorama 0

- 2026-07-24 轮85（headless·主驾驶自对标 home/rich/diff-thread/Review-split 全屏截 live 并排金标 + 3 并发 read-only finder 分屏取证 diff-split/rich-thread/scheduled+sidebar）：**带新鲜眼睛重截 live 全屏（含亲自点 Review 进 split 视图，最重要屏）逐屏并排金标**。主线独立核实 home(`codex-new-task-home`)/rich thread(`codex-task-thread`)/diff-thread/Review-split(`codex-crop-diff-rendering`) 均高度 parity。3 finder 里 2 个（rich-thread/scheduled+sidebar）诚实回报**无新 in-scope 差距**（成熟面防守严密），diff-split finder 领跑关闭 **1 个最重要屏、每次进 Review 即可见的真实功能/UI 差距**（单文件簇 DiffView.tsx+测试，主驾驶串行直接落，R77–R84 precedent 非 worktree 批，vitest 851 绿 + build 绿）：
  - ✅ **DIFF-FILE-EDGETOEDGE（P2→关闭·diff-split finder 领跑·金标+内部一致性+live 三证·最重要屏每次进 Review 可见）**〔R85 关闭·push 6a1fe767〕：Review/Changes 面板此前**桌面端**把每个文件框成 `rounded-[10px]` 卡片 + `m-3`(12px) 侧边距 + 完整边框——`fileCardClass`/`UntrackedFile` 的 edge-to-edge 只在 `narrow`(移动端)生效（`DiffView.tsx:815/828`），桌面保留卡片是遗留默认（git log 实证 `ee5a2f51` **仅**把移动端改边到边，桌面无 Codex-parity 论证）。这**双重框线**（面板 border + 卡片 border）加噪、且在本就 ~590px 的窄 split rail 里用 24px 侧边距挤占代码宽度（长行更早换行）。Codex `codex-crop-diff-rendering.jpg` 是**一条边到边连续代码流**——单 header + hairline 区隔相邻文件、无卡片框、贴面板左右边缘、全宽给代码。改：把两个调用点 `narrow`→`true`，桌面套用**已在移动端 shipping 的同一** `!m-0 !rounded-none !border-x-0`（保留 border-y 作文件分隔），并改写 199-204 注释（旧注释断言桌面"must keep the card shape"，其自身却承认"Codex lets file sections run edge-to-edge"、是自相矛盾的遗留论证）+ 更新 :990 陈旧注释（R84 后 split 也有 fold band）。**live 三证**(index-DsZImcRa)：Review split 右面板文件卡→连续流、代码贴面板边缘、`.golangci.yml` line 22「everywhere else」从折两行→单行放下（直证宽度增益）、light+dark 均是、mobile 390 无回退（thread 的 ArtifactRow 文档卡正确保留、非 diff 面板）；before(review-split-{light,dark}.png 带卡片框) / after(after/review-split-{light,dark}.png 边到边) 截图直证。vitest **851 绿**（改 DiffView.commit.test.tsx 桌面断言从"keeps cards framed"→"runs edge-to-edge too"）+ build 绿。全景 console(home/rich/diff × light/dark) **0**。
  **本轮定性**：**非失败轮** — 关闭 **1 个 diff-split finder 领跑·金标+内部一致性+live 三证、最重要屏（Review split）每次进入即可见的真实 UI 差距**：桌面 diff 从「一盒盒卡片 + 双重框线 + 24px 侧边距挤占代码宽」→ 边到边连续审阅面追平 Codex + 复用移动端 shipping 行为。非 a11y/装饰 nit（是宽度利用率 + 视觉整洁 + 移动/桌面自身一致性）。四闸门绿：改动屏 before/after 对金标复比确关小、qa/runs/2026-07-24-r85 存 live+after 双主题截图 + console 0、三层文档齐活。诚实：单文件簇 typography/layout 收敛，主驾驶串行直接落 + 3 finder 播种（2 空手 1 领跑），未派 worktree 批。
  - **★ 副产出（memo 闭合）**：scheduled+sidebar finder 代码实证 **scheduled-pause 后端已全链路接线**（前端 `useScheduledController.ts:427` api.schedule + Pause/Resume 按钮/右键菜单/Paused tab；后端 `webui/api.go:1571`+`composer_api.go:229`+ ar_test.go:246 有 pause 200 用例；第三 tab 已从 Finished 切为 **Paused**）——auto-memo「scheduled-pause backend unwired」已过时，本轮标注为已解决。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：
    - **ENV-OVERLAP-1440**（须真人 1440 实窗取证·INC-97 刻意浮层·续 R82/R83 seed）。
    - **低置信·须真机核**：① SettingsConfiguration rs-kv vs rs-row；② WorkedFold 排序；③ THREAD-CARD-DENSITY（rich-thread finder R85 复核：金标源图仅 819px 上采样、真机比例正确·续判勿盲 seed）。
    - diff 屏细节：file-status glyph `M↓` vs 我方 `M` 字母（STATUS_GLYPH/RV-3 刻意决策·✂）、删除行 hatch 纹理（a11y-adjacent 视觉·不做）。
  - <2026-07-24 轮85>：对标 home/rich/diff-thread/Review-split 全屏截 live 并排金标(主驾驶自对标含亲点 Review 进 split + 3 finder 分屏取证 + live 三证)、关差距 **DIFF-FILE-EDGETOEDGE**(P2·桌面 diff 卡片框→边到边连续审阅面·`narrow`→`true`·复用移动端 shipping 类·收 ~24px 侧边距给代码·金标+内部一致性+live 三证·最重要屏可见)、2 finder(rich-thread/scheduled+sidebar)诚实回报无新差距、副产出证实 scheduled-pause 后端已接线(memo 闭合)、开轮工作区干净无让路、主驾驶串行直接落 DiffView.tsx+test(非 worktree 批)、push 6a1fe767、live=index-DsZImcRa.js、vitest 851 绿、console panorama 0

- 2026-07-24 轮86（headless·主驾驶自对标 home/scheduled/rich/diff/Review-split 全屏重截 live 并排金标 + 3 并发 read-only finder 分屏取证 sidebar-projects/review-toolbar/home-composer + 2 并发 worktree implementer 落一个推一个）：**带新鲜眼睛重截 live 全屏逐屏并排金标**。home/composer finder 诚实回报**已 parity 无 in-scope 差距**（4 卡/headline/hero mark/chip strip/model pill/mic/send 全匹配，"Ask to approve"vs"Full access"是刻意安全默认+确认弹窗、Speed 行/Plugins/Attach Finder 均 out-of-scope 新后端）。另 2 finder 各领跑关闭 **1 个每屏可见的真实 UI 差距**，2 implementer 并发（touches 互斥·worktree 隔离·各自 vitest+build+push）：
  - ✅ **SIDE-PROJECT-HINT（P1→关闭·sidebar finder 领跑·golden+数据已存+live 三证·首要对齐面 sidebar 每屏可见）**〔R86 关闭·push a6ccbfec〕：同 basename 的两个 worktree 路径（如两个 `agentrunner-codex`）在 Projects rail 渲染成**两个完全相同的 repo 组头**——读起来像重复/bug 列表。view model **早已算好**去噪父路径消歧符存于 `ProjectGroup.hint`（viewModels.ts:386 `projectSubtitle`），但 sidebar **从未渲染它**（`SidebarProjectItem` 只画 `name`，`Sidebar.tsx` 未下传 `hint`）——未完成接线，非刻意抑制（无 QA-/INC-/R## 注释 gate）。修：`SidebarProjectItem` 加 `hint?` prop（SidebarItems.tsx:299/317）→ 在 `.proj-heading-name` 下渲染 `.proj-heading-hint` dim 第二行（仅 hint 为真时，单名 repo 不变）；`Sidebar.tsx:655` 传 `project.hint`；tw.css:412 加 `.proj-heading-hint{min-w-0 truncate text-[11px] leading-tight text-dim}` + `.project-group` 加 `mb-1.5`（组间留白·配合消歧让两 `agentrunner-codex` 组明确分开）。**live 三证**(index-DySCr-Xb→ywe_QJ_m)：`.proj-heading-hint` **0→5 个**（`a10c`/`07-23 06:24`/`07-23 03:54`/`main-flow`/`2026-07-11-QA-51`），两 `agentrunner-codex` 组显示各自时戳副标题、非 twin repo 无副标题保持单行；对照 golden codex-crop-sidebar-projects 密度/呼吸感一致。vitest 851 绿（改 Sidebar.nav.test.tsx twin 断言）+ build 绿。home/scheduled console(light/dark) **0**。
  - ✅ **DIFF-COLLAPSE-ALL-ICON（P2→关闭·review-toolbar finder 领跑·golden+内部一致性+live 三证·Review split 宽栏可见）**〔R86 关闭·push b74339b8〕：Codex Review 工具栏有一等 collapse-all/expand-all 图标；我方能力全接线（`onToggleAll`/`allShownOpen`）但控件**只在 `…` 溢出菜单里**（DiffParts.tsx:458 `fileCount>1`）·**每个宽度都埋着**——不像 Copy/Wrap/Split（宽栏一等图标·仅 barTight demote）·是未露出控件非刻意决策。修：加顶层工具栏 `IconButton`（gate `!empty && !barTight && fileCount>1`·置 `…` 与 search 之间对齐 golden 顺序·`ArrowsInLineVertical`/`ArrowsOutLineVertical`）；`…` 菜单项 gate 改 `barTight && fileCount>1`（窄栏 demote 一致 DIFF-CP/RD-8 模式）。**live 三证**(index-ywe_QJ_m)：宽栏(2100px·右栏>640px) 工具栏含 `Expand all files`（在 `More changes actions` 与 `Changed files` 间·与 Copy/Wrap/Inline/Split 并列）·点击翻转 Expand↔Collapse·light+dark 均是；窄栏(1300px) 工具栏无但 `…` 菜单仍有（reachability 不变）。vitest **855 绿**（新 DiffView.collapseall.test.tsx 4 例 + 改 DiffView.test/chrome.test）+ build 绿。
  **本轮定性**：**非失败轮** — 关闭 **2 个每屏可见的真实 UI 差距**：① sidebar 同名 repo 组从"看似重复的 bug 列表"→ 各带消歧副标题+组间留白（首要对齐面·数据早已存·纯未接线）；② Review 宽栏工具栏 collapse-all 从"埋在 `…` 里"→ 一等图标追平 Codex + 复用 app 自身 barTight demote 模式。均非 a11y/装饰 nit。四闸门绿：改动屏对 golden/兄弟控件复比确关小（sidebar after 截图 + hint 探针 0→5、collapse-all wide/tight 双模式探针 + toolbar 截图）、qa/runs/2026-07-24-R86 存 before/after + 探针输出、三层文档齐活。**2 worktree implementer 并发·touches 互斥（Sidebar/SidebarItems/tw.css ⊥ DiffParts.tsx）·各自 vitest+build+push·落一个推一个**（a6ccbfec 先落部署复验→b74339b8 后落 rebase 部署复验）。home/composer finder 诚实回报已 parity（避空转）。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：
    - **DIFF search-in-diff（净新前端·无后端）**：Codex Review 工具栏有 in-diff 内容搜索（match 高亮/next-prev），我方只有 file-path filter（ChangedFilesMenu）无行内容搜索——净新前端 feature（diff 文本已在 data.diff·无需后端）·下轮可评估价值/成本再定。
    - **SIDE-GROUP-SHOWMORE-INDENT（低·per-group "Show more" 对齐 folder gutter 而非 task 文本·差 ~23px·价值边际）**（sidebar finder 3）。
    - **ENV-OVERLAP-1440**（须真人 1440 实窗取证·INC-97 刻意浮层·续 R82/R83/R85 seed）。
    - **低置信·须真机核**：① SettingsConfiguration rs-kv vs rs-row；② WorkedFold 排序；③ THREAD-CARD-DENSITY（续 R81/R82/R85·金标源图仅 819px 上采样·勿盲 seed）。
    - diff 屏细节：file-status glyph `M↓` vs 我方 `M` 字母（STATUS_GLYPH/RV-3 刻意决策·✂）、删除行 hatch 纹理（a11y-adjacent·不做）。
  - <2026-07-24 轮86>：对标 home/scheduled/rich/diff/Review-split 全屏重截 live 并排金标(主驾驶自对标 + 3 finder sidebar/review-toolbar/home-composer 分屏取证 + live 三证)、关差距 **SIDE-PROJECT-HINT**(P1·同名 worktree 组渲染 dim 消歧副标题+组间 mb-1.5·数据早存 viewModels.ts:386 从未接线·首要对齐面每屏可见)+**DIFF-COLLAPSE-ALL-ICON**(P2·collapse/expand-all 从 `…` 提升为宽栏一等工具栏图标·复用 barTight demote 模式·对齐 Codex 顺序)、home/composer finder 诚实回报已 parity、**2 worktree implementer 并发·touches 互斥·落一个推一个**、push a6ccbfec+b74339b8、live=index-ywe_QJ_m.js、vitest 855 绿、console panorama(home/scheduled×light/dark) 0

- 2026-07-24 轮87（headless·主驾驶自对标 home/scheduled/rich/diff/Review-split 全屏重截 live 并排金标 + 3 并发 read-only finder 分屏取证 diff-toolbar/sidebar-thread/scheduled-home + 主驾驶串行落 3 条·落一个验一个）：**带新鲜眼睛重截 live 全屏（含亲点 Review 进 split）逐屏并排金标**。主线独立核实 home(`codex-new-task-home`)/scheduled(`codex-scheduled`)/thread change card(`codex-task-thread`)/Review-split(`codex-crop-diff-rendering`) 均高度 parity（4 卡/headline/hero mark/chip strip、Suggestions 段、fold band/gutter/语法高亮/红底删除行俱在）。**3 finder 全部领跑各交一个 golden+内部一致性锚定的真实差距**，主驾驶串行落全部 3 条（tw.css 两条不同区域 + DiffParts 一条·touches 互不重叠·R80–R85 主驾驶直接落 precedent·vitest 855 绿 + build 绿 + live DOM 探针三证）：
  - ✅ **SCHED-QUIET-DIM（P1→关闭·scheduled finder 领跑·掉功能回归·golden+内部一致性+live 探针三证·Scheduled 每屏可见）**〔R87 关闭·push d1c4f1d2〕：Tailwind 迁移 `68a8a69b` 删 `styles.scheduled.css` 时**未把 `.is-quiet` 规则搬进 tw.css**（grep `.is-quiet` in tw.css = 0），而 `ScheduledParts.tsx:161` 仍对 `!recover && !active` 的 settled/已完成行发出 `is-quiet` class——于是**所有行标题都渲染成同一 full-ink `font-semibold`**，抹平了 Codex active-vs-quiet 层级（`8f576fe3` INC-41 SCH-ICON 本意"停跑的行整行降调"被迁移弄丢，测试只断言 class 字符串存在故漏网）。修：tw.css `.scheduled-row.is-quiet .scheduled-copy b` 加 resting dim（`text-dim` + `font-weight:400 !important`），hover/focus 抬回 `text-ink-2`（quiet 是静息非禁用态）。**font-weight 须 `!important`**：标题 `<b>` 带 `font-semibold` 工具类（utilities 层），否则层序压过我 components 层规则（`text-dim` 无竞品工具类故正常生效）——这正是先前探针 color 变 dim 但 fw 仍 600 的根因，加 !important 后修复。**live 探针三证**(index-7l-K-7FX)：quiet 行 `b` fontWeight **600→400** + color **rgb(13→85)** dim；active 行保持 fw600+ink(13,13,13)；after 截图直证成功已结束行(Ran Nd ago)后退、active(spinner)与 failed(需关注·recover)行保持 bold ink——两态一眼可分。
  - ✅ **SIDE-NAV-INK（P1→关闭·sidebar finder 领跑·层级倒置·golden+内部一致性+live 探针三证·首要对齐面 sidebar 每屏可见）**〔R87 关闭·push d1c4f1d2〕：primary-nav 标签（New session / Scheduled）此前 rest 态 `text-ink-2`（#606060）——**比其下每一条 session/repo 行(`text-ink` #0d0d0d)都更暗**，是 rail 里唯一被 dim 的文本层，把 app 两个最主入口渲染成最弱视觉（层级倒置）。R82 `42f13b97` 注释只谈字号 15→13px/pitch 40→32px、从未谈颜色——legacy carryover 非刻意（git log -L 证 `text-ink-2` 自 Tailwind 迁移 `68a8a69b` 一直未改）。Codex `codex-crop-sidebar-nav` nav 标签近黑、对比靠灰色 leading icon 而非 dim 标签。修：tw.css:356 label `text-ink-2`→`text-ink`，仅图标 dim（`[&>svg]:text-ink-2`）；active 态遂只需加重字重、无 rest→active 颜色跳变。**live 探针三证**：`.primary-nav button` label **rgb(96→13)** ink、icon 保持 rgb(96,96,96) ink-2。
  - ✅ **DIFF-SEARCH-GLYPH（P2→关闭·diff-toolbar finder 领跑·affordance 正确性·golden+内部一致性+live 探针三证·Review split 工具栏可见）**〔R87 关闭·push 680f6dee〕：Review/Changes 工具栏的 changed-files jump/filter trigger（`DiffParts.tsx:307`）渲染 `TreeStructure`（文件树图标），但金标 `codex-crop-diff-header` 该槽是放大镜、且该按钮打开的 popover 本身就是文件搜索/过滤（`SearchField` + 空态 `FileMagnifyingGlass`）、R86 自己注释也称它"file-search icon"——tree 图标误发"层级/导航"信号而非"查找"。git log -S 证 `TreeStructure` 仅来自机械抽取 `99a42839`、非刻意。修：swap 成已 import 的 `FileMagnifyingGlass`（同文件 :237/:757 已在用·视觉一致）+ 删 unused `TreeStructure` import。**live 探针三证**：trigger svg 从 tree（多 node）→ 单 path 文件角起笔 `M213.66,82.34l-56-56`(FileMagnifyingGlass)。
  **本轮定性**：**非失败轮** — 关闭 **3 个 golden+内部一致性+live 探针三证、每屏/每次进入即可见的真实差距**：① Scheduled 掉功能回归修复（settled 行从"全 bold 抹平"→ 恢复 active-vs-quiet 层级）；② sidebar 首要对齐面层级倒置修复（nav 从"最暗文本层"→ 近黑锚定 + 灰图标）；③ Review 工具栏搜索 affordance 从"tree 图标不可辨"→ 放大镜追平金标。均非 a11y/装饰 nit（两条是掉功能/层级倒置回归、一条是 affordance 正确性）。四闸门绿：改动屏对金标/兄弟行复比确关小（scheduled after 截图 + fw600→400/color 探针、nav ink 探针、diff glyph svg 探针）、qa/runs/2026-07-24-r87 存 before/after + 探针输出、全景 console(home/scheduled/rich/diff × light/dark) **0**、三层文档齐活。主驾驶串行落 3 条（tw.css 两不同区域 + DiffParts·touches 互斥·R80–R85 precedent 非 worktree 批），修了探针初测暴露的 SCHED font-weight 层序 bug（!important）。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：
    - **DIFF in-diff content search（净新前端·无后端）**：Codex Review 工具栏有行内容搜索（match 高亮/next-prev），我方 `ChangedFilesMenu` 只 filter `e.path`（DiffView.tsx:691）无行内容搜索——净新前端 feature（diff 文本已在 data.diff）·下轮评估价值/成本再定（diff finder R87 复核确认无、但无法从静态 crop 证 Codex 是内容搜索 vs 我方已有 path filter）。
    - **ENV-OVERLAP-1440**（须真人 1440 实窗取证·INC-97 刻意浮层·续 R82/R83/R85 seed）。
    - **低置信·须真机核**：① SettingsConfiguration rs-kv vs rs-row；② WorkedFold 排序；③ THREAD-CARD-DENSITY（续 R81/R82/R85·金标源图仅 819px 上采样·勿盲 seed）。
    - diff 屏细节：file-status glyph `M↓` vs 我方 `M` 字母（STATUS_GLYPH/RV-3 刻意决策·✂）、删除行 hatch 纹理（a11y-adjacent·不做）、SIDE-GROUP-SHOWMORE-INDENT（~23px·价值边际·续 R86）。
  - <2026-07-24 轮87>：对标 home/scheduled/rich/diff/Review-split 全屏重截 live 并排金标(主驾驶自对标含亲点 Review + 3 finder diff-toolbar/sidebar-thread/scheduled-home 分屏取证 + live 探针三证)、关差距 **SCHED-QUIET-DIM**(P1·Tailwind 迁移掉 `.is-quiet` 规则回归·settled 行恢复 dim/normal·font-weight !important 破层序·golden+回归+探针三锚)+**SIDE-NAV-INK**(P1·primary-nav 标签从 text-ink-2 层级倒置→text-ink+灰图标·首要对齐面每屏可见)+**DIFF-SEARCH-GLYPH**(P2·changed-files trigger TreeStructure→FileMagnifyingGlass·affordance 正确性·对齐金标+popover 自身搜索)、home/scheduled/thread/Review-split 主线核实高度 parity(避空转)、开轮工作区干净无让路、主驾驶串行落 3 条(tw.css 两区+DiffParts·touches 互斥·非 worktree 批)、修 SCHED font-weight 层序 bug、push 680f6dee+d1c4f1d2、live=index-7l-K-7FX.js、vitest 855 绿、console panorama(home/scheduled/rich/diff×light/dark) 0

- 2026-07-24 轮88（headless·主驾驶自对标 home/scheduled/rich/diff/Review-split 全屏重截 live 并排金标 + 3 并发 read-only finder 分屏取证 settings/diff-split/thread-env + 主驾驶串行落 1 条·落一个验一个）：**带新鲜眼睛重截 live 全屏（含亲点 Review 进 split）逐屏并排金标**。主线独立核实 home(`codex-new-task-home`)/scheduled(`codex-scheduled`)/rich thread(`codex-task-thread`)/Review-split(`codex-crop-diff-rendering`) 均高度 parity。**3 finder 里 2 个（diff-split/thread-env）诚实回报无新 in-scope 差距**（成熟面防守严密·候选全落刻意 ✂），settings finder 领跑关闭 **1 个每次进 Settings 落地页可见的真实结构一致性差距**（单文件簇·主驾驶串行直接落·R80–R87 precedent 非 worktree 批·vitest 855 绿 + build 绿 + live DOM 探针双证）：
  - ✅ **SETTINGS-ROW-DESC-STACK（P2→关闭·settings finder 领跑·内部一致性+live 探针+before/after 三证·Settings Appearance/Git 每次进入可见）**〔R88 关闭·push c65cecb8〕：Appearance（Theme/Contrast/Diff markers/FontRow/ToggleRow）与 Git（commit template）把 `rs-row-label` + `rs-row-desc` 渲染成 `.rs-row-head`（`flex justify-between`·tw.css:1458）的**两个兄弟节点**——桌面端描述文字遂被推到卡片**最右缘**、与标题同基线、中间大空隙、与其所述控件也空间分离；而 General/Configuration **早已**把 label+desc 包进单个 `min-w-0` 左列（title-over-subtitle·SettingsGeneral.tsx:42-50）。R81 SETTINGS-GENERAL-CHROME 统一 General 时**未触及 Appearance/Git**（`git log` 证分歧源自 INC-41 bulk 创建 `7512c6b1`·无 QA/INC/R 背书右浮·移动端 `max-[500px]:grid-cols-1` 已 re-stack 证堆叠才是本意·桌面右浮只是宽屏副作用）。修：把 label+desc 包进共享 `<div className="min-w-0">`（镜像 General）+ desc 加 `mt-[3px] leading-[1.5]`，所有断点均 title-over-subtitle·移动端 grid/flex-col 冗余类删除（ToggleRow 保留 `max-[500px]:pr-12` 给绝对定位 switch）。**live 探针双证**(index-DU1Qs0nY)：Theme/Contrast/UI font size/Commit-template × light/dark 全 `sameParent:true`+`parentCls:min-w-0`+`descLeft==labelLeft`(leftDelta **0**·先前右浮)+`descBelowLabel:true`；before(`finder-settings-appearance-light.png` desc 右浮) / after(`verify-appearance-{light,dark}.png` desc 堆叠左对齐) 截图直证。vitest **855 绿**（更 `SettingsAppearance.mobile.test.tsx` 堆叠断言从 `max-[500px]:grid-cols-1` class 检查→`themeLabel.parentElement===themeDesc.parentElement` + `min-w-0` 结构检查）+ build 绿。
  **本轮定性**：**非失败轮** — 关闭 **1 个 settings finder 领跑·内部一致性+live 探针+before/after 三证、每次进 Settings Appearance/Git 落地页即可见的真实结构一致性差距**：设置行描述从「桌面端右浮到卡片最右缘、与标题脱节」→ 堆叠在标题正下方追平 General/Configuration + Codex 的 title-over-subtitle 行文法。非 a11y/装饰 nit（是布局一致性·R81 漏改的同类分歧）。四闸门绿：改动屏 before/after 对兄弟面/金标复比确关小、qa/runs/2026-07-24-r88 存 before/after + 探针输出、全景 console(home/settings-appearance/settings-git/rich × light/dark) **0**、三层文档齐活。主驾驶单文件簇串行直接落（SettingsAppearance.tsx+SettingsGit.tsx+mobile.test·touches 互斥·非 worktree 批），rebase 到 origin/main（无并发冲突）。diff-split/thread-env finder 诚实回报无新差距（避空转）。
  - **★ 副产出（探针胜过读图）**：review-split-dark 截图曾显示"左 light / 右 dark"·疑似 Review split 主题不一致回归；`elementFromPoint` 三点决定性探针证 sidebar/thread/diff 全 dark（rgb 20,20,22 / 15,15,17 / 23,23,26）——是 headless Chrome 对新挂载 split 面板的 **stale-paint artifact**（像素≠computed 态），**非真 bug**。diff-split finder 独立观察到同一现象、亦判截图时序·两方一致。避免了追幻影 bug。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：
    - **SETTINGS-CONFIG-KV-SEAM（✂/P3·可选)**：SettingsConfiguration `rs-kv`(bg-panel-2 填充块)vs 下方 policy `rs-row`(bg-panel 有边框卡)两种容器语言相邻·fill-vs-border 接缝。settings finder R88 决定性证 R87「read-only vs 交互」理论不成立(policy 行也 read-only 却用 card)·但**密度上可辩护**(4 行一句 fact 用紧凑 KV 表优于炸成 4 卡·Codex 亦用 row-style readout)·**建议不转卡**·若动仅最小对齐 fill(bg-panel-2→bg-panel+border)消接缝·非 parity failure。
    - **WORKED-FOLD-CARET（✂/低·内部一致性双族）**：Worked-fold/RetriedFold/tool-fold(act-caret) 用右向 caret(rotate-90)·而 Environment Worktree 抽屉/"Show N more" 用下向 caret(SupervisionPanel.tsx:621 注释明说右 caret 对 inline 展开是错的)——但 Worked-fold 与其**真正同族**(thread 内容折叠:tool-fold/RetriedFold)一致·改它反致新不一致·且金标 819px 无法证伪 Codex caret 方向·**须高清 Codex Worked-fold 探针 + 决定 thread 内容折叠该用哪族 caret 再定**。
    - **ENV-OVERLAP-1440**（须真人 1440 实窗取证·INC-97 刻意浮层·续 R82/R83/R85 seed）。
    - **低置信·须真机核**：THREAD-CARD-DENSITY（续 R81/R82/R85·金标源图仅 819px 上采样·勿盲 seed）。
    - **DIFF in-diff content search（净新前端·无后端）**：续 R86/R87·评估价值/成本再定。
    - diff 屏细节：file-status glyph `M↓` vs `M`（RV-3 刻意 ✂）、删除行 hatch 纹理（a11y-adjacent·不做）、SIDE-GROUP-SHOWMORE-INDENT（~23px·价值边际·续 R86）。
  - <2026-07-24 轮88>：对标 home/scheduled/rich/diff/Review-split 全屏重截 live 并排金标(主驾驶自对标含亲点 Review + 3 finder settings/diff-split/thread-env 分屏取证 + live 探针)、关差距 **SETTINGS-ROW-DESC-STACK**(P2·Appearance/Git 设置行描述桌面端右浮→堆叠标题下·套 General min-w-0 包裹·追平 title-over-subtitle 行文法·R81 漏改同类·内部一致性+探针+before/after 三证·每次进 Settings 可见)、diff-split/thread-env finder 诚实回报无新差距、副产出 elementFromPoint 探针排除 review-split-dark 幻影主题 bug(stale-paint artifact)、开轮工作区干净无让路、主驾驶单文件簇串行直接落(SettingsAppearance/Git/mobile.test·非 worktree 批)、push c65cecb8、live=index-DU1Qs0nY.js、vitest 855 绿、console panorama(home/settings×2/rich×light/dark) 0

- 2026-07-24 轮89（headless·主驾驶自对标 home/scheduled/rich/diff/review-split/approval/model-menu × light/dark × 1440 + 3 并发 read-only finder：sidebar-projects / scheduled-list / thread+split）：**失败轮（诚实标注）——穷尽对标后未发现「已有后端 · 非刻意 · 非 nit · 非 a11y」的可见 in-scope 差距可关闭。**
  - **本轮做了什么**：截 live 8809 全屏并排 5 张 Codex 金标 + 3 张裁剪组件图（sidebar-projects/composer/model-dropdown/artifact-cards/change-card/scheduled-list/scheduled-suggestions/diff-header/diff-rendering/newtask-emptystate）；点 Review 按钮渲染真实 diff split；开 model 菜单。3 finder 逐屏取证 + git log -S 判刻意。
  - **为什么是失败轮（每条候选的判决）**：
    - **sidebar Projects「二级副标题 + 同 repo 多组」**（我方最初判为最大结构差距）→ **✂ 刻意**：hint 副标题 = `SIDE-PROJECT-HINT`（`a6ccbfec`/`b6591b29`，今日为 Codex parity 落地，仅 twin 同名时出现），per-workspace 分组 = INC-78.1 用户裁决（`42722e02`，2026-07-19，DESIGN §12）。Codex 无二级行是因为它 by-repo 分组无 twin；我方 by-workspace 有 twin 故消歧。**不得回退，需重裁 INC-78。**
    - **scheduled 行密度（title/cadence 堆叠）**→ **无差距**：`codex-crop-scheduled-list` 证 Codex **也**堆叠（title 行 1 / cadence·next 行 2），与我方 `ScheduledParts.tsx:206-238` 一致。suggestion 图标我方 = 小彩色 inline glyph（无 tile 背景），亦匹配。Mark-all-as-read/unread dot 均已实现（数据为 0 故未显）。唯一 nit：suggestion 图标 22→20px（finder 判「near-imperceptible」）。
    - **artifact 破图占位卡**→ **✂ fixture 非代码**：dead-workspace fixture 无真实字节 → `fileURL` 404 → `onError` → 占位（`ChangesOutcome.tsx:54-56` 刻意优雅降级）。真会话有字节则正常渲染缩略图。修在 fixture 层非组件。
    - **diff Review split**（金标最重要屏）→ **已至/超 parity**：toolbar `DiffToolbar`（`DiffParts.tsx:726`）已含 CommitPushMenu(:899)、ChangedFilesMenu 文件搜索(:839)、collapse-all 一级图标(:820)、copy/wrap/inline-split 切换、scope picker——比 Codex toolbar 更全；1440 窄分栏下 barTight 折叠属正确响应式。逐文件折叠头 + +/- + 语法高亮 + unmodified fold band 均在。状态字形 `M↓` vs `M` = RV-3 ✂。
    - **Environment 浮层遮挡正文**（finder C 判「唯一真实可见 gap」）→ **✂ 刻意（float vs reflow 设计哲学）**：`SessionFeature.tsx:623-624` 明述「floating card 不 shift 对话列」；探针实证 1280/1440 正文右缘（1209@1440）确实伸入 340px 右侧浮层下、1680 才不遮。但修复 = 给正文加右 gutter/收窄 wrap = 采用 Codex 的 reflow 哲学 = **反转刻意的 float 决策**，非 parity 微调可单方做，须用户重裁。**本轮不擅改。**
    - **model 菜单**：我方 = 多页 drill-in（Model/Effort/Advanced 各带 CaretRight，`ComposerParts.tsx:1039`），Codex「Advanced ^」= inline-expand 上 caret 左对齐 + 有 `Speed` 行（我方无 Speed 后端）。Advanced caret 方向差属小且「哪个更好」不确定、matching 须重构；Speed 无后端 out-of-scope。
    - **home 建议卡**：实测 166×104 · pad 14/16 · radius 12，紧凑，与金标 ~1.6 比例一致。**无差距。**
    - Codex 独有（Plugins/Sites/Pull requests/Chat/Speed/Background-processes/Browser/Sources）= 无后端 **out-of-scope**（Background-processes 我方有数据时已渲染）。
  - **下轮补法（关键 process 发现）**：① **金标图分辨率是瓶颈**——现存 `qa/codex-reference/*` 为 819px 上采样，遮蔽了 typography/spacing/密度 的次级差距（连续多轮「parity/mature」部分是低分辨率金标的产物）。**需交互 session / 真人用 Computer Use 重截真像素 Codex 桌面 app** 补 `qa/codex-reference/`，才能暴露下一层 in-scope 视觉差距。② **Environment 浮层遮挡正文 @1280/1440** 是真实可见退化，值得作为 float-vs-reflow **设计重裁** 提给用户（非 headless 单方改）。③ 下轮换未深审面（mobile 390 需 Codex mobile 金标才可比；Settings 无 Codex 金标）。
  - 开轮工作区干净无让路；主驾驶仅 read-only 探针 + 截图（无源码改动）；未 push 源码（仅本台账 doc）；3 finder 无遗留在跑。live=index-DU1Qs0nY.js（未变）。qa/runs/2026-07-24-R89/ 存全屏截图 + model/add 菜单 + realsplit + 探针输出。console panorama 沿用 R88（本轮无源码改，未回归）。

- 2026-07-24 轮90（headless·主驾驶自对标 home/scheduled/rich/diff/Review-split + 亲开 Cmd+K 命令面板 + Scheduled 逐屏并排金标/裁剪 crop + live 探针三证·主驾驶单文件簇串行落一条）：**非失败轮 — 关闭命令面板对 Codex 的 2 个每次桌面 Cmd+K 即见的可见 chrome 差距。**
  - 本轮带新鲜眼睛重截 live 全屏并排金标 + 裁剪 crop（home/rich/diff/Review-split/change-card/diff-header/diff-rendering/message-actions/command-palette/scheduled）。逐屏独立核实：home/rich/Review-split/change-card(路径 dim+文件名 bold+右对齐 ±)/diff-header/diff-rendering(unmodified 折叠带我方已有)/scheduled(mark-all-read 仅 unreadCount>0 显,数据为0故不显·parity)/message-actions(copy+copy-link 已有；thumbs↑↓需 feedback 后端 out-of-scope) 均高度 parity。
  - ✅ **CMDK-FIELD-SEAMLESS + CMDK-CLOSE-DESKTOP-HIDE（P2→关闭·live 探针三证·每次桌面 Cmd+K 即见）**〔R90 关闭·push 0907d617·live=index-DLr9tkg5.js〕：命令面板搜索栏 2 个偏离 Codex 极简命令栏（`codex-crop-command-palette.jpg` 无边框/无蓝/无 X）的可见差距——① `flush` SearchField variant（仅命令面板用·Field.tsx:197）在面板自动聚焦瞬间闪 `focus-within:border-blue + ring-2 ring-blue/30` 内嵌蓝环，读作带框表单输入而非无缝命令栏 → 去蓝、仅留底部 hairline；② X 关闭钮设计为 mobile-only（`.cmdk-close` base `hidden` → ≤680px `grid`），但它是 `<IconButton>`·Button 基类带 `inline-flex` utility，utilities 层压过 components 层 `hidden` → **X 泄漏到桌面**（探针 `display:flex`+visible@1440）→ `!hidden`（+ mobile 块 `!grid`）强制回 mobile-only 意图（Codex 桌面亦无 in-field close·Esc/click-away 关）。**live 探针三证**（index-DLr9tkg5）：桌面 light/dark `cmdk_close_display:none`+`search borderBottomColor` 转中性 hairline(rgb 220,220,220 / 58,58,66·先前 rgb 1,105,204 蓝)+`boxShadow:none`(先前 blue inset ring)；mobile390 `cmdk_close_display:grid`(X 仍显·mobile 保留)；before(`cmdk-light.png` 蓝框+X)/after(`verify-cmdk-{light,dark}.png` 无缝无X) 截图直证。vitest **855 绿**（CommandPalette.test 18 绿·close 钮仍在 DOM·jsdom 不套 display 不受影响）+ build 绿。
  **本轮定性**：**非失败轮** — 关闭 2 个 live 探针三证、每次桌面 Cmd+K 即见、且对齐 Codex `codex-crop-command-palette.jpg` 极简命令栏的可见 chrome 差距（去蓝色 focus 环 + 修桌面 X 泄漏）。非 a11y（是**去除**装饰性蓝环、非加焦点环）·非刻意✂（X 的 base-hidden+mobile-grid 证明 mobile-only 是本意·泄漏是 cascade bug）·非 nit（每次开命令面板桌面即见）。四闸门绿：改动屏 before/after 对金标复比确关小、qa/runs/2026-07-24-R90 存全屏+cmdk before/after+探针输出、console panorama(home/rich/cmdk × light/dark) **0**、三层文档齐活。flush variant 仅 CommandPalette 用（grep 证）故改共享 variant 安全无外溢。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：① 金标图分辨率瓶颈（819px 上采样·需真人 Computer Use 重截真像素补 `qa/codex-reference/`）续 R89；② ENV-OVERLAP-1440 float-vs-reflow 设计重裁（提用户·非 headless 单方改）续 R82/83/85/89；③ SETTINGS-CONFIG-KV-SEAM(✂/P3)、WORKED-FOLD-CARET(✂/低·须高清 Codex Worked-fold 探针)、THREAD-CARD-DENSITY(须真机核)、DIFF in-diff content search(净新前端无后端·评估价值)。
  - <2026-07-24 轮90>：对标 home/scheduled/rich/diff/Review-split + 亲开 Cmd+K + Scheduled 逐屏并排金标/crop、关差距 **CMDK-FIELD-SEAMLESS**(flush 搜索栏去蓝色 focus 环·仅留 hairline·对齐 Codex 无缝命令栏) + **CMDK-CLOSE-DESKTOP-HIDE**(修 IconButton inline-flex 压过 `.cmdk-close hidden` 致 X 桌面泄漏·`!hidden`/`!grid` 回 mobile-only)、home/rich/diff/scheduled/message-actions 主线核实高度 parity(避空转·thumbs 需 feedback 后端 out-of-scope)、开轮工作区干净无让路、主驾驶单文件簇串行直接落(Field.tsx flush variant + tw.css cmdk-close·touches 互斥·非 worktree 批)、push 0907d617、live=index-DLr9tkg5.js、vitest 855 绿、console panorama(home/rich/cmdk×light/dark) 0

- 2026-07-24 轮91（headless·主驾驶自对标 home/scheduled/rich/diff 全屏+裁剪 crop + 3 并发 read-only finder〔home+composer / sidebar / thread〕+ 3 并发 worktree-隔离 implementer·touches 两两互斥·各自 vitest+build 绿后自推）：**非失败轮 — 关闭 3 个 sidebar/thread 对 Codex 的可见差距。**
  - 本轮带新鲜眼睛重截 live 全屏并排金标（home/scheduled/rich/diff × light/dark @1440）。主驾驶先核实：change-card(路径 dim+文件名 bold+±右对齐绿/红)、diff-split、scheduled 行 py-3 airy(R68 刻意)、home 卡几何 均已 parity/intentional → 派 3 finder 系统排查。finder 交叉核对后判为非差距/OUT-OF-SCOPE 的：task 行缩进/密度/folder icon(sidebar·已 parity)、Worked-fold 头、composer 结构(Full-access/provider/worktree chip 属内容/后端差·OOS)、Env 行密度、account row(SB-12 无账户后端 OOS)、composer context-usage ring / thumbs 反馈(无后端 OOS)。
  - ✅ **SIDE-PROJ-HEAD-INLINE（HIGH·finder 定位 file:line·Impl A tw.css）**〔push 309f66a3〕：Projects 里 repo 头原为 name 一行 + dim 消歧 hint(a10c/07-23/main-flow)另起一行**堆叠两行**，比 Codex(`codex-crop-sidebar-projects.jpg` repo 单行、无副行)臃肿。`.proj-heading-text` `flex-col`→`flex-row items-baseline gap-[6px]`、`.proj-heading-hint` 加 `shrink-0`，改为「name  hint」单行内联（保留 basename-twins 消歧 hint 后端，仅改 layout）。live 复验：agentrunner-codex 07-23 06:24 / agentrunner a10c / workspace main-flow / ws 2026-07-11-QA-51 均单行——rail 恢复 Codex 单行节奏。
  - ✅ **SIDE-SECTION-CARET-DROP + SIDE-SHOWMORE-COUNT-DROP（MEDIUM×2·Impl B Sidebar.tsx）**〔push ce129479〕：① section header("Pinned"/"Projects")原带 10px `CaretRight`(把标签推到 x≈25、比其下 folder 列 x=8 更靠右)，Codex(`codex-crop-sidebar-nav.jpg`)section 标签为纯文字、与 folder 列对齐 → 删两处 CaretRight(+ 未再用的 import)，section-toggle 点击折叠行为不变；② "Show more · N"(240/294)计数后缀 Codex 无 → 去可见 `· {count}`(aria-label 计数保留)。live 复验:"Projects" 纯文字左对齐、底部 "Show more" 无计数。Sidebar.nav.test.tsx 两处断言随可见文本去计数按原意更新。
  - ✅ **ARTIFACT-CHIP-TITLE-WEIGHT（MEDIUM·Impl C ChangesOutcome.tsx·仅 ArtifactCard 文档卡·非 change-card）**〔push 36c24c9d〕：artifact/document 卡标题原 `text-[15px] font-[550]` 比 Codex(`codex-crop-artifact-cards.jpg` 非粗标题)偏粗、行偏紧 → 标题 `font-[550]`→`font-[500]`、行 `py-[10px]`→`py-[13px]`。live 复验(rich·rd-d-untracked-probe.txt 卡):标题变轻、行更宽。
  **本轮定性**:**非失败轮** — 3 个 finder-定位、file:line 精确、用户可见、有后端支撑的 UI/UX 差距全部关闭并复验。非 a11y(sidebar 布局/字重·非焦点环)·非 nit(sidebar 每屏即见)。四闸门绿:改动屏 before/after 对金标复比确关小、qa/runs/2026-07-24-R91{,/after} 存 before/after 全屏、console panorama(home/rich × light/dark) **0**、vitest 855 绿 + build 绿。
  - **让路**:开轮工作区有并发 session 的 DiffParts/DiffView/DiffView.commit.test 脏文件(非本轮产生)——不 add/revert/rebase;部署时 stash→ff origin/main→build 纯净→pop 恢复,不把其在途工作带上 8809。
  - **T-GAP1 ENV-OVERLAP(遮挡·HIGH·finder 确认真实退化但记待裁决不 headless 改)**:Environment 浮层(`position:absolute right-3 top-60 w-340`)压住 720px 居中正文列右侧(goalColu…/.cm-panels 文字被截),但 `tw.css:766 .session-layout.environment { grid-template-columns: minmax(0,1fr) }` 是 **INC-97 明确决策**("Environment 是 inspection overlay·不给第二列")。finder 提的 `pr-[360px]` reflow 修法与该文档决策抵触 → 按 PROCESS **不 headless 单方覆盖**,续 R82/83/85/89/90 作 float-vs-reflow **设计重裁**提用户。
  - **仍开放 ☐(下轮 fresh-eyes 排·均 tw.css 本轮已被 Impl A 占用故让路下轮)**:① HOME-CARD-LABEL-12PX(H-GAP1·finder 实测 Codex ~12px·我方 13px 致第 4 卡 "Fix issues and failures" 折 2 行·tw.css:514/1553)；② HOME-CHIP-STRIP-TINT(H-GAP2·Codex composer chip 条 #F5F5F5 灰底·我方纯白无 seam·**部分反 R46 HOME-CHIP-FLAT 需裁**)；③ ENV-GOAL-SETTLED-GRID(T-GAP2·env 面板 settled-goal 段 items-center 破栅格·或按 Codex 不在面板复述 goal·SupervisionParts.tsx:140+tw.css)；④ 金标图分辨率瓶颈(续 R89/90·需真人 Computer Use 重截真像素)；⑤ ENV-OVERLAP 设计重裁(见上)。
  - <2026-07-24 轮91>:比对 home/scheduled/rich/diff 4 屏×2 主题、3 finder 系统排查、关差距 **SIDE-PROJ-HEAD-INLINE**(repo 头 name+hint 两行→单行内联) + **SIDE-SECTION-CARET-DROP+SIDE-SHOWMORE-COUNT-DROP**(去 section caret+去 Show-more 计数) + **ARTIFACT-CHIP-TITLE-WEIGHT**(文档卡标题 550→500·行 10→13)、派工 3(并发·worktree 隔离·touches tw.css/Sidebar.tsx/ChangesOutcome.tsx 互斥·各自推)、push 309f66a3+ce129479+36c24c9d、live=index-B1gijoaj.js、vitest 855 绿、console panorama 0、让路 DiffView(stash/ff/pop 部署)、T-GAP1 ENV-OVERLAP 记待裁决(INC-97 冲突)

- 2026-07-24 轮92（headless·主驾驶自对标 home 全屏并排金标 + live 探针实测卡片折行 + 2 并发 read-only finder〔富 thread / scheduled+sidebar+settings〕+ 主驾驶串行落 2 条单文件簇·touches 互斥·落一个推一个）：**非失败轮 — 关闭 2 个对 Codex 的可见差距（home 建议卡折行 + sidebar section 图标）。**
  - 本轮带新鲜眼睛重截 live home × light/dark @1440 并排 `codex-new-task-home`/`codex-crop-newtask-emptystate`。逐屏并排即见 home 建议卡第 4 张折行差异 → **live 探针实测**（非读图猜）：现 13px 时 4 卡折 2/2/2/2 行，12px 时折 2/2/2/1——精确复现 Codex 金标的 2/2/2/1 pattern（card 4 "Fix issues and failures" 单行）。2 finder 系统排查富 thread / scheduled+sidebar+settings。
  - ✅ **HOME-CARD-LABEL-12PX（H-GAP1·主驾驶自对标+探针确认·续 R91 seed·Impl A tw.css）**〔push 89bb70ba·live=index-DVeODpYu.js〕：home 建议卡 label 从 `text-[13px]`→`text-[12px]`。Codex 金标（`codex-new-task-home`/`codex-crop-newtask-emptystate`）卡 label ~12px 使第 4 卡 "Fix issues and failures" 单行、卡片读 2/2/2/1；我方 13px 致第 4 卡折 2 行（2/2/2/2）比 Codex 臃肿。**live 探针三证**（index-DVeODpYu）：改后 4 卡 font 12px、折行 2/2/2/1（card 4 单行）、before(`home-{light,dark}.png` card4 双行)/after(`after-home-{light,dark}.png` card4 单行)截图直证。R52 曾定 13px「evening wrap」但那使 card 4 也双行偏离 Codex；12px 恢复 Codex 精确 wrap 节奏。vitest **856 绿** + build 绿。
  - ✅ **SIDE-PINNED-PIN-DROP（sidebar finder 领跑·file:line 精确·Impl B Sidebar.tsx）**〔push 47c5a0c2·live=index-DVeODpYu.js〕：sidebar "Pinned" section 标题原带 `<PushPin size={12} weight="fill" />` glyph（Sidebar.tsx:581·e4ed4038 泛化 nav polish 顺手加），是三个 section 标题（Pinned/Projects/Sessions）里**唯一**带图标的——既偏离 Codex 金标（`codex-crop-sidebar-nav` Pinned/Projects 均纯灰文字·无 glyph），又与自己 Projects/Sessions 兄弟标题不一致，还违反 R47 tw.css 注释明述的「quiet title-case label·no glyph」意图。删 glyph + 未再用的 `PushPin` import。**四证三角定位**：代码删除（纯删·不可能再渲染 pin）+ 金标（Codex 纯文字）+ vitest 856 绿（`Sidebar.nav.test` 用 pinned fixture 渲染 Pinned section、`getByRole("button",{name:"Pinned"})` 名称仍解析为纯 "Pinned"）+ console 全景 0。live 数据无 pinned 会话故 Pinned section 不渲染、context-menu pin 自动化未命中——此为纯 glyph 删除，四证足够（非需像素复比的加法）。
  **本轮定性**：**非失败轮** — 关闭 2 个用户可见、有后端支撑、非 a11y/非 nit 的 UI/UX 差距：① home 建议卡折行追平 Codex 2/2/2/1（主驾驶亲对标+探针实证，每次进 home 落地页即见）；② sidebar section 标题去多余 pin 图标追平 Codex 纯文字 divider（finder file:line 定位·内部一致性+金标双据）。四闸门绿：改动屏 before/after 对金标复比确关小（card4 单行/Pinned 纯文字）、qa/runs/2026-07-24-R92 存 before(home-*)/after(after-home-*)+探针输出、console panorama(home/rich × light/dark) **0**、vitest 856 绿 + build 绿。
  - **让路**：开轮工作区有并发 session 的 DiffParts/DiffView/DiffView.commit.test 脏文件（非本轮产生）——全程不 add/revert/rebase 碰；每次部署 stash→build 纯净→go build 嵌纯净 dist→pop 恢复，不把其在途工作带上 8809。（注：首次部署误嵌旧 dist·live 仍 13px·探针即时揪出·重构建 go 二进制修正——探针胜过假设「部署即生效」。）
  - **finder 诚实回报**：富 thread finder **无新 tsx-only in-scope 差距**（artifact 标题 500 = R91 ARTIFACT-CHIP 刻意·message-actions thumbs/share 无后端 out-of-scope·change-card/env-popover/worked-fold 全刻意 parity·成熟面防守严密）。避空转。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：① HOME-CHIP-STRIP-TINT（H-GAP2·Codex composer chip 条灰底·我方纯白·**部分反 R46 HOME-CHIP-FLAT 需裁**·tw.css）；② ENV-GOAL-SETTLED-GRID（T-GAP2·env 面板 settled-goal items-center 破栅格·SupervisionParts.tsx+tw.css）；③ 金标图分辨率瓶颈（819px 上采样·需真人 Computer Use 重截真像素）续 R89/90/91；④ ENV-OVERLAP-1440 float-vs-reflow 设计重裁（提用户·非 headless 单方改）续 R82/83/85/89/91；⑤ suggestion 图标 22→~20px（scheduled finder 判低置信·须真像素金标核）。
  - <2026-07-24 轮92>：比对 home×2 主题+探针实测折行、2 finder(富thread/scheduled-sidebar-settings)、关差距 **HOME-CARD-LABEL-12PX**(卡 label 13→12px·card4 折行 2→1 追平 Codex 2/2/2/1) + **SIDE-PINNED-PIN-DROP**(删 Pinned 标题 PushPin glyph·追平 Codex 纯文字 divider+内部一致)、派工 2(主驾驶串行·单文件簇 tw.css/Sidebar.tsx 互斥·落一个推一个)、push 89bb70ba+47c5a0c2、live=index-DVeODpYu.js、vitest 856 绿、console panorama 0、让路 DiffView(stash/build/go/pop 部署)、富thread finder 诚实回报无新差距

- 2026-07-24 轮93（headless·主驾驶自对标 home/rich/review/env 4 屏×2 主题+单独截 Review 分栏 + 3 并发 read-only finder〔diff / rich-env / home-composer-scheduled〕+ 单 worktree implementer 串行落 A+B·均 funnel tw.css）：**非失败轮 — 关闭 headline 差距 MODEL-PILL-POSITION（每个 composer 都可见）+ ENV-GOAL-SETTLED-GRID。**
  - ✅ **MODEL-PILL-POSITION（headline·composer finder 领跑+主驾驶截图坐实·file:line 精确·Impl {ComposerView.tsx,tw.css}）**〔push bc474848·live=index-CnPhjDIn.js〕：composer 底部 model/effort pill（`<ModelPicker>`）原在**左侧 leading 组**（`ComposerView.tsx:124` 紧挨 AddMenu/AccessPicker），右侧 trailing 仅 mic+send；Codex 金标（`codex-new-task-home`/`codex-crop-composer`）model pill 在**右侧尾部**紧挨 mic/send（`… 5.6 Sol Extra High ⌄ 🎤 ↑`），左侧只留 add+审批。移 `<ModelPicker>` 到 `cx-bar-trailing` 首位 + 窄屏规则 `tw.css:1587-1590` 从死类 `.cx-bar-leading .cx-model-wrap{flex-1}` 重 scope 到 `.cx-bar-trailing`（去撑满、保 min/max-w 尺寸约束）。**四证**：DOM 探针（leading=[add,access]·trailing 首项=ModelPicker pop-wrap 后跟 mic+send）+ 1440/390×light/dark **无横向溢出**(`scrollWidth==clientWidth`)+ before(`home-*` model 在左)/after(`after-home-*-{1440,390}` model 在右紧靠 send)截图直证 + console panorama **0**。vitest **855 绿** + build 绿。
  - ✅ **ENV-GOAL-SETTLED-GRID（次·rich-env finder seed·续 R92 开放·Impl tw.css）**〔同 push bc474848〕：Environment 面板 settled-goal 行 `.goal-settled-line` 共享 `tw.css:1006` 含 `items-center`，多行 goal 文本时状态 pill 竖直居中浮在中段而非对齐首行。追加独立覆盖 `.goal-settled-line { @apply items-start; }`（未动 1006 共享规则·单行 attention/background/env-row 仍保 items-center）。纯 CSS·代码正确性+不回退单行行确认。
  **本轮定性**：**非失败轮** — 关闭 1 个每 composer 可见的 headline layout 差距（model pill 左→右追平 Codex）+ 1 个 env 面板破栅格 polish。非 a11y/非 nit。四闸门绿：改动屏 before/after 对金标复比确关小（model 落位紧靠 send）、qa/runs/2026-07-24-R93 存 before(home/rich/diff/review)/after(after-home-*×4尺寸+after-rich)+DOM 探针输出、console panorama(home 1440/390 + rich × light/dark) **0**、vitest 855 绿 + build 绿。
  - **让路**：开轮工作区有并发 session 的 DiffParts/DiffView/DiffView.commit.test 脏文件（CommitPushMenu pending 态 WIP·非本轮产生）——全程不 add/revert/rebase 碰；部署 stash→ff origin/main→build 纯净→go 嵌→pop 恢复，不带上 8809。**注**：diff/review 屏本轮被此 WIP 阻塞（工具栏改动都在 DiffParts/DiffView）→ diff finder 亦独立判该成熟面无新 in-scope 差距，双向印证本轮避开 diff 正确。
  - **judgement 弃做**：GOAL-OUTCOME-PHASE-COLOR（rich-env finder 提·`.goal-outcome` phase 类 achieved/cancelled/stopped 在 SupervisionParts.tsx:144/200 挂了但 tw.css 无对应规则=死类·banner 977-979 有色而 chip 灰）——但 finder 自认 Codex 该 inline chip **本就中性灰**，加色会**背离 parity 而非追平**，判**反 parity 不做**（内部不一致是代码 hygiene 非 Codex 可见差距）。
  - **finder 诚实回报**：diff finder **无新 in-scope 差距**（成熟面·唯一可见 delta=删除行斜纹已 QA-0718 裁决移除）；rich-env finder **无强新差距**（面已防守·唯一新条目=上述反 parity 的死类）。避空转。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：① HOME-CHIP-STRIP-TINT（Codex composer chip 条灰底·我方纯白·**部分反 R46 HOME-CHIP-FLAT 需裁**·tw.css）；② 金标图分辨率瓶颈（819px 上采样·需真人 Computer Use 重截真像素）续 R89-92；③ ENV-OVERLAP-1440 float-vs-reflow 设计重裁（提用户·非 headless 单方改）续 R82-92；④ suggestion 图标 22→~20px（低置信·须真像素金标核）；⑤ diff/review 工具栏对标（Codex 有醒目 "Commit or push ⌄" 文字 pill + diff-search 放大镜 + split toggle·我方多为 icon）——待 DiffView/DiffParts 的 CommitPushMenu WIP 落定后下轮排（本轮让路阻塞）。

- 2026-07-24 轮94（headless·主驾驶自对标 home/scheduled/rich/composer/change-card/approval × light/dark @1440 全屏并排金标 + hero 图标像素核 + 3 并发 read-only finder〔home+composer / scheduled+sidebar / rich-thread-env〕+ 亲开 Cmd+K 命令面板逐组件裁剪比对 + live 探针三证·主驾驶单文件 tw.css 追加规则串行落）：**非失败轮 — 关闭 CMDK-KBD-SELVIS(命令面板 ⌘N 徽章在选中/hover 行隐形，每次 Cmd+K 光标行即见)。**
  - 本轮带新鲜眼睛重截 live 全屏并排金标(home/scheduled/rich/composer/change-card/approval)。主驾驶逐屏核实高度 parity:composer(model pill 右置 R93 + chips flat R46)/change card(dim 路径+bold 文件名+±右对齐·light/dark 双证)/environment 面板/approval 卡/scheduled(All-Active-Paused+Suggestions)均 parity 或预先记账。**hero 图标像素核**:裁 home 空态 CloudMark vs Codex → 同为七瓣云 scalloped + `>_`,渲染忠实,**parity**(排除 finder A 误报「有出入」)。3 finder 逐面取证。
  - ✅ **CMDK-KBD-SELVIS(主驾驶亲开 Cmd+K 逐组件裁剪比对 file:line 精确·探针三证·Impl tw.css 追加规则)**〔push 8ed0aa48·live=index-Dsg3Leg9.js〕:命令面板 ⌘N 快捷键徽章 `.cmdk-kbd`(tw.css:741)填充 `bg-panel-2`,而选中/hover 行 `.cmdk-item.sel`/`:hover` **也**是 `bg-panel-2`(R77 CMDK-SEL-NEUTRAL) → 光标行上徽章与行同色**溶解隐形、读作纯文字**;Codex 金标(`codex-crop-command-palette.jpg`)的 ⌘1 pill 始终比光标行深一档、清晰可辨。追加规则 `.cmdk-item.sel .cmdk-kbd, .cmdk-item:hover .cmdk-kbd { @apply bg-line; }`——只在行背景变 panel-2 时把徽章降一档到 line、白/默认行保 R78 soft-grey pill 不动(R78 line 741 未改·非反转·兑现 R78「可见 pill」本意)。**探针三证**(index-Dsg3Leg9):light 选中行 rgb(238,238,238)/徽章 rgb(220,220,220)、dark 选中行 rgb(31,31,36)/徽章 rgb(58,58,66)——两模式徽章均与行背景明确区分;before(`crop-ours-kbd.png` ⌘1 纯文字隐形)/after(`crop-after-kbd.png` ⌘1 可见灰 pill)裁剪直证 + Codex crop(`crop-codex-kbd.png`)同款 pill。vitest **856 绿**(CommandPalette.test 18 绿) + build 绿。
  **本轮定性**:**非失败轮** — 关闭 1 个用户每次开 Cmd+K 光标行即见、in-scope(命令面板是我方有后端功能)、非 a11y(修**可见性隐形**·非加焦点环)、非刻意✂(R78/R77 均未处理二者同为 panel-2 的相互作用·是 refinement 非反转)、非 nit、非让路区的可见 UI/UX 差距。四闸门绿:改动屏 before/after 对金标复比确关小(⌘N pill 恢复可见)、qa/runs/2026-07-24-R94 存全屏×5屏×2主题 + cmdk before/after + hero 像素核 + 探针输出、console panorama(home/rich/cmdk × light/dark) **0**、vitest 856 绿 + build 绿。
  - **让路**:开轮工作区有并发 session 的 DiffParts/DiffView/DiffView.commit.test 脏文件(CommitPushMenu WIP·非本轮产生)——全程不 add/revert/rebase 碰;部署 stash→rebase origin/main(并发推进到 dc2c58d2)→build 纯净→go 嵌→pop 恢复,不带上 8809。
  - **3 finder 诚实回报**:A(home+composer)仅 1 条 LOW(model 菜单 Advanced 整页 push vs Codex inline·`5a1a549a` 证 drill-in 是刻意 Codex-parity 决策·改反转需裁决)、B(scheduled+sidebar)**无**(全刻意 SCH-ICON/SC-10 彩色 glyph、per-workspace 分组、数据驱动 unread/next-run、OOS Plugins/Sites/PR)、C(rich-thread-env·39 tool 穷举)change/artifact/message-actions/env/goal 全 parity 或预先记账·唯一 borderline(access pill 色)判我方更准不做。避空转。主驾驶亲查命令面板(finder 未覆盖的有金标交互面)才捞出本轮唯一可做差距。
  - **仍开放 ☐(下轮 fresh-eyes 排)**:① HOME-CHIP-STRIP-TINT(Codex composer chip 条两段式灰底 vs 我方单白卡·**部分反 R46 HOME-CHIP-FLAT 需裁**·f4eaffa4 证 R46 刻意去 hairline/border·819px 下 tint 真伪不可判)续 R91-93;② 金标图分辨率瓶颈(819px 上采样·需真人 Computer Use 重截真像素·多轮 parity 墙的根因)续 R89-93;③ ENV-OVERLAP-1440 float-vs-reflow 设计重裁(提用户·非 headless 单方改)续 R82-93;④ model 菜单 Advanced inline-vs-drill-in(需高清金标裁决·`5a1a549a` drill-in 刻意 vs Codex ^ caret 疑似 inline·冲突)新;⑤ diff/review 工具栏对标·待 DiffView CommitPushMenu WIP 落定后下轮排(本轮让路阻塞)续 R93。
  - <2026-07-24 轮94>:比对 home/scheduled/rich/composer/change-card/approval 6 屏×2主题 + hero 像素核 + 3 finder + 亲开 Cmd+K 逐组件裁剪、关差距 **CMDK-KBD-SELVIS**(命令面板 ⌘N 徽章选中/hover 行 panel-2→line 保可见·追平 Codex 始终可辨 pill·探针三证)、派工 0(主驾驶单文件 tw.css 追加规则串行落·finder 全 read-only)、push 8ed0aa48、live=index-Dsg3Leg9.js、vitest 856 绿、console panorama 0、让路 DiffView(stash/rebase/build/go/pop 部署)、3 finder 诚实回报仅 1 LOW 需裁决·主驾驶亲查命令面板捞出唯一可做差距

- 2026-07-24 轮95（headless·主驾驶自对标 home/scheduled/rich×light/dark 全屏并排金标 + **高分辨率组件裁剪 `codex-crop-composer.jpg` 逐像素采样** + 3 并发 read-only finder〔settings / mobile390 / approval〕+ 1 worktree implementer〔SettingsAppearance.tsx〕+ 主驾驶单文件 tw.css）：**非失败轮 — 关闭 2 个用户可见、有后端、非 a11y/非 nit 的 UI/UX 差距：① HOME-CHIP-STRIP-TINT（每个 composer 都可见的 headline，多轮"真伪不可判"墙被高清 crop 像素采样击穿）② APPEARANCE-ROW-INLINE（Settings 面板控件行内右对齐）。**
  - ✅ **HOME-CHIP-STRIP-TINT（headline·主驾驶高清 crop 像素采样坐实·续 R91-94 长期 defer·Impl tw.css）**〔push 0b1d00f7·live=index-m9gykY_t.js〕：Codex composer 是**双色**——env/context chip 条坐在浅灰 shelf（**像素采样 `codex-crop-composer.jpg`：chip 条背景 (244,244,244)#F4F4F4、输入区 (255,255,255) 纯白、纵向色带 y18-90 灰/y96+ 白**）上，白输入卡在下；我方是**单张纯白扁平卡**。此差距 R91-94 因"819px 全屏金标上采样·tint 真伪不可判"连续 defer——本轮改用**高分辨率组件裁剪金标**逐像素采样，真伪一举坐实。新增 `--cx-shelf` token（light #f5f5f5 / dark #1b1b1f·**刻意保持比 --panel-2 更浅**，使 chip 的 panel-2 hover 仍在 shelf 上可辨）+ 给 `.cx-env-strip` 上背景 + 顶圆角 `calc(radius-composer - 1px)` 贴合卡内、底部保方角与白输入以**色调缝**相接（无 hairline·**兑现而非反 R46 HOME-CHIP-FLAT**——f4eaffa4 去的是单个 chip 的 pill 边框/条底 hairline，与整条 shelf 的 tint 正交）。**live 探针三证**（index-m9gykY_t）：light strip rgb(245,245,245) vs card rgb(255,255,255)、dark strip rgb(27,27,31) vs card rgb(23,23,26)、顶圆角 21px，两模式均 distinct；before(`crop-composer-ours.png` 全白)/after(`after-crop-composer-{light,dark}.png` 灰 shelf+白卡双色) + live CSS 直证 `cx-env-strip{background-color:var(--cx-shelf)...}`。vitest 857 绿（唯一红 SessionView.chrome TH-14 = 并发 session 4e8d8a01 遗留既有红·干净 HEAD 同失败·已被并发 44a0a95c 修·非本改致）+ build 绿。
  - ✅ **APPEARANCE-ROW-INLINE（Finder A HIGH·Impl SettingsAppearance.tsx·worktree 隔离）**〔push c5a8874e〕：Settings→Appearance 面板 toggle/字号步进器/diff 分段器原**掉到标签下方**（行高约 Codex 2 倍、大片死空白、控件与标签解耦），且移动端 switch 已右上定位、桌面反更糟=遗漏（`c65cecb8` SETTINGS-ROW-DESC-STACK 只处理副标题堆叠·未触桌面控件行内）。把三类控件从 `.rs-row-head` 兄弟节点**移入 row-head 作第二 flex 子**（row-head 已 `flex items-center justify-between`·**无需动 tw.css**·避与主驾驶 tw.css 白名单冲突），控件天然行内右对齐+垂直居中·行高追平 Codex。FontRow 步进器加 `w-auto` 覆盖 `.rs-slider` 的 w-full·ToggleRow 保 `max-[500px]:absolute right-2.5 top-2.5` 移动端不回退。Impl 自证：vitest 3 绿（含 mobile）·@390 `scrollWidth==clientWidth==390` 无溢出·light/dark×1440/390 4 张截图·rs-row DOM 探针 inHead+sameRow。
  **本轮定性**：**非失败轮** — 关闭 2 个可见差距（①每 composer 可见的双色 shelf ②Settings 行内控件密度）。四闸门绿：改动屏 before/after 对金标复比确关小（chip 条现灰 shelf 双色、appearance 行内紧凑）、qa/runs/2026-07-24-R95 存全屏×3屏×2主题 + composer before/after crop + 像素采样输出 + finder 证据、console panorama(home/rich × light/dark) **0/0**、vitest 857 绿(既有红非本改) + build 绿。
  - **并发竞态处置**：origin/main 本轮被并发 session 连推（44a0a95c/92354059/9e49a204 + Impl X c5a8874e），主驾驶 tw.css commit rebase 过并发干净落地；**8809 被并发用旧 dist 覆盖两次**（live CSS 缺 shelf 背景·探针即时揪出）→ ff 到最新 main（含双改）重 npm build+go build 重嵌纯净 dist 重部署，直至 live CSS `grep cx-env-strip` 见 background ✓——**探针胜过"部署即生效"假设**（续 R92 教训）。
  - **让路**：开轮工作区有并发 session 的 DiffParts/DiffView/DiffView.commit.test 脏文件（CommitPushMenu WIP·非本轮产生）——全程不 add/revert/rebase 碰；每次部署 stash→ff→build 纯净→go 嵌→pop 恢复，不带上 8809。
  - **3 finder 诚实回报 + 下轮 seed**：**A(settings)** — Settings 整体高 parity·2 条：①APPEARANCE-ROW-INLINE(本轮已做)②CONFIG-CARD-MIXED(MED·Configuration 面板 rs-kv 灰 slab vs rs-row 白描边卡混用·`SettingsConfiguration.tsx:39-52`+tw.css)。**B(mobile390)** — 结构扎实零文档级溢出·2 条：①BRANCH-CHIP-RTL-TRUNC(MED·390 下 "No branch" 被 RTL 左截成"…branch"歧义·`ComposerParts.tsx:370`[direction:rtl]+`tw.css:1600` leading 组无 min-w·非刻意 refactor 4bc32091)②HOME-HEADLINE-REPO-TRUNC(LOW-MED·390 长 repo 截断后 `?` 浮空+虚线拖尾·`tw.css:1580`+`Home.tsx:172`)。**C(approval)** — approval/env/空载错态整体到位·3 条：①ATTN-NOTICE-CLICKABLE(高·父会话 appr/answer/recovery notice 是死文本无跳转·`SupervisionPanel.tsx:101-106`+`SupervisionParts.tsx:335-344`·与同段可点 child-answer 自相矛盾)②APPROVAL-DIFF-PREVIEW(中·edit/write approval 卡只显路径不显 diff·`ApprovalCard.tsx:73-98`+`approvalPresentation.ts:49-56`·复用 DiffView)③EVENTS-FAIL-EMPTY(中低·events 拉取非-notfound 失败静默退化成"No messages yet"空态无错态/重试·`useSessionDataController.ts:128-136`+`TimelineFeature.tsx:1449-1452`)。
  - **仍开放 ☐（下轮 fresh-eyes 排）**：① **ATTN-NOTICE-CLICKABLE**（Finder C 高·父会话 approval/answer notice 加滚动锚点跳转·功能入口缺失）；② **CONFIG-CARD-MIXED**（Finder A MED·Configuration 面板卡片语言统一）；③ **BRANCH-CHIP-RTL-TRUNC**（Finder B MED·390 "No branch" 左截断歧义·须 tw.css，与后续 composer 改协调）；④ **APPROVAL-DIFF-PREVIEW**（Finder C 中·edit approval 卡内联 diff）；⑤ 金标图分辨率瓶颈（**注：本轮证明高清组件裁剪 crop 可击穿"真伪不可判"墙**·仍需真人 Computer Use 补更多 crop 金标）续 R89-94；⑥ ENV-OVERLAP-1440 float-vs-reflow 设计重裁（提用户）续 R82-94；⑦ diff/review 工具栏对标·待 DiffView CommitPushMenu WIP 落定续 R93。
  - <2026-07-24 轮95>：比对 home/scheduled/rich 3 屏×2主题 + **高清 composer crop 像素采样击穿多轮 tint 真伪墙** + 3 finder(settings/mobile390/approval)、关差距 **HOME-CHIP-STRIP-TINT**(composer 加灰 chip shelf 双色·追平 Codex #F4F4F4 两段式·探针三证) + **APPEARANCE-ROW-INLINE**(Settings 控件行内右对齐追平 Codex row grammar)、派工 1(ImplX SettingsAppearance.tsx worktree·主驾驶 tw.css 并行·白名单互斥·落一个推一个)、push 0b1d00f7+c5a8874e、live=index-m9gykY_t.js、vitest 857 绿、console panorama 0/0、让路 DiffView(stash/ff/build/go/pop 部署·并发覆盖 8809 两次探针揪出重部署)、3 finder 各回报 7 条 seed 供下轮
