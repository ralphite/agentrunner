> **归档（2026-07-10）**：已完成；活定义已并入 SPEC/QA/GAPS/LOG 与
> `design-qa.md`，QA-36 PASS。

# INC-29 Web UI UX Round 3（结构化详情/标题/状态语义）

## 动机与 journey 锚

UJ-24 的 Codex 式主路径已由 INC-23/QA-34 跑通，但 INC-23 执行记录明确
...
三步全部完成。QA-36 黑盒反打发现并修复 agent 重复计数、普通 input
false-attention、recovery stale status；23 frontend tests 与根 check 全绿。
