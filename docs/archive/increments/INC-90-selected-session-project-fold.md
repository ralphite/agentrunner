...
- 实现提交：`de6b3966 INC-90.2: let selected projects collapse`。
- A 闸：`npm test -- Sidebar.nav viewModels viewModels.nav` 92/92；完整
  frontend tests 659/659；production build 与 `./scripts/check.sh` 通过。
- B 闸：QA-81 在共享 store、真实 `:8809` 上通过，部署版本
  `de6b3966-214638`；选中 session 所在 `mt-test` 可立即折叠，4.5s refresh
  与 reload 后保持折叠，hash/central thread 不变；重新展开恢复
  `current` 行，console 0 error/warn。
- 证据：`qa/runs/2026-07-21-QA81-selected-session-project-fold/`；未清理
  session/workspace/journal，测试结束后将 `mt-test` 折叠偏好恢复为展开。
