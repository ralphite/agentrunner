...
| Web UI Markdown 渲染增强（react-markdown + remark-gfm 表格/删除线/任务列表 + 按需 highlight.js 语法高亮（core+19 语言，`common` tree-shaken）+ 每代码块 line-wrap 开关；token 配色映射主题变量双主题可用；**禁 raw HTML**：react-markdown 默认转义、无 rehype-raw、无注入面；对外 `<Markdown text>` prop 不变，Timeline 两调用点零改动） | ⚠️ | UJ-24 | INC-51（HANDA #20）· A 闸绿：frontend vitest `Markdown.test.tsx`（表格/高亮/line-wrap/raw-HTML 转义 4 断言）+ `tsc -b` + `vite build`；B 闸真浏览器 DOM 断言待验（收口转 ✅）；mermaid 懒加载记余项 |
