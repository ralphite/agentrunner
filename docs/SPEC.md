...
|---|---|---|---|
| 续聊（答完待命；close = 标记；**任何显式重开——send 或 compact/clear 复活——清标记，状态回 waiting:input，不残留 closed**，INC-74） | ✅ | UJ-01/03/09 | QA-01 · C1 · 孪生（见 e2e）· TestReopenAfterCloseClearsMark |
...
| Web UI progressive-disclosure composer | ✅ | UJ-22/24 | QA-69(真浏览器:Add/Plugins/Advanced 三组根动作)· INC-19/23/38/40/65 · 默认输入/附件/access/model/send；New session 上缘为独立 Project、Local/New worktree、Local environment、Branch，Project/Branch 可搜索，worktree 尊重 selected ref；Goal/Repeating/Best-of-N、persona 与 YAML 收入 Advanced |
