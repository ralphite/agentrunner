...
| Web UI 交互语义（session row/button、dialog/menu/listbox、Escape/方向键/focus return、popover viewport containment、移动端 sidebar scrim；command palette 的九个 `⌘1..⌘9` 真实快捷 session 后先露 Commands 再露 attention overflow，宽度 560px/顶边 15vh 且 mobile 回到 12px viewport inset；keyboard context menu 的 Escape 回 session/project opener；mobile Changes 独占 overlay 时移除底层 sidebar trigger；menu group 只在有动作时出现，View 只显示可切换目标；Environment 是不改动主栏 geometry 的右上浮动卡，不画空 Changes/disabled Commit，子 agent 无 Commit；Settings desktop 只有 Done、mobile 只有 Back，dismissal 回到原 trigger/input；打开 Settings 或 mobile sidebar 同步收起另一 overlay/Environment；审批只在宽屏自动展开 Supervision；running/ready/attention/failed/terminal 统一状态色，具体 provider failure 优先于泛化 terminal notice，model-not-found 明示换模型后 Retry；activity 数量/child link 在窄屏可读，child header 优先 inspect agent spec） | ✅ | UJ-24 | INC-23/29/40/41/60/65/68/88/95/97/98.2/98.2h · CommandPalette.test.tsx/SessionView.chrome.test.tsx/Timeline.thread.test.tsx/SupervisionPanel.test.tsx/Settings.test.tsx/Sidebar.nav.test.tsx/App.mobile.test.ts Environment CSS contract/viewModels.test.ts · QA-34/36/42/43/61/68/80/86/87/88 DOM/light/dark |
