...
| durable team task/DAG/lease + workspace assignment（生产默认隔离 worktree，显式 shared；revive 复用；isolated 快照语义入工具契约+子任务注入,spawn_agent.replaces 显式回收前任,INC-30） | ✅ | UJ-16/18/23 | INC-11.6 · TestIsolatedTeamWorkspaceSurvivesRevive/TestTeamTaskDependencyPlan · INC-30 · TestIsolatedChildTaskCarriesSnapshotNotice/TestSpawnReplacesCancelsPredecessor · QA-INC30（真 Gemini 三场景） |
...
| Web UI 产品面：Codex 式 New task/Scheduled/Pinned/Projects→task、单一 thread、Changes、deep link、responsive sidebar；首次 session readiness 不投影假空态/raw id；task hover pin/archive+project/branch/status；自动标题去同质指令前缀 | ✅ | UJ-24 | INC-19/23/29/38/40 · conciseTitle/frontend view-model/timeline tests · QA-27/34/36/41/42 |
| journal-backed session metadata（`sessions list --json` 输出 workspace/title/kind/schedule，Web UI cache 非真相源；title 优先 `RawTitle` 投影、回退首行） | ✅ | UJ-24 | INC-19/23/52 · TestCLIResumeAfterCrash · TestCLISessionsJSONProjectsDriverMetadata · TestCLISessionsJSONSurfacesAutoTitle · TestMetaStoreMerge* |
...
| Web UI progressive-disclosure composer | ✅ | UJ-22/24 | INC-19/23/38/40 · 默认输入/附件/access/model/send；New task 上缘为独立 Project、Local/New worktree、Local environment、Branch，Project/Branch 可搜索，worktree 尊重 selected ref；Goal/Repeating/Best-of-N 与 persona 收入产品化 Task options；YAML 收入 Advanced |
| Web UI Codex 式任务收尾（最终 answer 前 journal-ts Worked duration；Copy；checkpoint/worktree-backed Continue in new task；真实 diff Changes 摘要→Review；Changes `Working tree / Last turn` durable 范围，缺 barrier truthful unavailable） | ✅ | UJ-24 | INC-38/57 · TestShadowRepoDiffAgainstSnapshot · TestCLIDiffLastTurnJSON · TestHandleDiffLastTurn · completedTurnDurations/summarizeChanges tests · QA-41/54 |
