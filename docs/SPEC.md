...
| 子 agent 实时进度镜像（成员事件带 session 标签入树根 hub;`ar attach <child-sid>` live 过滤;webui 子会话 SSE;CLI 前台锚定折叠）/ 子审批根宿主路由与 crash 重挂接;**child 卡审批可发现**（inspect 树递归 in-flight child 携 waiting+answer_with,webui approval stack/Attention 持久浮出,INC-81 修 G39） | ✅ | UJ-18/23 | INC-12.6 · TestDaemonAttachChildFiltersLive/TestCrashResumeReattachesApprovalWaitingChild · QA-20(G10 关闭) · INC-81 · TestBuildInspectTreeSurfacesInFlightChildApproval（闸门 B 复验挂 G39） |
