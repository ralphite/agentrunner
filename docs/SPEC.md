...
| ask_user（wait-class 提问：park WAITING_INPUT，应答走 inbox→配对 tool result，同 session 续跑；一批限一问、interrupt/crash/headless 全覆盖；**结构化形态**：questions[]≤4 问×2–4 选项/multi_select/allow_free_text，park detail 携带结构，`AskResolved.Answers` typed，`ar answer <sid> <q>:<n>`/`--skip`（cancelled 非错）+ daemon `answer` 通道，free-text send 兼容保留，crash 重放配对 pending answer） | ✅ | UJ-06/24 | INC-5 · TestAskUser* · QA-13 · INC-47（HANDA #7/CC #10 步1）· TestValidateAskQuestions/TestValidateAskAnswers/TestAnswerCommandPairsAcrossRestart/TestParseAnswerSpecs · 真验 2026-07-11（真 Gemini 双问 typed 复述+--skip 自主决策，qa/runs/2026-07-11-INC47）；webui 表单卡=步2 |
...
| mode 运行中切换（default↔acceptEdits 用户命令；`ar mode`/webui `/mode`+pill live 化；plan 退出仍归 exit_plan_mode 审批、bypass 仅启动时；非法目标显式 rejected receipt；子 agent frozen-at-spawn 不变） | ✅ | UJ-06 | INC-42（G29 关闭）· TestModeControlSwitchesToAcceptEdits/SwitchBack/RejectsInvalid/IdempotentReplay/TestReplayProjectsModeChanged · QA-44（真机六红线+webui playwright 真用户流） |
