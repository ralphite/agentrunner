...
| loop mode（interval fixed-rate/cron/self_paced、durable absolute tick、overlap skip/coalesce；每个 retry attempt parent-journaled；全 child error=`child_failed`；**session 内形态见 A 表(INC-74),driver 形态维持至 E1 收敛完成**；**merged-stream 接线 opt-in（INC-80.2a）**：`drive/submit --series` 让 goal/interval/cron 走 RunSeries 会话形态（SessionStarted+SeriesStarted 头,零 DriverStarted）,resume/boot-sweep 按 journal 头双分派,stranded sweep 排除 series 会话,`ar resume` 拒绝并指路;self_paced/parallel/retry 仍 legacy） | ✅ | UJ-14 | S6+INC-66 · TestDriverIntervalOverlapPolicies/TestDriverChildFailRetryRecovers/TestDriverChildFailSurface · INC-80.2a · TestDriveSeriesGoalJournalsMergedStream/TestDriveSeriesRefusesUnsupportedSpec/TestScanDriveSessionsIncludesSeriesForm/TestCLIResumeRefusesSeriesSession · INC-80.2b（retry/self_paced 收编 series：TestSeriesChildFailRetryRecovers/TestSeriesChildFailRetryExhausts/TestSeriesSelfPaced/TestSeriesSelfPacedNoIntentFinish/TestSeriesSelfPacedFinishDenied） |
