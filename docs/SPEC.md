...
| Web UI progressive-disclosure composer | ✅ | UJ-22/24 | QA-69/86/88（真浏览器：Add/Advanced 两组；root 精确 Files and folders/Goal/Plan mode/Automation；Goal 单 composer + options chip、Send 前零 session；Plan Add 可逆并恢复 prior access；Ask/Auto/Plan→Full access 先列 files/terminal/internet 风险并确认，Cancel/Escape 保留 posture/draft/focus；Automation 子页含 Loop/Best-of-N/background/Agent→YAML；desktop 长 draft `min(320px,38dvh)`、窄屏 `180px` 且 controls 可见）· INC-19/23/38/40/65/88/95/98.2e/98.2f/98.3h · Composer.addMenu/Modals.mobile/effort.test.tsx · 默认输入/附件/access/model/send；New session 未选 Project 时上缘只显示 project picker，选定后才显示 Local/New worktree 与 Branch，Project/Branch 可搜索，worktree 尊重 selected ref；占位面(Plugins 五件套/Environment chip/Settings Not-wired 字段)已移除(PLAN 5.1,原则:UI 只承诺已接线的能力) |
