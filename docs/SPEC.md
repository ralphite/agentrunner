...
| mode 运行中切换（default↔acceptEdits 用户命令；`ar mode`/webui `/mode` + pill 点击切换（选择器：Ask↔Auto-accept 可点，Full/Plan disabled 带原因）；plan 退出仍归 exit_plan_mode 审批、bypass 仅启动时；非法目标显式 rejected receipt；子 agent frozen-at-spawn 不变） | ✅ | UJ-06 | INC-42（G29 关闭）+ INC-54（pill 点击入口）· TestModeControlSwitchesToAcceptEdits/SwitchBack/RejectsInvalid/IdempotentReplay/TestReplayProjectsModeChanged + runtimeModeTarget 单测 · QA-44 + QA-51（真机 pill 点击切换六红线+webui playwright 真用户流） |
