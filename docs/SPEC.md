...
| Web UI 产品面：Codex 式 New task/Scheduled/Pinned/Projects→task、单一 thread、Changes、deep link、responsive sidebar；首次 session readiness 不投影假空态/raw id；task hover pin/archive+project/branch/status；自动标题去同质指令前缀 | ✅ | UJ-24 | INC-19/23/29/38/40/41 · conciseTitle/frontend view-model/timeline tests · QA-27/34/36/41/42/43 |
...
| Web UI 交互语义（task/button、dialog/menu/listbox、Escape/方向键/focus return、popover viewport containment、移动端 sidebar scrim；打开 Settings 同步收起 mobile sidebar/scrim；审批只在宽屏自动展开 Supervision；running/ready/attention/failed/terminal 统一状态色） | ✅ | UJ-24 | INC-23/29/40/41 · frontend build · QA-34/36/42/43 DOM/light/dark |
