...
| Web UI 产品面：Codex 式 New task/Scheduled/Pinned/Projects→task、单一 thread、Changes、deep link、responsive sidebar；首次 session readiness 不投影假空态/raw id；共享大历史首 40 条立即 ready、后台 80/页补齐且 refresh 不重入；task hover pin/archive+project/branch/status；自动标题去同质指令前缀 | ✅ | UJ-24 | INC-19/23/29/38/40/41/60 · conciseTitle/frontend view-model/timeline/store session tests · QA-27/34/36/41/42/43/61 |
...
| Web UI Codex 式任务收尾（最终 answer 前 journal-ts Worked duration；Copy；checkpoint/worktree-backed Continue in new task；真实 diff Changes 摘要→Review；Changes `Working tree / Last turn` durable 范围，缺 barrier truthful unavailable；generated/unbounded untracked output 有计数地隐藏，超大 diff 默认逐文件折叠且首 paint 前决定 disclosure） | ✅ | UJ-24 | INC-38/57/60 · TestShadowRepoDiffAgainstSnapshot · TestCLIDiffLastTurnJSON · TestHandleDiffLastTurn/TestHandleDiffNestedWorkspace · completedTurnDurations/summarizeChanges/shouldExpandDiffByDefault tests · QA-41/60/61 |
...
| Web UI 交互语义（task/button、dialog/menu/listbox、Escape/方向键/focus return、popover viewport containment、移动端 sidebar scrim；Settings 与 Command palette dismissal 回到原 trigger/input；打开 Settings 同步收起 mobile sidebar/scrim；审批只在宽屏自动展开 Supervision；running/ready/attention/failed/terminal 统一状态色） | ✅ | UJ-24 | INC-23/29/40/41/60 · frontend build · QA-34/36/42/43/61 DOM/light/dark |
...
| curl 一行安装分发（install.sh 多平台探测/私有 repo token 路径/sha256 校验/版本化解包+symlink 切换；scripts/package-release.sh 单机交叉编译 4 target；release workflow tag→构建→smoke→发布稳定命名资产；Windows 裁掉随其形态立项） | 🟡 | UJ-25 | INC-63 · gate A 孪生 scripts/test-install.sh（5 场景，check.sh 常跑）· gate B=QA-63：workflow 构建+smoke 段真跑，"tag 发布→公网 curl 安装"全程留待首个真实 release，完成后转 ✅ |
