...
| Web UI 产品面：New session/Scheduled runs/Pinned/Projects→sessions、单一 thread、Changes、deep link、responsive sidebar；首次 session readiness 不投影假空态/raw id；共享大历史首 40 条立即 ready、后台 80/页补齐且 refresh 不重入；session hover pin/archive+project/branch/status；自动标题去同质指令前缀；自动 workspace 各自成组（默认名 `Scratch · 创建时间`，组键=真实路径，不聚合混合；INC-78 撤销单一 Scratch 聚合）；project hover/focus 摘要+project-scoped New chat 快捷入口（seed Home composer，不提前建 session），标题行 ⋯/右键同源六项菜单（pin/Finder/permanent worktree/rename/archive/safe Remove），project pin/remove overlay 可恢复；button pressed state 尺寸稳定；desktop sidebar 220–480px pointer/keyboard resize，Pinned/Projects section 独立收展且偏好跨刷新，mobile 保持 fixed drawer | ✅ | UJ-24 | INC-19/23/29/38/40/41/60/65/78/87/89 · TestMetaStoreProjectOverlayRoundTrip/TestProjectUpdatePinnedRemoved · Sidebar.nav.test.tsx project controls/new-chat/section fold/resize · Composer.projects.test.tsx project seed · buttonPress.test.js · conciseTitle/frontend view-model/timeline/store session tests · viewModels buildSidebarModel scratch-split 断言 · QA-27/34/36/41/42/43/61/78/79 + INC-78 闸B(run 29672007663:分组/触屏改名/刷新持久) |
...
| Web UI progressive-disclosure composer | ✅ | UJ-22/24 | QA-69(真浏览器:Add/Advanced 两组根动作)· INC-19/23/38/40/65 · 默认输入/附件/access/model/send；New session 上缘为独立 Project、Local/New worktree、Branch，Project/Branch 可搜索，worktree 尊重 selected ref；Goal/Repeating/Best-of-N、persona 与 YAML 收入 Advanced；占位面(Plugins 五件套/Environment chip/Settings Not-wired 字段)已移除(PLAN 5.1,原则:UI 只承诺已接线的能力) |
