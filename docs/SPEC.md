...
| 子 agent 实时进度镜像（成员事件带 session 标签入树根 hub;`ar attach <child-sid>` live 过滤;webui 子会话 SSE;CLI 前台锚定折叠）/ 子审批根宿主路由与 crash 重挂接;**child 卡审批可发现**（inspect 树递归 in-flight child 携 waiting+answer_with,webui approval stack/Attention 持久浮出；Subagents typed wait 显示 Needs approval；approval 卡使用 delegation 的真实 child worktree,INC-81/98.3g 修 G39） | ✅ | UJ-18/23 | INC-12.6 · TestDaemonAttachChildFiltersLive/TestCrashResumeReattachesApprovalWaitingChild · QA-20(G10 关闭) · INC-81 · TestBuildInspectTreeSurfacesInFlightChildApproval · ApprovalCard/Subagents/SessionView INC-98.3g tests · QA-88 TH-09（闸门 B，G39 关闭） |
...
| fork（单创世、in-flight 一律取消合成收尾（PLAN 5.9 裁 policy 向量）、随行库复制、独立 worktree） | ✅ | UJ-15 | S7 收口 review 修复 + fork-of-fork 测试 · TestCutOfForkKeepsSingleGenesis/TestCutAppliesCancelAtFork/TestCLIForkRewindsAndContinues（G30 还锚 audit-0717 C1） |
...
| Web UI model/access picker（provider model、Light/Medium/High/Extra High、custom model id/exact thinking budget、Full/Ask/auto-accept/Plan；菜单只显示会改变 spec/runtime 的 action） | 🟡 | UJ-24 | INC-98.2d · Composer.effort.test.tsx（Model/Effort/Advanced 可达且无无效 Speed root）· QA-88；provider service tier `Fast` 与 model-specific `Ultra` 缺 G45 |
...
| journal-backed session metadata（`sessions list --json` 输出 workspace/title/kind/schedule/`updated_at`，Web UI 映射 `updatedAt` 且 cache 非真相源；title 优先 `RawTitle` 投影、回退首行；可选 `--limit/--offset` 先按 journal mtime 排候选再仅 fold 请求页，无 flag 仍全量兼容） | ✅ | UJ-24 | INC-19/23/52/60/94 · TestCLIResumeAfterCrash · TestCLISessionsJSONProjectsDriverMetadata · TestCLISessionsJSONSurfacesAutoTitle · TestCLISessionsPaginationNewestFirst · TestHandleSessionsMapsJournalUpdatedAt · TestMetaStoreMerge* · QA-61/85 |
...
| Web UI 交互语义（session row/button、dialog/menu/listbox、Escape/方向键/focus return、popover viewport containment、移动端 sidebar scrim；command palette 的九个 `⌘1..⌘9` 真实快捷 session 后先露 Commands 再露 attention overflow，宽度 560px/顶边 15vh 且 mobile 回到 12px viewport inset；keyboard context menu 的 Escape 回 session/project opener；mobile Changes 独占 overlay 时移除底层 sidebar trigger；menu group 只在有动作时出现，View 只显示可切换目标；New session 升级 Full access 必须经结构化风险确认且取消不改 draft/posture；Environment 是不改动主栏 geometry 的右上浮动卡，不画空 Changes/disabled Commit，子 agent 无 Commit；Settings desktop 只有 Done、mobile 只有 Back，dismissal 回到原 trigger/input；打开 Settings 或 mobile sidebar 同步收起另一 overlay/Environment；审批只在宽屏自动展开 Supervision；running/ready/attention/failed/terminal 统一状态色，具体 provider failure 优先于泛化 terminal notice，model-not-found 明示换模型后 Retry；精确 durable `interrupted` 是用户 Stop，只显示 Stopped+Retry 并保留普通 composer，不叠 Resume recovery，含 crash 的复合异常仍走 recovery；queue receipt 后只显示唯一 durable/可撤回 card，steer 在 journal receipt 前保留 optimistic bubble，Cmd+Enter 单次反选投递模式；tool detail 的短单行 command 只在 summary 出现一次，长/多行仍给全文；长输出限高内滚动且 Copy 取完整 projection，非成功 Copy 保留 `Exit N/Cancelled/Failed`；activity 数量/child link 在窄屏可读，child header 优先 inspect agent spec） | ✅ | UJ-24 | INC-23/29/40/41/60/65/68/88/95/97/98.2/98.2h/98.3b/98.3c/98.3d/98.3h/98.3j · CommandPalette.test.tsx/Composer.addMenu.test.tsx/Modals.mobile.test.tsx/SessionView.chrome.test.tsx/Timeline.thread.test.tsx/Timeline.toolcards.test.tsx/SupervisionPanel.test.tsx/Settings.test.tsx/Sidebar.nav.test.tsx/App.mobile.test.ts Environment CSS contract/viewModels.test.ts · QA-34/36/42/43/61/68/80/86/87/88 DOM/light/dark |
