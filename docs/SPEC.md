...
| Web UI 产品面：New session/Scheduled runs/Pinned/Projects→sessions、单一 thread、Changes、deep link、responsive sidebar；首次 session readiness 不投影假空态/raw id；共享大历史首 40 条立即 ready、后台 80/页补齐且 refresh 不重入；sidebar session/workspace-less/Pinned 按 journal last update newest-first，project 按成员最大 update newest-first（project pin 优先分区，分区内仍按 update）；session row resting state 显示 managed-worktree/running/unread/attention 事实 icon，hover/focus 显示 Pin/Archive quick actions 且 running spinner 保留；row 无 `…`，完整菜单由右键/键盘 context menu 与 session title `…` 承担，hover/focus 与 current 背景均覆盖整行和 icons；普通菜单不重复暴露 raw id/deep-link；自动标题去同质指令前缀；自动 workspace 各自成组（默认名 `Scratch · 创建时间`，组键=真实路径，不聚合混合；INC-78 撤销单一 Scratch 聚合），project display label 可重名且无 path subtitle，完整路径只在 tooltip/hover preview；project hover/focus 摘要+project-scoped New chat 快捷入口（seed Home composer，不提前建 session），完整 project row 背景覆盖 heading 与 trailing icons，标题行 ⋯/右键同源六项菜单（pin/Finder/permanent worktree/rename/archive/safe Remove），project pin/remove overlay 可恢复；project group fold 优先于 active session，active group 在 section cap 外只保证 heading 可见；connected daemon 是静态 status，offline 才显示 Restart；button pressed state 尺寸稳定；desktop sidebar 220–480px pointer/keyboard resize，Pinned/Projects section 独立收展且偏好跨刷新，mobile 保持 fixed drawer | ✅ | UJ-24 | INC-19/23/29/38/40/41/60/65/78/87/88/89/90/92/93/94 · TestMetaStoreProjectOverlayRoundTrip/TestProjectUpdatePinnedRemoved · Sidebar.nav.test.tsx project controls/new-chat/section fold/selected-session fold/resize/action-density/session states/context actions/duplicate labels/project row actions · viewModels.test.ts current-cap/fold precedence/last-update order · viewModels.nav.test.ts pinned/workspace-less last-update order · Composer.projects.test.tsx project seed/no-project disclosure · buttonPress.test.js · conciseTitle/frontend view-model/timeline/store session tests · viewModels buildSidebarModel scratch-split 断言 · QA-27/34/36/41/42/43/61/78/79/80/81/83/84/85 + INC-78 闸B(run 29672007663:分组/触屏改名/刷新持久) |
...
| Web UI progressive-disclosure composer | ✅ | UJ-22/24 | QA-69(真浏览器:Add/Advanced 两组根动作)· INC-19/23/38/40/65/88 · 默认输入/附件/access/model/send；New session 未选 Project 时上缘只显示 project picker，选定后才显示 Local/New worktree 与 Branch，Project/Branch 可搜索，worktree 尊重 selected ref；Goal/Repeating/Best-of-N、persona 与 YAML 收入 Advanced；占位面(Plugins 五件套/Environment chip/Settings Not-wired 字段)已移除(PLAN 5.1,原则:UI 只承诺已接线的能力) |
...
| Web UI turn 收尾（最终 answer 前 journal-ts Worked duration；message Copy；有 durable anchor 的 human/final assistant message row 可直接 Continue 到 dormant new session，attachment-only 同样可用；raw checkpoint 选择仍在 Advanced；终态 recovery 例外保留必要续跑动作；有 composer 的 session 以 composer Stop 为唯一直接停止入口，RunView header Stop 例外；真实 diff Changes 摘要→Review；Changes `Working tree / Last turn` durable 范围，缺 barrier truthful unavailable；generated/unbounded untracked output 有计数地隐藏，超大 diff 默认逐文件折叠且首 paint 前决定 disclosure） | ✅ | UJ-24 | INC-38/57/60/64/65/88/91 · TestShadowRepoDiffAgainstSnapshot · TestCLIDiffLastTurnJSON · TestHandleDiffLastTurn/TestHandleDiffNestedWorkspace/TestContinueFromMessageHTTPContract · completedTurnDurations/summarizeChanges/shouldExpandDiffByDefault/timeline message action tests · QA-41/60/61/80/82 |
