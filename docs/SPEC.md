...
| Web UI 产品面：New session/Scheduled runs/Pinned/Projects→sessions、单一 thread、Changes、deep link、responsive sidebar；首次 session readiness 不投影假空态/raw id；共享大历史首 40 条立即 ready、后台 80/页补齐且 refresh 不重入；session row 只有一个 `…` 管理入口，普通菜单不重复暴露 raw id/deep-link；自动标题去同质指令前缀；自动 workspace 各自成组（默认名 `Scratch · 创建时间`，组键=真实路径，不聚合混合；INC-78 撤销单一 Scratch 聚合）；project hover/focus 摘要+project-scoped New chat 快捷入口（seed Home composer，不提前建 session），标题行 ⋯/右键同源六项菜单（pin/Finder/permanent worktree/rename/archive/safe Remove），project pin/remove overlay 可恢复；project group fold 优先于 active session，active group 在 section cap 外只保证 heading 可见；connected daemon 是静态 status，offline 才显示 Restart；button pressed state 尺寸稳定；desktop sidebar 220–480px pointer/keyboard resize，Pinned/Projects section 独立收展且偏好跨刷新，mobile 保持 fixed drawer | ✅ | UJ-24 | INC-19/23/29/38/40/41/60/65/78/87/88/89/90 · TestMetaStoreProjectOverlayRoundTrip/TestProjectUpdatePinnedRemoved · Sidebar.nav.test.tsx project controls/new-chat/section fold/selected-session fold/resize/action-density · viewModels.test.ts current-cap/fold precedence · Composer.projects.test.tsx project seed/no-project disclosure · buttonPress.test.js · conciseTitle/frontend view-model/timeline/store session tests · viewModels buildSidebarModel scratch-split 断言 · QA-27/34/36/41/42/43/61/78/79/80/81 + INC-78 闸B(run 29672007663:分组/触屏改名/刷新持久) |
