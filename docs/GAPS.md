...
| UJ-16 三路并击 | ✅ | 并行隔离+选优 ✅；胜者晋升 ✅（PLAN 5.8 ar promote/Apply winner） |
...
in-flight 子 run 的 settle-from-child-fold、barrier 对在飞 child run
的处置语义。实现完全缺失。
→ UJ-18
...
run 曾与一次完成绑定：end_turn 即 run_ended。"答完 → 等下一条
...
唯一入口（`Loop.Run` opening prompt + `journalInput` 每条 send）展开，
...
需求之一，但从未成文为 journey；S6 在"run 与 session 生命周期绑定"的
...
**G15 best-of-N 胜者晋升语义 — ✅ 已消解（2026-07-19,PLAN 5.8）**
取 apply-diff 形态:`ar promote <sid>`/webui "Apply winner"——workspace
shadow repo 内做 base(SeriesStarted 钉的快照)→winner 树 binary patch,
`git apply --check` 干跑通过才落、冲突原样报错零改动(INC-49 教义),
改动不 staged 由用户 review。fork 接管形态未做(需求出现再议)。
闸门:TestPromoteWinner{CleanApply,ConflictLeavesWorkspaceUntouched,
NoChanges}。
→ UJ-16

### 监督与恢复

**G27 双 session 形态的显式重开 — ✅ 已消解（2026-07-05,静止模型决策 #31）**
（编号更正：本条曾误编 G24，与"团队成员 worktree 快照时机"重号；
2026-07-10 更正为 G27。）第二种 durable 会话形态已删除，交付非终点，
任何 session 都可继续。`send` 重开后延续同一 context；epilogue、outputs
与 barrier 按每次静止边界重复执行。闸门：TestSendReopensClosedConversational。
→ UJ-03/09 连带

**G22 监督语义：崩溃自动恢复与重启接续 — ✅ 已关闭（INC-54+INC-71+INC-72，2026-07-18）**
...
根因是 batch 内每个 launch 独立读取同一个 parentRemaining，首个无 cap child
预留全部。INC-66 在执行 batch 前统计可解析 launch 数，按
`remaining/remaining_launches` 动态 fair-share，再受 child cap 约束；小 cap
留下的余额自然供后续 sibling 使用，总 reservation 仍不越 parent cap。
...
Team Lead=shared）。闸门：TestIsolatedChildPromptCarriesSnapshotNotice
...
**G30 SPEC 弱锚存量燃尽 — ✅ 已关闭（audit-0717 C1+F1/QA-69，2026-07-18）**
24 行还具名测试锚(C1);最后 2 行(composer 披露/用户消息折叠——
jsdom 无布局测不了的两行)由 QA-69 真浏览器场景收口,债基线清零、
只防回归。原登记:
...
逐步截图）run #4 首次全绿（4/4，2026-07-12）。**余项更新
（audit-0717 B8，2026-07-18）**：schedule interval/cron 内联校验已落
（Schedule modal + Loop launcher，scheduleValidate 镜像 + 字段旁
role=alert + Start 禁用）；错误 `.details` 披露已落（toast 内
Details 折叠，RunModal/worktree/Diff/store 站点接线）。仍开：更多
Settings/审批卡黑盒覆盖。原余项文句：schedule
interval/cron 内联校验、错误 .details 披露 UI、更多 Settings/审批卡交互
黑盒覆盖——待排期,非阻塞。三轮黑盒真机产品级新发现已归零(曲线压平)。
**iOS session 细节收口（INC-68，2026-07-13）**：共享 587-session store
以指定中文 prompt 新建真 Gemini session，并在 390×844 打开三 worker parent
与全部 child、failed/recovery/Changes/New session/pickers。修复 mobile sidebar
与 Environment 双 overlay、具体+泛化 failure 双卡、activity 原生 marker/裸数量、
child link 黏连、child header machine slug；572 frontend tests 与 QA-68 真机全绿，
console warning/error 与横向 overflow 均为 0。既有低优先余项不变。
→ UJ-01/UJ-24（webui 产品面）

**G37 设计契约加固审计 — ✅ 已关闭（INC-67，2026-07-13）**
全量/race/边界审计发现并修复一组不需产品裁决的 correctness/security
缺陷：session 路径可经 `..`/symlink 越共享 store；上传超限静默截断；JSON
尾随命令被忽略；memory/settings/trust/hooks/artifact 多 writer 丢更新或 torn
write；daemon socket chmod 失败仍服务；16-bit session suffix 易碰撞；slash
branch 新建被误拒；Scanner 超限伪装正常结束并可能挂子进程；clean checkout
在 frontend build 前跑 embed 测试必 panic；deploy 可把仅 `.gitkeep` 的 dist
嵌进启动即 panic 的 binary；取消把 sandbox wrapper reaped 误当进程组消失，
TERM-resistant 孙进程可变孤儿。统一 advisory flock + unique temp fsync/rename、
...
**G55 Scheduled 全局 series pause/resume lifecycle 与 paused projection 缺失 — 🟡 闸门 B 待验（INC-98.5a，中）**
Codex Scheduled 的真实 Paused filter 显示 `cloc`，说明 pause 是计划 series 的持久 lifecycle；
INC-98.5a 已实现 canonical merged-stream interval/cron/self-paced 的 typed durable
Pause/Resume：command fsync-before-ack、cadence wait 撤 timer、in-flight child 结算后停、
paused boot-sweep exclusion、pending command crash replay、resume clock base 重锚/no catch-up，
以及 CLI/API/session capability 与 Web UI Active/Paused/action 投影；旧 `DriverStarted`
journal 明确无 action。scripted 闸门 A 与 production build 已绿。仍需在不影响现有 live
scheduled work 的获准窗口完成 shared-store `pause in-flight → daemon restart → still paused
→ resume → re-anchored next tick` 与 1100×700 双侧视觉/交互闸门 B；完成前本项不关闭，
SC-05 继续 GAP。
→ UJ-14/UJ-24
...
**G39 spawn 子 agent 卡不可见审批死锁 — ✅ 已关闭（INC-81 + INC-98.3g，2026-07-22）**
**闸门 B 关闭实证（QA-88 TH-09）**：shared-store 真 Gemini child 以 bash `pwd` 停在
approval；fresh deep link/reload 仅靠 parent inspect tree 即恢复具名 approval card 与
Attention，Approve once 精确送达 child sid，child 在真实 isolated worktree 执行，parent 自动
收到完成结果并回 `waiting:input`。修后 Subagents 也显示 `Needs approval`，card 明示真实 child
worktree。`34/35/37/38` 截图、parent/child inspect/events 与空 browser logs 均保留在
`qa/runs/2026-07-22-QA88-98.3f-approval/`。sidebar 的全局 Ready 错标是独立 session-list
projection 缺口 G49，不再混入本条“parent thread 内不可见死锁”。
...
**定因进展(QA-0718 第六轮)**:父 journal(diag run 29658309926)显示
`spawn_requested`×2 + `activity_started{Background}` 后无任何终态事件,
且 daemon 重启 + journal replay 后 rail 依旧 "still running"——running
态源自 journal 悬挂配对,非前端轮询。代码对读:child loop 的
`Approvals: l.Approvals`(spawn.go childLoopWithExec)把 child 审批汇入
同一 seam,但 webui 只订阅**父会话**的 journal 流,child 的
`waiting:approval` 落在 child 子 store(`<store>/sub/<callid>-a1/`,不入
`ar sessions`)——**假说:child 卡在无人可见的 Write-file 审批上,
不可见审批死锁**。转储已扩为含 sub-store journal,下轮重现即可实证。
修复面(surfacing child approvals→父 ATTENTION/可检视 child)涉及
...
共享 Agent definition 与 session model selection 的职责分离也已由 INC-96
覆盖：runtime/CLI/Web UI/TUI 面向同一 catalog，user override 与 name/path
解析明确；Agent YAML 不再拥有 model，Web 显式提交 model/effort，CLI 省略时
走 user default/compiled default。该需求没有遗留实现缺口，不新增 GAPS 编号。

已记 PROGRESS backlog 的实现项（不再重复）：HTTP/WS 壳、output 进度 tail、
overlap:interrupt。
