...
**G37 设计契约加固审计 — ✅ 已关闭（INC-67，2026-07-12）**
...
在 frontend build 前跑 embed 测试必 panic。统一 advisory flock + unique temp
fsync/rename、严格输入 cap/EOF、显式流错误与 64-bit entropy 后，针对性 race
5 轮和全量 gate 通过；共享 store/Web UI 重启验收见 QA-67。
