...
| 56 | permission modes | 6 档：Manual/acceptEdits/plan/auto/dontAsk/bypass；**shift+tab 运行中随时切换** | ✅ default/plan/acceptEdits/bypass（S2/S3）+ headless fail-closed（=dontAsk 语义，决策 #34 收紧 driver ask→deny）；**auto ❌；运行中切换 ❌（仅 plan→default 已接线，default↔acceptEdits 用户命令入口缺失，G29）** | auto 见 §3.3；切换=GAPS G29 |
