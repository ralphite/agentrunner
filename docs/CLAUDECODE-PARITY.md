...
| 18 | **microcompact（不调 LLM 的轻量层）** | 先把可重算的旧工具结果换成 `[Old tool result cleared]`（Read/Shell/Grep/Glob/Web*），不动对话；四级体系 snip→microcompact→collapse→autocompact | ❌ 只有"全量 LLM 摘要"一档 | **P0①**（高杠杆：零模型成本回收大头 context；UJ-09） |
...
| 30 | 记忆写回（"记住 X"→文件） | 自然语言即写 auto-memory；"add to CLAUDE.md" 手编 | ❌ | G9（INC-D4 设计稿已在） |
...
| 35 | Glob/Grep | ripgrep、output_mode/-A/-B/-C/multiline/type | ✅ INC-3（G18a 关闭；参数面较薄） | 参数增强候补 |
...
| 45 | Skill 工具（模型侧 invoke skill） | Skill(command)；`disable-model-invocation` 可关 | ❌ skills 是读侧注入（S5）；模型不能主动 invoke | 候补（见 §3.5） |
...
| 53 | Bash 规则工程细节 | 任意位通配、`:*`≡尾部 ` *`、**复合命令逐段匹配**（&&/;/\|）、**wrapper 剥离**（timeout/nice/xargs）、只读命令内置免提示集 | 🟡 command 规则在（S2）；复合拆分/wrapper 剥离/只读集三件套 ❌ | 候补 P1（权限疲劳的主解） |
...
| 58 | "允许且不再问"持久化 | Bash 按 project+命令永久、文件修改到 session 末；复合命令拆条存（≤5） | ❌ 单次批/拒 | **G5**（INC-D5 设计稿：下次生效不触不变量） |
| 59 | protected paths（`.git`/`.claude`/rc 文件等写保护长表） | 除 bypass 外从不自动批 | 🟡 凭据硬排除（读侧，决策 #15c）✅；写侧 protected 集 ❌ | 候补 P1（配 acceptEdits 才安全） |
