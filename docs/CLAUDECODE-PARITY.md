...
| # | 功能 | Claude Code | AgentRunner | 缺口 |
|---|---|---|---|---|
| 32 | 文件读/写/编辑 | Read（图/PDF/ipynb 多模态、offset/limit、PARTIAL）/Write/Edit（read-before-edit 三检查、replace_all） | ✅ read/write/edit_file（S1）；**工具读图/PDF ✅ INC-33**（media envelope+CAS ref,assembly part 注入,journal byte-free;QA-38 真机像素级验证）；read-before-edit 纪律 ❌（INC-21 defer） | ipynb/offset-limit 低优；read-before-edit 护栏候补（SPRINT #11 📐） |
...
| 35 | Glob/Grep | ripgrep、output_mode/-A/-B/-C/multiline/type | ✅ INC-3 + **INC-22 + INC-24 + INC-27**（2026-07-09）：case_insensitive/glob/output_mode(content/files_with_matches/count)/-A/-B/-C context lines/multiline 跨行 regex((?sm) 整文件匹配)，QA-30+QA-31+QA-35 真机；默认=旧行为 | type 过滤余项（低优,可由 glob 近似） |
| 36 | 相关性检索 | ❌ 无等价物（LSP 是符号级） | ✅ keyword_search（S7,BM25 词法排名——非语义 embeddings,PLAN 5.2 如实改名） | — |
...
| 45 | Skill 工具（模型侧 invoke skill） | Skill(command)；`disable-model-invocation` 可关 | ✅ **INC-20 + INC-31**（2026-07-09）：`skill` 工具按 name 返回正文（QA-29）；`context: fork` 在一次性子 agent 执行——ingest 展开为 spawn_agent{role}（RoleSpec 冻结/树预算/深度上限全链复用，agents_dynamic 门控，frontmatter allowed-tools ⊆ 父），TestForkSkill* · QA-37 真机；维持命令=用户宏裁决不动 | — |
...
| 56 | permission modes | 6 档：Manual/acceptEdits/plan/auto/dontAsk/bypass；**shift+tab 运行中随时切换** | ✅ default/plan/acceptEdits/bypass（S2/S3）+ headless fail-closed（=dontAsk 语义，决策 #34 收紧 driver ask→deny）+ **运行中切换 ✅ 已实现 INC-42**（2026-07-10，G29 关闭：`ar mode`/webui `/mode` = 对方 shift+tab；差异记档——我们 plan 退出仍须 exit_plan_mode 审批、bypass 不可 runtime 进入，比对方更严）+ **INC-58**（2026-07-11：webui composer mode pill 由只读 badge 改为点击切换选择器——Ask↔Auto-accept 可点、Full/Plan disabled 带原因，最贴近对方 shift+tab 手势；QA-51）；**auto ❌** | auto 见 §3.3 |
| 57 | **auto mode（分类器审批）** | 2.1.200 起默认：server 端注入探针 + Sonnet 4.6 transcript 分类器双层；黑白名单 `autoMode.{allow,soft_deny,hard_deny}`；连拒 3 次回退人审 | 📐 设计稿 INC-32 awaiting-review（分类器只接手 would-ask、fail-closed 不变式、连拒回退、headless 收紧不动;判定 journaled 反超）| §3.3;实施拆 32a/b/c,待用户裁决 |
...
| 78 | 内置 subagent 库（Explore/Plan/general-purpose，跳 CLAUDE.md、thoroughness 档） | 内置 + 可禁 | ✅ explore/plan 随发行 embed（只读工具面 read_file/grep/glob/keyword_search）；spec `agents:` 列名即 spawn 无需自带 spec，内置优先同名 sibling，model 继承父（INC-25；TestBuiltinSpec*/TestResolver* · QA-32）；**默认全自动可用**（不列名即可用）拆 #16b | 默认可用 #16b → 变更单 INC-34 awaiting-review（推荐 B:已声明 spawn 面则内置隐式在册,opt-out 者守边界） |
...
| 91 | **结构化输出**（`--json-schema` → structured_output 字段） | schema 校验、失败重试 | ✅ `ar new --json-schema <path>`：客户端 schema 校验 + 失败重发纠正（`--json-schema-max-retries`，默认 2）+ 打印 canonical structured_output（INC-26；纯 `internal/structured` 包用 google/jsonschema-go；TestStructured*/TestNewJSONSchema* · QA-33 真机 Gemini）；与 --detach 互斥 | **INC-35** provider-native（spec `output_schema`,gemini 原生约束 tool-less 轮,QA-39 真机裸 JSON;anthropic downgrade）;`--json-schema` 端到端下传+durable 事件拆 8c |
