...
| 35 | Glob/Grep | ripgrep、output_mode/-A/-B/-C/multiline/type | ✅ INC-3 + **INC-22 + INC-24 + INC-27**（2026-07-09）：case_insensitive/glob/output_mode(content/files_with_matches/count)/-A/-B/-C context lines/multiline 跨行 regex((?sm) 整文件匹配)，QA-30+QA-31+QA-34 真机；默认=旧行为 | type 过滤余项（低优,可由 glob 近似） |
