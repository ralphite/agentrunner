...
> 表述（run=task-to-completion、阻塞 spawn 为唯一形态、旧分层图）以
...
- spec、event、API 的向后兼容。event schema 变更即丢弃旧 run 日志重跑，
  不做 migration。`SessionStarted` 记录代码/spec 版本，resume 时版本不匹配
...
历史教训（v1 的失败诊断）：它把"本分"默认成了"把一个 task 跑到完成"，
...
- **消息改变编排**：用户 steer 消息进父 inbox → 下个 turn
  模型看到它 → 模型自己决定发 `task_kill(h2)` + `spawn_agent(...)`
  工具调用。编排的智能在模型，runtime 只提供"随时能投、随时能杀、
  随时能起"的原语。
- **interrupt 与输入分立**：输入进 inbox（追加语义，不打断）；
  interrupt 是一个**带外信号**（不进 inbox），直接 cancel 当前 turn 的
  活动 ctx，把部分输出收尾成 journal——通常此时 inbox 里
  正躺着用户那条"改方向"的消息，随即触发下一个 turn。两个通道，
  两种语义，同一个交汇点（安全边界，或 turn 之间）。
...
task。现在"任何一方对 session 说话"统一成"往 inbox 投一条 `Input`"。
...
2. **journal-inputs-first**：一条输入先落 journal 成 `InputReceived`
   ...
   daemon 把每条投递先 redact+fsync 进 per-session mailbox
   （`inbox.jsonl`，单调 delivery_seq）再回执"delivered"；消费侧
   journal 回写 seq，fold 记高水位；resume 重放未消费尾巴、按 seq
   去重——确认即持久，at-least-once + 去重 = 恰好一次。
...
Session。** v1 曾有 spawn 阻塞路径、后台 task 路径、driver 的 child
路径三套各自为政的"子执行"；目标形态只有一套（当前收敛程度见 §17
实现状态注记）。

**生命周期全部是 inbox 动作**：

- **启动**（非阻塞）：父 turn 里模型调 `spawn_agent{agent, task, budget}`
...
- **完成 → 激活父**：子 session 走到它的待命点且被标记为"一次性任务"
  ...
  的先投先处理。
- **杀死**：父 turn 里模型调 `task_kill{handle}`，或用户
...
- **改变编排**：steer → 模型在下个 turn 同时发
  `task_kill{h2}` 和 `spawn_agent{迁移文档}`。runtime 不需要懂
  "重定向"，它只提供杀和起。

**一次性任务 vs 持续会话**：子 session 默认是"一次性任务"——完成即向父
投回执并进入终态。但因为它就是个 Session，它**也可以**是持续的（父保持
它的 handle，多次投 `user_message` 复用它）。不为这两者建两套东西，
...
父 inbox；子还在跑则重新挂接（settle-from-child-fold 纪律）。
...
generation step 预算（防单 turn runaway；task 形态按整个执行累计）。
...
- **后台 effect 不阻塞 loop**：background call 的立即配对结果就是
  ...
  `task_output`（读 log，read-class）/ `task_kill`（协作取消，
...
[3] Budget           # turns/tokens/cost 从 event stream 统计；
  ...
  ▼
[4] Execute          # 以 activity 执行（retry/cancel 语义见 §6）
  ▼
[5] Hooks (post)
```
...
- **每种关卡结果都定义"模型看到什么"**。所有 provider 都要求 tool call
  ...
  - budget 超限（session 级 token/cost/turns）→ 让模型收尾的最后一条
    ...
    （spawn 深度/扇出，同在 budget 关卡校验）→ error 结果，loop 继续；
...
`spawn_agent` / `task_kill` / `ask_user`(wait-class，向用户提问=待命
...
1. **所有外部输入 journal 成 event，先落盘再消费。** 用户消息、steering、
   interrupt、审批应答、timer 到期——任何 loop 能观察到的输入，
   都以 `InputReceived` 类 event append 进该 session 的 stream，然后才
   进入处理。崩溃时不丢审批、不丢插话；历史完整可审计。（投递侧的
   durable mailbox 见 §2 铁律 2。）
2. **State 是 event log 的纯 fold。** `state = fold(apply, events)`，
   apply 是纯函数、不读时钟、不执行任何代码副作用。因此对话状态永远可
   从 log 重建。
3. **Snapshot-resume。** 在安全边界给对话 state 打 snapshot；
...
journal 进来即把 session 带出等待——未决审批按 denied-by-interrupt 解决，
...
- **待命中的 session**：无事可做——待命跨进程存活，journal 与上下文
  ...
  **显式重开手势，对任何 session 成立**（含已 close 的、已交付的
  task——close/kill 意图与 TaskCompleted 回执只约束自动路径，见 §12
  终止语义）；自动路径（timer sweep、boot sweep）绝不越过意图记录。
- **turn 中途崩溃**：in-doubt 纪律（见上）。**分模式**：
  自愈渲染（`[interrupted by crash]` 后续聊）只对 conversational 会话
  生效——长期会话必须自愈；task 模式保留 refuse-and-surface
  （InDoubtError 上浮给调用方处置）。进了副作用关卡没 `EffectResolved`
  的两种模式都上浮（hooks 可能半跑）。
...
3. 用户投 user_message("别查依赖了，看迁移文档")  → inbox
   loop drain → turn3: 模型发 task_kill(h2) + spawn_agent(迁移)→h4
...
- **`outputs:` 声明 = 交付物 contract**：spec 声明期望产出（name、
  path、required），task 模式收尾 epilogue 自动 publish 并检查 contract
...
- **命名空间与类别**：MCP tool 在 permission rules 里只以全限定名
  ...
  排除），除非 spec 显式为其标注类别。网络出口记账见 §5（恒 "all"）。
...
- **两种运行形态**：**conversational**（默认交互形态——答完待命，
  没有"终结"这回事，close 只是意图记录）与 **task**（一次性任务跑到
  完成并**交付**，子 session 与 driver 迭代用）。形态记录在
  `SessionStarted`；恢复语义按形态分流（§6）。
- **终止语义（2026-07-05 裁定）**：**待命是唯一的静止状态；"结束"是
  journal 的形状，不是一个状态**。journal 只记录两类终止相关事实：
  - **意图**——用户显式 close/kill：`SessionClosed`（source=control）
    与任务取消事实。意图只约束**自动**路径（timer/boot sweep 不得
    唤醒它），**不**约束用户显式重开：对任何 session，一条显式 `send`
    都是合法的重开，同一上下文继续。
  - **回执**——task 形态完成时 journal `TaskCompleted{reason}`（交付
    时刻：driver 判定迭代、父结算子任务、headless 取退出码都读它）。
    回执是事实，不是封印，同样不阻止显式重开。
  - 除此之外**没有"session 结束了"的事件**：conversational 的 turn 以
    final generation 自然结束，待命由形状自明——在事实之上再造状态机
    只会制造边界情况（v2 收口修的"send 假确认"一族即其代价）。
- **task 形态的收尾是固定 epilogue**：(1) 按 `on_run_end` quiesce 后台
  任务（`await` 是纯静默等待——完成只入 journal 不再进对话，且必有
  durable timer 兜底；默认 `cancel` → 协作取消残留 task）→ (2) 自动
  publish `outputs:` 声明的交付物并检查 contract → (3) 切终态
  `CheckpointBarrier` → (4) journal `TaskCompleted` 回执。任何给
  "task 收尾"加步骤的 feature 都必须挂进这个序列，不得自行定义时序。

### Fork / Rewind（时间旅行，扩展层）

- **fork/rewind 的唯一合法目标是 `CheckpointBarrier` event**：barrier
  是 **consistent-enough cut**，在安全边界与 task 收尾（epilogue 固定
  ...
  stream 的 final seq）+ workspace snapshot ref + **in-flight 后台任务
  的处置向量**（v0 一律 `cancel_at_fork`：fork 出的 session 不复活任务，
...
- **fork** = 在新 id 下复制该切面内的 events，以
  ...
  经父 journal 链回溯），tasks 处置向量在复制时**落实**（cancel_at_fork
  任务获得合成终态，fork 的 fold 无 in-doubt 活动）；`sub/` 子 journal
...
- **goal 和 loop 是同一个 driver actor 的两种 schedule**，one-shot 是
  ...
  一个 **fresh child session（task 形态）**（同 spec → prefix 逐字节稳定可跨迭代命中
...
- **统一事件族**：`IterationScheduled / Launched / Completed`、
  ...
  child 目录（sub/iter-N，已终态则从其 fold 结算）**保证。
...
- **驱动与会话内核的关系（目标形态）**：driver 是"一种特殊的、由程序
  而非人投递 inbox 的父 session"——当前实现仍为独立子系统（v1 兼容），
  收敛与否见 §17 与 GAPS。
...
- **routing provider**：按子 session 的 id/task 路由各自的脚本，
...
| 21 | 运行模式 | one-shot/goal/loop/best-of-N（`parallel{n}`）是同一 `IterationDriver` 的四种 schedule；每轮迭代 = fresh child session（task 形态） | 避免多套近似驱动机制；fresh session 保 prefix 稳定与故障隔离。 |
...
| 24 | 收尾时序 | task 形态固定 epilogue：quiesce 后台任务 → auto-publish outputs → 终态 barrier → `TaskCompleted` 回执 | 多个 feature 都往末尾加步骤，顺序必须唯一定义。 |
| 25 | 中心模型（v2） | Session = 持续消费 inbox 的 actor；续聊是循环的默认形态（conversational），task 跑到完成是另一种形态 | "本分"是持续的多方协调；run=task-to-completion 的默认让日常交互全变成补丁。 |
...
| 29 | 恢复分模式（v2） | conversational 崩溃自愈（渲染 [interrupted by crash] 续聊）；task 保持 refuse-and-surface | 长期会话必须自愈；无人值守任务需要显式处置。 |
| 30 | 终止语义（2026-07-05） | 待命是唯一静止状态；"结束"是 journal 形状而非状态。只记两类事实：**意图**（`SessionClosed`/kill 取消——只挡自动恢复）与**回执**（`TaskCompleted`——交付时刻，不封印）。显式 `send` 对任何 session 都是合法重开 | 在形状之上再造终态状态机只制造边界情况（v2 "send 假确认"一族即其代价）；可续聊是持久化模型免费给的普适性质。 |
...
- **C5 杀子 agent**：task_kill(handle) → 子收尾（部分输出留存）→
...
（`spawn_agent` / `task_kill` / `SpawnRequested` / `SubagentCompleted`
...
- **§2 inbox 字面统一度**：`user_message` 与 `control{kill}` 已按
  ...
  `child_result`/`tool_result` 语义上是 inbox 输入，机制上暂由
  background activity（`ActivityStarted{Background}`→终态，fold 渲染
  user-role 消息）兑现——语义等价、事件形状不同；字面统一与否待
  真实压力再决。
- **§3 "一套机制取代三套"尚未收敛**：阻塞 spawn、后台 spawn、driver
  子系统并存（阻塞路径与 driver 保留 v1 兼容）。
- **§1 interrupt 语义**：实现新增"待命处 interrupt = close（记
  `SessionClosed` 意图）"（交互惯例）；turn 中 interrupt 仍是 steer。
...
| **max_generation_steps**（spec 字段） | **per-turn 的 generation step 预算**（conversational 从最后一条用户输入起算，防单 turn runaway；task 形态累计）。 |
...
| 显式终止（意图） | `SessionClosed`（close）/ 取消事实（kill）——只挡自动恢复，不挡显式重开 |
| task 交付（回执） | `TaskCompleted{reason}`（epilogue 收束；不是封印） |
...
| **spawn** | 工具 `spawn_agent`：阻塞与 background 两形态；后者立即返回 handle。 |
| **handle** | spawn/后台任务的立即配对结果；kill/output 都凭它。 |
| **child_result** | 子完成/失败/被杀的回执（event `SubagentCompleted`），投父 inbox 触发新 turn；先回先处理。 |
| **kill** | 工具 `task_kill`：凭 handle 协作取消，与后台 bash 共用原语；用户侧 `ar kill`。 |
...
| **显式重开** | `send` 是用户的显式手势，对**任何** session 成立（含已 close、已交付的 task）——同一上下文接续。自动路径（timer/boot sweep）则受意图记录约束。 |
| **crash vs kill** | 判别器 = journal 里的**意图记录**：显式终止（kill/close/interrupt）留有意图/取消事实，永不被**自动**恢复；crash 的特征是什么都没留 → 有恢复资格。（不变量成文：决策 #30） |
| **epilogue** | task 形态固定收尾：quiesce 后台任务 → auto-publish outputs → 终态 barrier → `TaskCompleted` 回执。 |
...
| **iteration** | driver 的一轮 = 一个 fresh child session（task 形态）。**第三个计数词**：iteration（driver 轮）⊃ turn（对话段）⊃ generation step（模型调用），三者不混。 |
...
| 词 | 坑 |
|---|---|
| **run** | 已废：说"task 形态的一次执行"。历史文献（archive/）中大量出现。 |
