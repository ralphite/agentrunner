...
| close/kill 标记 | `SessionClosed` / 取消事实（含来源 user/parent）——只被检查引用，不挡显式 send |
...
| 词 | 坑 |
|---|---|
| **run** | 已废：`ar run` 仅是"开 session+发消息+等静止+读结果"的便捷命令名。历史文献（archive/）中大量出现。 |
| **step** | 裸词禁用；说 generation step 或 tool step。 |
| **iteration** | 只指 driver 轮（fresh child session），不与 step/turn 混用。 |
| **task** | 已废（2026-07-05）：后台工作说 **handle**；"task 形态"概念删除（静止模型，决策 #31）。 |
