...
| **IndexStore** | 第四类状态：可随时从 workspace 重建的派生索引（keyword_search 底座）；不入 journal/快照/fork。 |
...
| **IterationDriver** | best-of-N / one-shot 的 driver actor（fresh-child-run）——**并行选优/批式专用（INC-102 收窄）**；批式 loop / driver-goal 仅余 legacy 兼容投影。**loop 与 goal 的用户默认形态均为会话内**（loop = in-session schedule INC-74，goal = in-session goal INC-D1；都挂 `agent.Loop`、context 延续）——见 §13、决策 #21。 |
...
1. `ui/`：Button、IconButton、Field、Input、Select、Status、Empty/Error 等基础
   primitive；
2. `components/`：SidebarProjectItem、SidebarSessionItem、Popover、Markdown 等
   跨 feature 展示组件，以及保持旧 import 稳定的薄 compatibility entry；Composer、
   Session、Timeline 这类高层 entry 不得在此持有 state、effect 或 JSX 实现；
3. `features/`：Composer、Session、Timeline、Scheduled 等 controller/model/action
   与 production view 边界；API/store/polling/SSE/storage/scroll lifecycle 在这里，
   controller 通过 typed props/render slots 驱动纯 view，纯 view 不得读取 AppServices
   或 store；
...
依赖主方向为 `app/pages → features → components/ui`。`components/` 中指向 feature
的 compatibility entry 是保持 public import 的唯一例外，必须是无 state/effect/JSX
的薄 re-export，feature 也不得再反向 import 该 entry。兼容 import 可以从旧 module
re-export，但实现所有权必须落在上述真实层；禁止用 re-export 或单纯移动文件冒充
controller/view 拆分。所有 exported visual component 都进入 manifest；非视觉
controller hook 不创建无意义 Story。
