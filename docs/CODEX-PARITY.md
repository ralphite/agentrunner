...
| NS-08 | access/model/thinking picker：搜索/切换/disabled/error | GAP | 2026-07-22 `QA88-98.2d-model-access/01..10/20..34` + `QA88-98.3g-errors/40/50..56` 同逻辑 1952×1465：双侧 root/model/effort/access 真实展开；Codex 新增 Full Access 风险确认后，我方补同态结构化 files/terminal/internet 说明，Cancel/Confirm/draft/focus 真浏览器通过。仍因 Codex 有真实 `Fast` service tier 与 `Ultra`、我方按 G45 不画假选项而记 GAP；disabled/error 继续取证 |
...
| TH-06 | queue/steer toggle、queued bubbles、⌘⏎、reorder 边界 | GAP | 2026-07-22 `QA88-98.3c-queue-steer/01..08/17..19`：双侧真实 long tool + running follow-up；我方 Queue/Steer toggle、Cmd+Enter opposite mode、durable order/Withdraw 通过，并修 queue optimistic+durable 双影及 Withdraw 幽灵。2026-07-23 `QA88-98.4y-queue-steer-review/05..09` 又修复 Steer 被 `/queue` 错投为第二张 `Queued/Withdraw`：修后长 foreground tool 中只留 `steering…`，同 turn journal/ACK 通过；Codex queued row 可原子 `Steer`，我方 backend 只有 unqueue/send 两条非原子命令，缺 G47，禁止竞态拼接 |
...
| TH-08 | ask_user waiting、answer、reload、cancel 边界 | PASS | 2026-07-22 `QA88-98.3e-ask-user/01..36`：Codex Default 明示 request_user_input 不可用；Plan 生成 `Asked 1 question` disclosure，但 app bridge 当批记录 `No answer provided`，Alpha 走普通 follow-up。AgentRunner shared 真 Gemini 逐项通过 structured single/multi/free-text/two-question/Skip、reload persistence 与普通 composer answer；真浏览器反证只靠 AskResolved poll 仍有 receipt race，并确认实际 send receipt 是 `delivered`；以 AskForm 可见 context + 成功 receipt 修到无需 reload 即零 `queued…` 幽灵。2026-07-23 `98.4o/98.4q` 又覆盖 root/child 全局 `Needs answer`、旧 parent 超出最近 40 条、parent→child AskForm 直达、typed Beta resolve 与 restart 不复活；ordinary idle/resolved 保持 Ready。`98.4aa` 再以 root ask + child approval 同时 pending 的真实链证明先 answer 只消一项、reload 不复活 |
| TH-09 | approval：details/approve/deny/child approval/reload | PASS | 2026-07-22 `QA88-98.3f-approval/02..38`：Codex Ask access 下真实 sandbox DNS approval 的 Allow once/Deny；AgentRunner shared 真 Gemini root approve/deny/details/reload 与 child fresh-load/approve/recovery 全通过。修复 child row `Ready`、parent workspace 误标及 1280 Environment 遮卡；修后同态合并图 `38`、browser logs 为空。2026-07-23 `QA88-98.4o-typed-attention/07/08` 又验证 parent list 递归 child typed wait：单 child `approvals:1`、三 child team `approvals:4`、重复 spawn 去重、resolved nil，sidebar/palette 均为 `Needs approval` 且跨 restart 保持，G49 关闭。`98.4aa` 的复杂双待办链再验证 answer 后 child approval 独立保留，Approve once 后 child→root 原子收敛 |
| TH-10 | provider/network/tool error、Retry 原位替换 | UNTESTED | 2026-07-22 `QA88-98.3g-errors/41/43..49/54/59..64`：双侧同一 `bash exit 23` 真实 tool error 均由 Worked/disclosure + assistant explanation 收口；Codex nested detail 与我方 compact transcript 都保留 concrete command、stderr/Exit 23。2026-07-23 `QA88-98.4z-failure-retry-continuity/02..09` 又以 legacy+fresh shared data 证明 model-not-found→换模型→manual Retry→follow-up，并修复人工恢复被伪称 automatic；Codex provider/network Retry 仍无可控同态证据，故不提前判 PASS |
| TH-11 | completed/failed/recovery/continue terminal chrome | UNTESTED | 2026-07-23 `QA88-98.4m-complex-recovery/01..04`：AgentRunner retained 20-turn/125-agent-step step-limit→bar-t61 fork 真链路通过；默认 latest、较早点按需展开、child durable 继承短标题、stranded 仅 Resume 且 reload 保持。Codex 同态复杂终态仍缺，故不提前判 PASS |
| TH-12 | long thread hydration/scroll anchor/new message badge | PASS | 2026-07-23 `QA88-98.4p-long-thread/01..09`：Codex current goal 与 AgentRunner retained 70-line/4-turn shared Gemini session 均在 1100×700 真滚动；我方运行中追加保持 `scrollTop`、新增 9 activity 可发现。修前 reload 与 SPA session A→B→A 均跳底；修后两路径恢复偏差均 0px，显式 Send/Jump/自然滚底清除位置，Jump badge 对外部新 turn 显示 2 updates。daemon/webui restart、health/versionMatch、body containment 与 browser logs 全通过；合并图 `09`。 |
| TH-13 | goal banner：active/pause/edit/blocked/budget/complete | UNTESTED | 2026-07-23 `QA88-98.4r-goal-progress/01..12`：1100×700 同态 active 对照暴露我方全宽 banner 重复 Environment 的 objective/checks/actions；修为 Codex 式紧凑 `Pursuing goal` 控制条，完整内容留在 Environment。shared 真 goal 覆盖 active→pause→resume→budget stop、edit/direct actions/reload，390×844 无横溢且详情关闭焦点返回 opener。Codex paused/edit/blocked/budget/complete 尚缺同态证据，故不提前判 PASS |
| TH-14 | plan/progress visibility 与完成真实性 | UNTESTED | 2026-07-23 `98.4r/98.4s`：新 driver 在 goal-bar 专区可逆展开 Codex native goal，并捕获独立 `Step 5/7` 摘要；AgentRunner shared 真模型四步 progress 保持 1/4（done/running/pending/pending），新增 `Step 2/4 · Implement…` current-step pill，点击进入 Environment 完整 checklist，pause/resume/budget 不伪造完成。1100×700 合并图 `23`、390×844 详情/焦点/containment 通过；Codex replan 与 failed/complete progress 逐态仍缺，保持 UNTESTED |
...
| EV-03 | Goal/Agents/Attention/Background empty/populated/overflow | UNTESTED | 2026-07-23 `98.4aa` 已在 AgentRunner shared 同屏呈现 root Answer requested、child Approval requested 与 worker；全局 sidebar/palette 由单类型优先修为精确 `2 actions needed`。Codex 同态 overflow 仍缺 |
| EV-04 | child member navigation/full timeline/back/reload | UNTESTED | 2026-07-23 `98.4aa` AgentRunner completed child row→完整 child timeline→deep-link reload→Back to root 全链通过；Codex 同态仍缺 |
| EV-05 | child approval/recovery/answer attention 与去重 | UNTESTED | 2026-07-23 `QA88-98.4o/98.4q` 已完成 AgentRunner shared 单 child、真实三 child team、approval/structured ask、old-parent recent-page refresh、resolve、restart、sidebar/palette/Environment→child 直达与 scripted duplicate-spawn 去重；`98.4aa` 又覆盖同一 root session 的 answer + child approval 并发、分步解决与自动续跑；尚缺 Codex 同态复杂 child 场景，因此不提前判 PASS |
| EV-06 | Environment↔Changes/sidebar/settings overlay 互斥 | UNTESTED | QA-87/86 为我方候选证据；2026-07-23 `QA88-98.4w/09..12` 真验 1100×700 pending root ask 与 child approval 两条复杂链的 Environment→Changes 互斥、close 回焦及 attention 不丢；sidebar/settings 全组合仍待补 |
...
| CH-05 | desktop split/mobile overlay + close/focus return | UNTESTED | 2026-07-22 `QA88-98.4a-changes-shell` + `98.4f-changes-compact`：双侧 1280 split；900 同图显示 Codex 三栏挤压、我方 intentional overlay；我方 900/390 containment 与 focus return 通过。2026-07-23 `98.4w/03/08..12` 又以双侧 1100×700 populated Review 真对照，把我方 conversation/Review 从 359/421px 修至约 390/390px，并真验 pending child approval、structured ask、composer 与 close→Environment focus；Codex close focus 仍未取证 |
| CH-06 | Commit or push menu/validation/progress/error/success | UNTESTED | 2026-07-22 `QA88-98.4h-commit-push/01..10` + `98.4i-conflict-multisession/01..11` + `98.4j-main-macro/01..05`：retained shared repos + local bare remote 真验首次 Commit & push/set-upstream 成功、clean-tree push、no-remote、detached HEAD、non-fast-forward、真实 `UU` merge conflict 与同 workspace 双 session；修复 clean tree 后 Push 不可达、rejected generic error、跨 session stale toast、conflict marker 可误 commit、主时间线隐藏 blocker 五项。双 scope 与主 Changes card 均明示 conflict，Commit/Commit & push disabled、backend 409 且 HEAD/UU 不变，Push 仍可用；900×700/1100×700 无溢出且 logs 空。未触外部 remote；Codex 同态尚缺 |
...
| SC-05 | active/paused/finished/failed/overlap/retry | GAP | 2026-07-22 `QA88-98.3p-scheduled/08/11/13/16`：Codex Paused 真显示 `cloc`。2026-07-23 INC-98.5a 已完成我方 canonical merged-series durable Pause/Resume、真实 Paused filter/status/action capability 与复杂 crash/in-flight scripted 闸门 A；legacy driver 不显示假动作。shared-store daemon-restart + 1100×700 双侧闸门 B 尚未在安全窗口执行，见 G55，故不提前改 PASS。`QA88-98.4x/13..16/20..21` 的 failed/retry 修复保持 |
| SC-06 | run detail/deep link/edit/next run/history | GAP | 2026-07-23 `QA88-98.5b-scheduled-detail/01..10`：我方已用 safe typed projection 真开 responsive same-page series detail，1100×700/390×844 的 deep-link reload、Back、Open history、footer containment 与零 console 通过；修复 mobile sidebar/Back 重叠。shared 500 条中无 paused canonical fixture，loading/error 与 dark theme 也未真验；edit/notification backend 仍缺，见 G56，故不提前判 PASS。Codex split 基线见 `98.3r/01..04` |
| SC-07 | restart 后 cadence/nextRun/status truthful | PASS | 2026-07-22 `QA88-98.3s-create-restart/`：UI 创建 retained `168h` fixture，首轮 Gemini 无工具完成；普通 SIGTERM deploy/restart 后仍 `running / Every 7d / 2026-07-29T21:30:25-07:00`，journal 仅 `series_iteration(n=1)`，timer cancel/re-arm 同 fire_at、无重复 tick。另发现 process-local `#run:run1` reload 失效并修为等待 daemon `sessionId` 后导航 durable route，`Modals.scheduleRoute.test.tsx` 钉死 |
