...
| CH-06 | Commit or push menu/validation/progress/error/success | UNTESTED | 2026-07-22 `QA88-98.4h-commit-push/01..10` + `98.4i-conflict-multisession/01..11`：retained shared repos + local bare remote 真验首次 Commit & push/set-upstream 成功、clean-tree push、no-remote、detached HEAD、non-fast-forward、真实 `UU` merge conflict 与同 workspace 双 session；修复 clean tree 后 Push 不可达、rejected generic error、跨 session stale toast、conflict marker 可误 commit 四项。双 scope 明示 conflict，Commit/Commit & push disabled、backend 409 且 HEAD/UU 不变，Push 仍可用；900×700 无溢出且 logs 空。未触外部 remote；Codex 同态尚缺 |
