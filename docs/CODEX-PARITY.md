...
| TH-08 | ask_user waiting、answer、reload、cancel 边界 | GAP | 2026-07-22 `QA88-98.3e-ask-user/01..24`：Codex Default 明示 request_user_input 不可用；Plan 生成 `Asked 1 question` disclosure，但 app bridge 当批记录 `No answer provided`，Alpha 走普通 follow-up。AgentRunner shared 真 Gemini 逐项通过 structured single/multi/free-text/two-question/Skip、reload persistence 与普通 composer answer；修复 AskResolved 后 `queued…` 幽灵。仍缺 G48：session list 把 active structured ask 与 ordinary idle 都投为 `waiting:input`，sidebar/command palette 误标 Ready、不能抬 attention |
