...
| TH-08 | ask_user waiting、answer、reload、cancel 边界 | PASS | 2026-07-22 `QA88-98.3e-ask-user/01..36`：Codex Default 明示 request_user_input 不可用；Plan 生成 `Asked 1 question` disclosure，但 app bridge 当批记录 `No answer provided`，Alpha 走普通 follow-up。AgentRunner shared 真 Gemini 逐项通过 structured single/multi/free-text/two-question/Skip、reload persistence 与普通 composer answer；真浏览器反证只靠 AskResolved poll 仍有 receipt race，并确认实际 send receipt 是 `delivered`；以 AskForm 可见 context + 成功 receipt 修到无需 reload 即零 `queued…` 幽灵。2026-07-23 `QA88-98.4o-typed-attention/06` 进一步以 backward-compatible `attention.answers` 把 active ask 全局投为 `Needs answer`，ordinary idle/resolved 保持 Ready，跨 daemon/webui restart 不丢；Environment 同时显示 `Answer requested`，G48 关闭 |
...
| EV-05 | child approval/recovery attention 与去重 | UNTESTED | 2026-07-23 `QA88-98.4o-typed-attention/07/08` 已完成 AgentRunner shared 单 child、真实三 child team、resolve、restart、sidebar/palette 与 scripted duplicate-spawn 去重；尚缺 Codex 同态复杂 child 场景，因此不提前判 PASS |
