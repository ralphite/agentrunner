...
| TH-10 | provider/network/tool error、Retry 原位替换 | UNTESTED | 2026-07-22 `QA88-98.3g-errors/41/43..49/54`：双侧同一 `bash exit 23` 真实 tool error 均由 Worked/disclosure + assistant explanation 收口；我方 detail 保留 stderr/Exit 23。另有 shared provider model-not-found→换模型→Retry→`Failed attempt · retried` 真链路。Codex provider/network Retry 仍无可控同态证据，故不提前判 PASS |
