...
## 2026-07-19 · INC-80 三视角 review（正确性/契约视角）结果与修复

正确性视角（P0×2、P1×1，全修）：
- P0-1 scanDriveSessions merged 分支漏 self_paced——daemon 重启后
  self_paced series 永久失联（无终态+无 sweep 收编）。修：sweep 收编。
- P0-2 scanSessionTimers 无 Series 过滤——series_tick timer 会把
  program-driven 的 series 会话经 agent-resume seam 错形态复活，污染
  journal（违反决策 #21）。修：timer sweep 过滤 + hostResumeFunc 入口
  守卫双防。孪生 TestSweepsRouteSeriesSessionsToDriveSweepOnly。
- P1-1 interval/cron+overlap=skip 下硬 crash mid-iteration：merged 形态
  tick 后置导致 resume 把在飞迭代当漏 slot skip——子工作与 spend 双双
  蒸发（相对 legacy 的真回归）。修：ResumeSeries 检测 in-flight 子库
  （resumeInFlightN），driveSeries 对其消费 slot 而非 skip；孪生
  TestSeriesIntervalCrashMidIterationResumesInFlightChild。
- 记档接受（legacy parity，非本增量引入）：P2-1 self_paced finish 意图
  跨 crash 丢失、P2-2 retry 同预留复用可超卖、P2-3 budget 终态 BestIter
  绕过 pass-first、P2-4 seriesAllChildrenFailed 漏 failed* 前缀、P2-5
  best_of_n 的 MaxIterations 投影误导、P2-6 SessionStarted/SeriesStarted
  间 crash 窗口、P2-7 series --retry 丢 SpecPath、P2-8 重放 Spawns 计数
  膨胀——随真实使用逐项立项。

契约视角（P1×4）：
- P1-1 series 版本 bump 违决策 #18（additive 不该 bump）且 checkVersions
  精确匹配令 v1 窗口（07-18 起）全部会话 resume 硬拒。修：checkVersions
  改"接受旧、拒绝新"（TestSchemaGuardAcceptsOlderSubStateVersion），
  bump 保留（此后 additive 不再 bump）。
- P1-2 SPEC F/B 表与 DESIGN §17 矛盾（opt-in/仍 legacy/🟡 未收敛三处
  陈旧）——下轮文档 sweep 修。
- P1-3 闸门 B 零登记 + QA-66 #4 断言被 2c 推翻——下轮登记场景清单并重裁。
- P1-4 E1④ 定义向下重定义打勾——工作纸补计划变更记录。
- P2 批（usage 串 [--driver]、submit -h 宣传、stale 注释、前端 driver
  词残留、INC-80.3 无台账、SPEC:184 前向引用失效、DESIGN §13 base ref
  单形态表述、kind:"driver" 词汇张力）——下轮 sweep 一并。

## 2026-07-19 · INC-80 收口 sweep（契约 review P1-2/3/4 + P2 批）

SPEC：B 表"子执行收敛为递归 session"🟡→✅（E1 四步全落）；F 表 loop
行去 opt-in/仍-legacy 陈句；I 表 184 前向引用更正；附录命令表补
`submit --drive` 注。DESIGN §13 best-of-N base ref 双形态表述。QA：
QA-66 #4 断言重裁（retry 新建 series 会话）、QA-77 登记闸门 B 五场景。
工作纸补计划变更记录（④ 降级 transport 的对账、步骤 3 残余、
parallel×retry 达成度）。代码文案：init usage 去 [--driver]、
drive/submit --series flag help 与注释更新为"默认+insist"语义、前端
...
## PLAN 5.8 best-of-N 胜者晋升（2026-07-19,G15 消解）

**增量**:`ar promote <best-of-N-sid>` + webui "Apply winner"(session
形态,thin-shell 经 /promote 端点)。机制:workspace shadow repo 内以
临时 index 把胜者 worktree(<sid>/wt/att-N,留盘)写成 tree object,
`diff --binary BaseRef→tree` 出补丁,对 workspace `apply --check` 干跑
通过才落(clean-or-nothing、不 staged、非 git workspace 也可),冲突
原样报错零改动——INC-49 Apply-to-project 教义的会话形态复用。前置
守卫:非 best_of_n/未终局/无胜者/无 BaseRef/胜者树被清各自明白报错。
顺手补齐 webui timeline 的 merged-stream series 事件渲染
(series_started/iteration/ended 镜像 legacy driver chips,bestIter
入 TimelineModel 驱动按钮)。钉子:TestPromoteWinner 三场景 +
timeline.test series 渲染两场景。SPEC F 表/GAPS G15/UJ-16 收口。

## PLAN 5.9 时间旅行减负:处置向量写死 cancel_at_fork（2026-07-19）

**减法**(可裁项,裁决=做):BarrierHandle 的 per-handle policy 向量是
从未长出第二个值的投机泛化——唯一写值 cancel_at_fork、唯一读点只是
跳过未知值。裁除 Policy 字段(旧 journal 带 policy 字段 decode 忽略,
...
## 2026-07-19 · S10(定时与后台)完成:原生 schedule/hook 对 agent 不可发现(HIGH 缺口)

S10 实测:用户"挂 30 分钟定时"→agent 自写 log_monitor.py 起后台
进程模拟(零 schedule 事件、Scheduled 页全程无条目、"暂停"被实现成
kill 不可恢复);"webhook 叫醒"→agent 不知原生 hook(INC-50),反问
用户 CI 细节。webui 本身零缺陷(BACKGROUND WORK/ATTENTION 随进程
生死正确增删,Scheduled 页/对账全对,0 error 0 溢出)。**定性:spec/
工具暴露层缺口**——原生 schedule(INC-74)与 hook(INC-50)未进
...
## 2026-07-19 · QA-0719 S5 goal 状态 pill 溢出修复 + 批次红转绿收尾

- **8f96182 fix**：settled goal 卡片 phase 标签(Stopped/Cancelled/
  Satisfied)溢出压住 goal 文案。根因:`.goal-outcome` 一直和整行元素
  共用 tw.css:644 的重量级 row 规则(min-w-0 + flex shrink:1),作为
  goal copy 旁的 flex 子项被压到近零宽,单词标签不换行不裁剪→溢出
  重叠。改为遵循 `.pill`/`.readonly-tag` 约定的紧凑 pill(inline-flex
  shrink-0 whitespace-nowrap rounded-full),永不收缩。手机端实测锚点。
- **批次红转绿核验**(env #23,build 1aecafc):sa-name GREEN(7 个
  "worker" 全名 44px、Completed/Cancelled 全可读)、goal terminal rail
  GREEN(settled "Stopped" 卡无 Edit/Pause/Cancel)、utext GREEN
  (whiteSpace:pre-wrap+overflowWrap:anywhere)。
- **S9 drawer 澄清**:先前 drawer 验证卡住是选错目标会话——选了非-repo
  的 scratch session("Go 1.26 迁移"),EnvironmentSection 在
  `!isRepo` 时 return null(SupervisionPanel.tsx:739)是正确行为,rail
  正常打开只显示"Nothing needs you"+Run details。grid min-w-0 修复
  (7490fb7)只作用于真实 repo 会话的 `.env-rows > *`,纯防御性 CSS,
  判定 logic-verified。

## 2026-07-19 · QA-0719 S9 定性 + QA-env 方法论边界

- **S9 worktree 抽屉"不渲染"= env 数据恢复假象**（driver #23 n=5 硬证）:
  worktree 会话 diff 返回 `known:true, isRepo:false, nested:false,
  worktree:false, 空 diff/untracked`。根因:remote-qa-env 从 cache 只
  恢复 store（journal），未恢复 worktree 磁盘检出(`~/.local/share/
  agentrunner/worktrees/...`,在原 phone-webui runner 盘上)。目录不在
  → `git rev-parse --show-toplevel` insideRepo=false(meta.go:495) →
  isRepo 保持 false → EnvironmentSection 早退 null(SupervisionPanel
  :739)。非产品 bug;grid min-w-0 修复(7490fb7)保持 logic-verified,
  真机 phone-webui 上 worktree 真实存在会正常渲染。
- **QA-env 方法论边界**:恢复出来的 env 只有 journal、无 workspace
  检出,故只能验 journal 投影面(timeline/goal/subagents/attention/
  run details);**workspace/diff/changes/worktree 面必须在活 env 里
  新建会话跑真实 turn 改文件才能验**——下一轮 QA 焦点。

## 2026-07-19 · QA-0719 真实-turn workspace change 面测试 → CJK last-turn diff 修复

- **方法论突破**：在活 remote-qa-env(#24)里对 scratch 会话 110404
  发真实 Gemini turn 创建 CJK 文件名文件 `图表说明.md`,正面测
  workspace change 语义面(恢复态 env 测不到的盲区)。
- **ghost-diff 锚 PASS**：全新会话首条纯问答回复下无 `Edited N files`
  幽灵(t4)。
- **发现并修复 f783ac0**：**in-thread ChangesOutcome(最近一轮 review)
  卡把 CJK 文件名 octal-escape** —— `Edited 1 file / diff --git
  "a/\345\233\276\350\241\250\350\257\264\346\230\216.md"`,而同屏
  Environment rail(working-tree)正常显示 `5 files +160 ·2 new`。根因:
  in-thread 卡走 `ar diff --scope last-turn`,其 diff 由 shadow 快照
  后端 `ShadowRepo.gitWithEnv` 产出,该 chokepoint 缺
  `core.quotePath=false`。07fa01e 只修了 webui git() 的 working-tree
  路径,漏了 shadow 这条。在 gitWithEnv 统一 pin,一并修 diff+numstat。
  钉 `TestShadowRepoDiffKeepsUnicodePathsUnescaped`。
- **教训**：CJK 转义有**两条独立 git 路径**(webui git working-tree /
  CLI shadow last-turn),修一条不等于修全。全非-ASCII 路径面都要过。

## 2026-07-19 · QA-0719 真实-turn 续测：Undo 与 Queue 验证为正确(无 bug)

- **Undo 语义(真机 t14/t15)**：ChangesOutcome 卡"Edited 1 file"的 Undo
  实为**工作区级 revert 到 HEAD**(丢弃全部 7 个未提交文件,含本轮前的
  6 个)。**by-design**——ChangesOutcome.tsx:400 注释已明言"卡显示
  last-turn,revert 吞整个 working tree,确认弹窗必须报真实范围",
  `beyond` 变量正是那句"including N files changed before this turn.
  This can't be undone."。modal 完整披露+Cancel,安全。无 bug。测完
  Cancel,110404 的 5 tracked+2 untracked 全保留。
- **Queue UI+投递(真机 t16/t17)**：运行中发第二条消息→`bubble.pending`
  干净渲染+底部 "queued..." kicker,composer 切 Queue/Steer/Stop,无溢出
  无 console error;turn 1 完成后排队消息**自动投递为 turn 2 并独立作答**
  (cherryDelivered:true, stillPending:false)。闭环用户最初排队消息关注点。
- 方法：均在活 env #24 对 scratch 110404 发真实 Gemini turn。证据先行
  (读代码/看截图)两次避免误报(Undo 非 bug、dark-mode 非 bug)。

## 2026-07-19 · QA 截图基建：headless Chromium 装 CJK 字体 + access 链接更新

- **4925765 fix(ci)**：用户要看每个 task 的无乱码截图时暴露——QA driver
  的 headless Chromium 无中文字体,所有截图里 CJK(侧栏标题/文件名/
  消息正文)渲染成豆腐块 ▢,中文产品的 QA 截图无法目测。给
  remote-qa-env(直接 apt)与 setup-ar(qa-blackbox 复用)都装
  `fonts-noto-cjk`。装后实测:侧栏"智能家居系统需求梳理与冲突分析"、
  文件名"图表说明.md"、消息正文全部可读。
- **双修确认**：图表说明.md 在修复版构建上既无 octal(f783ac0)也无
  tofu(4925765)——两个乱码源都清。
- **交付**:S1–S12 共 16 张任务整页截图(fullPage,含全部对话信息)发
  用户目测。
- **access 链接**已随新 env 更新到 qa/remote-env-url.txt。

## 2026-07-19 · QA:091500 短链接三人协作会话全量深审(用户指定)

- **933985c fix**:sa 行 status 截断根修二刀。81177ce 不完整——identity
  组 flex-1(basis 0)使 trailing 先按 auto basis 拿满,缺口压给唯一可缩
  的 status("Completed"→"Co…",真机 statusW 38px+scrollWidth>clientWidth
  ×7 实测;先前 GREEN 判定误读)。根修:identity flex-none+spacer 吸收
  +meta 列承担收缩+status shrink-0。
- **CRITICAL 待审计(登记)**:同一 workspace(ws-091500)17:10 rail 快照
  显示 `72 files +28723 · 428 new`(自洽:untracked pytest 生态包内联
  synthetic diff),17:32 同 API 返回全 0;窗口内无用户操作。且
  test_main.py 曾真实运行(timeline 09:47 pytest "6 passed")现从磁盘
  缺失(files?q=test 空),_pytest/ 等包目录仍在。嫌疑:daemon 恢复/
  快照回滚 或 env cache 快照边界。需专项 reproduce。
- **架构矛盾(登记)**:spawn 的 workspace note 明言 child 在 ISOLATED
  snapshot("changes do not flow back automatically"),而
  SupervisionPanel.tsx:744 注释+逻辑按 "sub-agent shares parent's
  workspace" 隐藏 commit 行——runtime 契约与 UI 假设相悖,必有一处
  过时,对照 DESIGN 裁决。
- **待核(登记)**:timeline 首条 "from worker" 消息("工作区是空的请
  提供需求")与 call_5_0 child 视图内容(17s 完成完整设计)不对应——
  疑另一 child 的消息或消息路由展示错位。
- 子会话视图(sub-call_5_0-a1)渲染正常:Read-only sub-agent 标头、
  child prompt/回复/PROGRESS 完整;"Commit or push" 在树干净时正确
  disabled;PROGRESS "Implementation" done:true 而 call_22 child
  canceled——PROGRESS 与子终态无对账(并入 INC-66/S4 家族)。

## 2026-07-19 · 091500 "workspace 静默改写" CRITICAL 撤销定性 + 溯源标注问题登记

- **撤销**:上一条登记的 "workspace 无人操作被静默改写" 非产品 bug。
  journal 铁证(seq 881):17:30:19 一条 `input_received{text:"Create
  and apply git ignore", source:cli, principal:local-user}` 进入会话,
  触发 turn 75-81 真 Gemini 生成并写入 .gitignore(列全 pip --target
  装的包目录)→ 428 untracked 被盖住。时间线:17:15 access 链接交付
  用户 → 17:30 用户经隧道 webui 亲自发送 → 17:32 我读到 0。17:10 的
  `72 files +28723 · 428 new` 是操作前的真实状态。schedEvents 空、
  Scheduled 面板无关联注册,排除 cron/hook。
- **新登记(真实问题)**:webui 发送的消息在 journal 里标
  `source:"cli"`——arwebui 的 send 走 runAR CLI,溯源层无法区分
  webui/CLI 真实来源,审计误导(本次排查被带偏半小时)。建议 send
  链路透传 origin(webui/cli/schedule)。
- **#3 架构矛盾修正(注释级)**:SupervisionPanel 曾断言 sub 一律
  shares parent workspace;spawn.go 真相是 spec 决定 isolated|shared
  (builtin worker=isolated,isolationNotice 为证)。禁 commit 行为对
  两种模式都对,错的是理由——注释改为双模式准确表述,行为零改动。
- 待核残留:test_main.py 仍缺失(timeline 09:47 pytest 6 passed 为
  证曾存在)——可能被 17:30 turn 整理掉,下轮问 agent 或查 turn 75-81
  activity payload。

## 2026-07-19 · staged 变更对 Changes 面全盲——核心语义 bug 修复

- **误判更正**:上条 "test_main.py 缺失" 不成立——turn 79(用户 17:30
  触发)执行了 `git add .gitignore api_design.md main.py test_main.py`,
  文件被 staged 而非删除;我 files?q=test 读空是 API cap 50 + 客户端
  过滤 _pytest/ 的叠加假象(第三次误判,均已 LOG 留痕)。
- **由此暴露真 bug 并修复**:working-tree diff 用裸 `git diff`(仅
  unstaged)+untracked 只认 porcelain `?? `——staged 文件(`A `/`M `)
  从整个 Changes 面蒸发:rail 显示空、Commit or push 灰死
  ("Nothing to commit"),而 `git commit` 此刻会提交 4 个文件。091500
  用户真机操作实锤,且恰逢 unborn HEAD(scratch 首 commit 前)。
- **修**:有 HEAD → `git diff HEAD`(一次覆盖 staged+unstaged);
  unborn HEAD → `git diff --cached`(index vs 空树)joinDiffText 拼
  unstaged。numstat 同构。untracked 逻辑不变。钉
  `TestHandleDiffSurfacesStagedChanges`(unborn+committed 双形)。
  副作用即收益:staged-only 时 Commit 按钮恢复可用。

## 2026-07-19 · staged-diff + sa-status 双修复真机红转绿(env #27, build 7937ae3)

- **staged-diff(7937ae3) GREEN**:091500(unborn HEAD+4 文件 staged 活
  标本)rail 从全空灰死 → `Changes 4 files +577`,diff heads 含全部
  4 个 staged 文件(test_main.py 在列,"消失"彻底证伪),Commit or push
  恢复可用(commitDisabled:false)。时间线 git status "Changes to be
  committed" 与 UI 一致——UI/git 事实分裂修复。
- **sa-status(933985c) GREEN**:7 行 clipped:false,截图全字
  "Completed"/"Cancelled"+steps/tok/open 完整(对比修复前 "Co…")。
- INC-83(send origin 透传)提案已立(f9e8a8f),待裁决。

## 2026-07-19 · 移动端验证轮(env #27, 390×844)

- **双修复移动端 GREEN**:sa-status 7 行 clipped:false(<520px grid
  分支正常);staged Changes rail `4 files +577` 完整;overflow 0。
- **新发现并修**:sa-row 是 button(fit-content),移动 rail 上卡片
  参差不齐(~2/3 宽、各行不等),与全宽的 Changes/PROGRESS 行不一致
  ——桌面内容宽近似掩盖了它。加 w-full(桌面 spacer 吸收,视觉不变)。

## 2026-07-19 · QA 流程沉淀为可复用资产(用户指令)

- 新增 **`.claude/skills/qa-remote-loop/SKILL.md`**:本 session 全部
  实战纪律的单一沉淀——GitHub-transport driver 协议与传输坑、驱动
  纪律 8 条(check-then-click/截图判色+reload/scrollWidth 判截断/
  恢复态先验底座/真 turn 测 workspace 面/保留数据…)、找问题 oracle
  (三方对账 UI↔API↔journal、journal ts 定案、真 turn 必测面、双主题
  双视口覆盖)、修复闭环 7 步(根因→回归测试→push→phone 重部署→
  新 env 同锚会话红转绿→LOG 留痕含误判)、升级边界(不变量→增量提案,
  模型行为→登记)。一天 6 修 3 证伪的流程全量可复放。
- 新增 `qa/driver/dispatch-env.sh`(起 env+等 ready)与
  `qa/driver/shot.sh`(下载截图)——确定性管道;探索决策仍由 agent
  亲自驱动(伪探索 MJS 禁令不变,已写入 CLAUDE.md 指针)。

## 2026-07-19 · dispatch-env.sh 并发护栏(skill 首次实战即出改进)

- 险情:本 session 21:37 dispatch 差点(POST 恰好失败才没有)cancel 掉
  并发 session 正在活跃驱动的 env(run 29703154416,driver issue #30
  已 57 条评论);此前 20:39 的 #29 就是这样被对向 cancel 的。
- 护栏:dispatch-env.sh 先查 in_progress run,有则 REFUSED(FORCE=1
  显式抢占);SKILL.md 传输坑补"dispatch 前查并发、不往别人 driver
  issue 插指令(n 撞号)"。
- 移动端 DiffView/artifacts 扫尾推迟至并发 env 空闲,不硬抢。

## 2026-07-19 · diff view 全线不可用(用户手机报障)——滚动几何三连修

- **报障**:用户手机(phone-webui,390×844)打开 Changes:文件卡上下
  压成一叠几像素"横条",代码区不可竖向滚动,review 完全不可用。
- **根因 1(主凶,Tailwind 迁移即有)**:`.diffwrap` 是 flex 列但没有
  `overflow-y`,外层 `.changes-panel.session-side` 是 overflow-hidden
  定高框;`.filediff` 自带 overflow-hidden ⇒ flex 自动最小尺寸归零
  ——装不下时浏览器不给滚动条而是把全部文件卡按 flex-shrink 压扁
  (本地 Chromium 双视口量化:190px 内容 → 15px/8px,7711px → 628px,
  canScroll:false;**桌面同病**,此前仅因 diff 不够长未暴露)。
  `.diffbar` 的 sticky top-0 本就假定 .diffwrap 是滚动容器。
  修:`.diffwrap` + overflow-y-auto,`.filediff` + shrink-0。
- **根因 2(顺藤摸出)**:Wrap long lines 开关无效——`.dl { min-w-max }`
  的 max-content 不看软换行机会,行仍按完整行宽撑开照旧横向溢出。
  修:`.diff-wrap` 下行容器 min-w-0,split 网格回容器宽。
- **根因 3(split 视图)**:每行各开一个 min-w-max grid,列宽按本行
  内容独立解析(短行半列 344px、长行 946px),右列起始 x 逐行漂移,
  长行代码被推出视区,看起来像"隔行消失"。修:`.fd-split` 成为唯一
  grid(max-content 宽 + min-width:100%),行盒 `display:contents`,
  五格直落共享列轨;`.dl-hunk-span` grid-column 1/-1。
- **钉死**:`diffScroll.test.js` 四条 CSS 源文契约(iosViewport 同法)
  ——滚动容器、卡片拒缩、wrap 释放 min-w-max、split 共享列轨。
- **验证**:本地 Chromium 红转绿(远程 env 槽位被并发 session 占用,
  scripted provider 造大 diff 会话):phone scrollH 818→9252、
  压扁卡 13→0、wrap 溢出→0、split 两半列 344/946→统一;双视口
  (390×844、1280×800/1920×1080)+ 双主题,console 0 错误;
  vitest 62 文件 598 测试全绿。证据 `qa/runs/2026-07-19-diffview/`。
  远程真机红转绿待 env 空闲后补(并发 session 正在驱动 #30)。
- **过程误判留痕**:split 修复首轮"验证无效"是假象——第 4 轮 arwebui
  bind 失败(8788 被上一轮进程占用),量的是旧二进制;`curl` 探活只证
  "有人在听",不证"是刚起的那个"。后续起服务先 pkill 再 bind,
  或校验 served asset hash。

## 2026-07-20 · v2sim 探索轮:goal 耗尽假终态死局(用户 iPhone 实测复现→双修+提案)

- **入口**:用户手机点开 v2sim anchor 会话(L1/L4,`…cd1a751d`),
  见 "Budget limit reached" 终态卡:正文一词一行、标题与 meta 重叠,
  点 "Continue in new session" 后新会话原样复制同一死局——"没有任何
  办法 continue"。
- **三方对账定性**:list 状态 `goal_budget_exhausted`(Quiescence
  ivan-08 有意持久化)vs `/state` `waiting:input`(会话完全可聊)vs
  fork 子会话 `running`(继承 exhausted goal 恢复 verify 循环烧
  token)。UI 把 goal 的 check 预算耗尽子串匹配成会话 token 预算终态。
- **修 1(语义,`0c359d9`)**:`terminalNoticeFor` 对 `goal_*` 返回
  null(GoalBanner stopped 相接手:label+checks+dismiss);
  `friendlyStatus` 改标 "Goal stopped — check budget"/"Goal
  completed"。真会话级 budget(limit_exceeded 等)措辞不变。
- **修 2(布局,同 commit)**:非 resume 终态卡由旧 flex 行统一为
  resume 变体的响应式 grid——实测 390px 下正文列被 intrinsic 宽按钮
  +meta 压到 **4px**(一词一行成因);sm 以下按钮整行。
- **钉死**:pill.test 新增 goal_* 判空+标签断言(旧码必红);
  chrome test 锚定 grid class。62 文件 600 测试绿。
- **登记不修(INC-84 提案)**:verifier 判据不校验(L4-I1)、pause
  不跨重启(00:13 verify re-fire 铁证)、fork 全量继承病态 goal、
  goal 类 one-shot HTTP 挂死(两次独立复现:approve goal-verify 后
  与 goal cancel 后,60s ctx 超时未兑现,直接卡死无超时客户端)。
- **过程误判留痕**:L4 曾把 daemon 对 goal-pause 的固定回执文案
  ("a no-op unless a goal is attached")读成 pause 失败;实为回执
  不反映真实结果——已在 INC-84 里改为"回执必须可观测"要求。
- **QA 通道坑新增**:GitHub WAF 对含 `/goal`+cancel 字面量的 issue
  comment 返回 503,拆词(`'/go'+'al'`)绕过;executor 因 n5 的
  无超时 fetch 再次卡死——驱动侧 fetch 必须一律 Promise.race 包超时
  (PLAYBOOK 已列硬规则,本轮违例一次即中招)。
- **红转绿**:含修复的 env(run 见 qa/remote-env-url.txt)对同一
  anchor 会话断言:无 terminal-alert、gbar "Goal stopped" 出现、
  composer 可用、390px 正文列恢复全宽。(结果见
  docs/audit-2026-07-19-v2sim/ANALYSIS.md 附录。)

## 2026-07-20 · diff view 修复远程收尾:槽位接管记录

- 远程红转绿被三重外因拖延:并发 session 连占三个 env(#30/#31/#32,
  活跃驱动不抢占)、GitHub issues API 503 故障(00:00-01:48Z,REST 与
  MCP 双通道同瘫)、本 session 的 GITHUB_TOKEN 无 workflow dispatch
  权限(403,改走 GitHub MCP actions_run_trigger 成功)。
- #32 自 01:46Z 起零指令超 70 分钟(沿用本 session 对 #31 定的空闲
  阈值),02:56Z 判定并发 session 收工,接管槽位起 agentrunner-store-
  env,在用户真实 phone 会话上补断言。phone-webui 已于 23:13Z 起在
  含修复的 main 上服务(cf120c5)。
- **远程真机红转绿 GREEN**(env run 29713367643 / issue #33,借 phone
  store,390×844):①用户报障截图的同一会话(20260719-204419,
  +1853 −0)——diffwrap {overflowY:auto, scrollH:36501, canScroll:true},
  23 卡 0 压扁 0 溢出 0 console 错误,顶/底截图归档;②store 最重会话
  (working-tree +26209、500 卡、1MB diff+389 untracked)——
  {scrollH:114859, canScroll:true, squashedOpen:0}。证据补录
  qa/runs/2026-07-19-diffview/。任务闭环。

## 2026-07-20 · L2-I1 根修:收尾竞态吞消息(daemon rehost-on-dangling-input)

- **定性修正**:L2-I1 原判"steer 竞态被静默丢弃",探索轮深挖后改判
  ——与 delivery 模式无关,是 **accept-vs-consume 收尾竞态**:输入在
  宿主 loop 退场窗口被 accept('delivered' 已回、命令日志已持久),
  泵进再无读者的 inbox;唯一捞回是下次 daemon boot 的
  pending-command sweep,期间消息静默不起 turn(L2 的 C 会话即卡在
  waiting_entered 后 8 秒的窗口里)。用户裁决:悬空消息就该是一个
  普通的新 turn,不需要新语义。
- **修(`65f8131`)**:四个 host 注销点 defer 里加
  rehostIfPendingInput——命令日志仍有未消费 CommandInput 即重新
  host(复用 boot sweep 机制,补触发时机)。护栏:仅 CommandInput
  触发、resume 失败过不重试风暴、marked 会话按输入 user-class 走
  决策 #30。
- **钉死**:internal/daemon/teardowninput_test.go 三条;主测撤修必红
  (resume calls 1→2)。daemon 包绿;check.sh 首跑 RED 系本地缺
  bwrap(装后 internal/tool 绿),复跑全量闸确认中。
- **远程红转绿说明**:该竞态窗口 ~秒级、远程按需复现不可靠,回归锚
  以确定性 Go 测试为准;下一轮 env 周期在真机上抽验(send-at-idle
  风暴式复测)。

## 2026-07-19 · FEATURES.md v1.2:修复队列收口后全面对账

用户要求更新功能清单。v1.2 把 PLAN 4.1–5.9 + Phase 0–3 的全部结果落
入盘点:close/compact 语义(INC-82)、动词两概念、title/promote 两个新
CLI 命令(计数 40→42)、sessions cadence 投影、drive 降级 transport、
...
## queue 措辞修正（2026-07-19,用户复核）

用户复核 queue 语义:实现无误(TestQueueDefersToTurnEnd 钉死——忙时
queue 消息 seq 必 ≥ 当前 turn 末次 generation、拿自己后续 turn),但
FEATURES §1.2 与 SPEC A 表把"安全边界按序消费"当成 queue 语义、与
steer 混淆。改:queue=turn 跑完进 idle 时一次性按序消费(全部进同一
next turn),"安全边界即时消费"明确归 steer。docs-only。

## doc↔code 一致性审计（2026-07-21,workflow w0gcqg5ny）

用户要求系统性对比全部功能与文档、找出所有不一致。方法:14 域各一
auditor 读 doc 断言+实现代码报不一致,每条再对抗性复核(驳倒诚实
hedge/误读)。12 raw findings → 11 CONFIRMED / 1 REFUTED(context:fork
skill 漏提 agents_dynamic 前置,正确驳回)/6 域干净。全部修复:

**代码修正(3)**:(1) grep 省略 max_results 实际返回 200 而非契约承诺
的 100——exec.go 补 grepDefaultMatches=100 默认(cap 仍 200),grep.json
契约自此为真,补钉子 TestGrepDefaultLimitIs100NotCap;(2) conversation.go
删 INC-83 遗留的 killCmd 孤儿注释(函数早删、注释悬在 interruptCmd 前);
(3) daemon.go unknown-command 错误文案去掉已删的 kill、补 PLAN 5.6 新增
的 title。

**文档修正(8)**:FEATURES §2.2 `ar kill` 划线(INC-83 已删,handle 取消
=模型工具);§2.3 help 措辞(仅位置参数类命令转发专用 blurb,flag-parsed
回退全局);§6 自动路径标记门(只 kill gate 自动复活,close/stopped 投影
idle);§7 diff(仅 last-turn 基于 barrier 快照,working-tree=webui 原生
...
**INC-86.3 后端根治(2026-07-21,应用户"贯彻到后端"指示,关闭 G41)**:前端
(INC-86)只绕开 webui;CLI/手写 thinking-off spec 仍 400。改 gemini `toConfig`
统一逻辑——**永不发 `thinkingBudget:0`**:任何未显式开 thinking 的请求也拿正的、
clamped budget(默认 `geminiDefaultThinkingBudget` 经 resolveThinkingBudget 收进
answer room,"会话死亡"防御不变),thought 摘要仅 Thinking.Enabled 露出;cap 太小
交给模型 floor(nil budget)而非非法 0;删 pro/非-pro 的 budget:0 分支。**不触
不变量**("answer 恒保 room"仍由 resolveThinkingBudget 保证)。验证:go test 全绿
(更新 TestToConfigDisablesDefaultThinking/EnabledTinyCap 两处旧 budget:0 断言、
移除 live 作废探针)+ golangci-lint 0 issues + 真机 thinking-off spec `run
completed`(20260721-170537);部署 b5a00898 到 8809。至此 webui + CLI + 所有 spec
的 thinking-off 路径全部消除,"默认 medium、无 no-thinking"在前后端一致成立。

---

## 2026-07-21 · INC-87 Sidebar project controls + resizable/foldable rail

**需求**：UJ-24 的 project 层补齐 Codex desktop 同类管理面：project hover/focus
摘要与快捷入口；`…` 菜单含 Pin project、Reveal in Finder、Create permanent
worktree、Rename project、Archive chats、Remove；desktop sidebar 可调宽；Pinned /
Projects 可整体收展。

**产品裁决**：project pin 只在 Projects 内稳定置顶，不复制 chats 到 session
Pinned；Remove 只写 `removed` overlay、隐藏 rail projection，永不删除 session /
journal / workspace / files，并提供 Show removed + Restore。permanent worktree
复用既有 `/api/worktree` 校验；width（220–480）与 section folds 是
browser-local preference，mobile 仍为 fixed drawer。UI/UX review 让高风险 Remove
收敛为可恢复显示偏好，未触 DESIGN 不变量。

**实现**：`webui-meta.json` project overlay 增 `pinned` / `removed` optional
fields 与 partial update；project row/menu/preview/排序/恢复落地并共用
`renderProjectActions`；sidebar separator 支持 pointer + keyboard + reset，Pinned /
Projects caret 支持独立持久折叠。A 闸新增 Go overlay/HTTP tests 与
`Sidebar.nav.test.tsx` project/fold/resize cases。为恢复 main 在本机 Bash 3 的
全树门，check runner 从 Bash 4 associative array 改为 indexed array，并删除
stale `parsePSTable` deadcode baseline；均为门禁兼容修正，无产品语义 delta。

**验收**：前端 vitest **653/653**、build、webui Go tests、`./scripts/check.sh`
全绿。QA-78 在真实 `http://127.0.0.1:8809/` + 共享
`~/.local/share/agentrunner/` 验证 width/fold/pin 跨刷新、390px mobile、六项菜单、
console error/warn=0；从 `agentrunner dev2` 创建并保留
...
## 2026-07-21 · INC-88 低价值 UI surface 收敛（1–12）

**需求/裁决**：用户要求移除 Copy session link、raw ID、重复 Stop/
Continue/Fork、无动作菜单分组、前置条件不成立的 composer/Environment
控件、重复 Settings 出口与过密 sidebar 行动作。deep-link router、
composer Stop、RunView Stop、Advanced Continue、offline Restart 与内容型 Copy
保留；只收敛 projection，不删底层 contract 或数据。UI/UX review 复用现有
Menu、progressive disclosure 和 responsive breakpoint，不引入新视觉概念。

**实现**：Timeline/session/Scheduled/sidebar 普通用户面移除 link/raw ID与
重复操作；session row 统一为单一 `…` 菜单；connected daemon 改静态
status；View/Run group 按真实动作渲染；projectless composer 只留 picker；
clean/sub-agent Environment 不画空/disabled 任务；Settings desktop/mobile 分别
只留 Done/Back。新增/更新 9 个 component test 簇覆盖 12 项与保留的唯一入口。

**验收**：frontend targeted **138/138**、全量 vitest **658/658**、build、`./scripts/check.sh`
全绿。QA-80 用当前 worktree 前端 `:5188` 连真实 `:8809` + shared
store，验证 projectless、completed deep-link reload、Scheduled、clean Environment、
desktop/390px Settings/sidebar，console 0 warning/error；证据
`qa/runs/2026-07-21-QA80-low-value-ui-cleanup/`。无 running session 故未为截图
新起模型任务，该态与 dirty/sub-agent 分支由 component tests 锚定。
...
## 2026-07-22 · INC-91 从任一消息 Continue 到独立 session

**需求/裁决**：timeline 中每条有 durable message anchor 的 human input 与每轮最终
assistant answer 可直接 Continue。human cut 停在输入前并恢复完整 multimodal draft；assistant
cut 停在回答后并等待下一条输入。legacy/non-final/tool/control row 不伪造入口；Advanced fork 保留。

**实现**：runtime 在 human intake 前与 eligible assistant final 后写 message-scoped barrier；
`messagefork` 以 parent/item/stable request id authoritative resolve，staged atomic publish dormant
child。draft 使用 ref-only CAS whitelist，保留 text/image/file/name/part id，attachment-only 可发；
首次 Send 通过 durable CommandLog claim 保证 single winner，retry 幂等，veto/revoke 释放。Web API、
Timeline action、Composer reload recovery、navigation/focus/error 状态同步落地。

**验收**：Go barrier/fork/inbox/conversation/Web API tests、frontend timeline/composer tests与
`./scripts/check.sh` 全绿。QA-82 在真实 `:8809` + shared store 验证 user/assistant 两种 cut、
multimodal draft reload、stable-request retry、attachment-only 真 Gemini、Back 与 console 0；探索中
发现并修复 timeline before-anchor 单 pass 漏 action及空 text provider part。所有 QA session、
workspace、journal 与证据保留在 `qa/runs/2026-07-22-QA82-message-level-continue/`。

**实现后独立 review 收口**：同一只读 reviewer 首轮 No-Go 的 4 个 P1（opening crash recovery、
ordered multimodal metadata、sub journal exact cut/staging fsync、opening hook veto 空跑）全部修复并
逐项复核 CLOSED。最后一个 duplicate same-kind/same-ref 身份缺口以 source attachment ordinal
闭合，服务端按 `ordinal + kind + ref` authoritative 授权且拒绝重复 ordinal；最终 verdict **Go，
无 blocker**。

**QA-82 post-review re-run**：部署 `c5983a62-dirty-015737` 到真实 `:8809` + shared store，浏览器
从 user message 创建并原样发送 child
...
## 2026-07-22 · INC-98.1 Codex UI 持续实证 loop 基座

**立项**：用户要求把 Codex 对标从一次性 screenshot review 改成长时间循环，覆盖全部
页面、状态、功能点；可直接修的 UI 差异持续修，需要 backend/产品语义的能力持续登记。
新增进行中的 INC-98 工作纸、QA-88 与 `CODEX-PARITY §7` executable matrix（79 个
surface/state 横切行），状态严格区分 `UNTESTED/BLOCKED/GAP/PASS/INTENTIONAL`；新增
G42 全表面实证覆盖债与 G43 Plugins 包生命周期缺口，Pull Requests 复用 G13，Sites
裁为 journey 非目标。

**真实 Codex 交互基建**：`qa/capture-codex-ui.sh` 从可能阻塞的 System Events 改为
`lsappinfo + CGWindow + CGEvent`；按最大 on-screen layer-0 window 取窗，支持 New chat /
Pull Requests / Sites / Scheduled / Plugins 白名单导航、settle 与 Cmd+K query restore，
未知 target fail-closed。新增平台无关 contract test 并接 `check.sh`，钉参数/target/
恢复 trap 且禁止回退 System Events。

**98.1 真实证据**：真实 Codex current/palette + 五个 sidebar 主入口逐张验图；Pull
Requests 首张仍是 skeleton，明确拒收并在 8s 后重捕。AgentRunner production `:8809`
+ shared store 以相同逻辑 1952×1465/light 捕获 New session 与 Scheduled，再合成两张
side-by-side comparison；两处稳定 shell 无 P0/P1 visual drift，细状态仍保持 UNTESTED。
browser warning/error=`[]`，health `ok/daemonUp/versionMatch=true`；Codex 每次恢复原 thread，
AgentRunner theme 恢复原 dark 且 viewport override reset。无 Send/Approve/Deny/Archive/
Remove/Commit/Push，无新建/关闭/删除/清理 session/workspace/journal。完整证据保留于
`qa/runs/2026-07-22-QA88-codex-ui-continuous-loop/`。
...
## 2026-07-22 · INC-98.3d Stop / partial output / recovery 终态纠偏

**双侧实证**：Codex 真 shell 循环运行 44s 后点 Stop，thread 保留 `PARTIAL-1..4` 并显示
`You stopped after 44s`。AgentRunner shared `QA5 queue` session 的 12 段真 shell 循环在
第 4 段后点唯一 Stop；journal 按序落 `activity_cancelled`（exit=-1、partial stdout）、
interrupt、final barrier、waiting，且无 `AR2-SHOULD-NOT-PRINT`，证明进程组确已取消。

**发现与修复**：API/CLI 已精确给出 durable status=`interrupted`，thread 也已有
`Stopped — you interrupted this turn`，但 `pill.ts` 的宽泛 `includes("interrupt")` 又把它
投成 stranded，叠加 `Session needs recovery / Resume`，把用户主动动作误报为 crash。
现于 crash 判别之后精确匹配裸 `interrupted`：映射中性 `Stopped/closed`、不生成 recovery
notice；topbar Retry 与普通 composer 保持。`interrupted_by_crash` 等复合原因仍走异常恢复。

**恢复与裁决**：同一 stopped session 不点 Resume，直接发送无工具 follow-up 成功得到
`TH07-RECOVERED` 并回到 waiting。TH-07 升 PASS；矩阵为 `PASS 19 / GAP 8 /
INTENTIONAL 4 / BLOCKED 1 / UNTESTED 47`。targeted 25 tests、frontend 全量、production build、
`./scripts/check.sh` 与 clean deployment shared-browser 复拍均通过；证据根：
`qa/runs/2026-07-22-QA88-98.3d-stop-recovery/`。所有测试数据保留；INC-98/G42/QA-88 继续开放。

---

## 2026-07-22 · INC-98.3e ask_user 全形态与 compatibility ghost 修复

**Codex 实窗**：Default thread 明示 `request_user_input` 不可用；新增 driver 以真实 Plan
composer 发起后，thread 出现 `Asked 1 question` disclosure，展开为 `Select a test option /
No answer provided`；Alpha 只能作为普通 follow-up 继续。本批据实记录 app bridge 行为，不把
Default failure 或折叠卡冒充完整交互表单。driver 同时新增 current-thread disclosure 与
Plan send，OCR 前后 fail-closed；EXIT cleanup 先卸 trap，失败不再递归卡死。

**AgentRunner 真链路**：6 个 shared Gemini session 覆盖单选 Alpha、multi Alpha+Beta、
free-text Gamma、双问 Red+Large、Skip、普通 composer Beta，全部经 AskResolved 同 session
继续并给精确结果；structured form 跨 reload 完整恢复。所有 session/workspace/journal 保留。

**发现与修复**：兼容 composer 的答案不会落 `InputReceived`，而是直接配对成
`AskResolved{answered,answer}`。前端只以 InputReceived 消费 optimistic bubble，导致 agent
已回复 `COMPAT=Beta` 后仍永久显示 `Beta / queued…`，reload 才消失。poll reconciliation
现同时匹配 `AskResolved.answer`，新增 SessionView 回归。另 sessions list 只投统一
`waiting:input`，sidebar 把 active structured ask 标成 Ready、无 attention；前端不能把普通
idle 一并误抬，新增 backend gap G48。

**裁决与验证**：TH-08 记 GAP；矩阵为 `PASS 19 / GAP 9 / INTENTIONAL 4 / BLOCKED 1 /
...
## 2026-07-22 · INC-98.3g approval truth：G39 闭环与 child worktree 校正

Codex Ask access 的 Allow once/Deny 与 AgentRunner root approve/deny/details/reload 均跑真实
交互；shared Gemini child approval 再以 fresh deep link 跨 daemon/webui restart 恢复，Approve
once 精确唤醒 child 并把 isolated worktree 的 `pwd` 结果回灌 parent，G39 闸门 B 关闭。

同状态合并图发现三处前端 drift：Subagents 把 typed approval wait 错标 Ready、child approval
卡拿 parent workspace 冒充执行位置、1280px Environment 浮卡遮住 primary decision。修为
typed wait 优先、复用 inspect `delegations.workspace` 的 path/mode 明示 `Child worktree`，并在
900–1400px approval 时暂收 Environment（attention 清除后恢复；>1400px 仍可强化）。26 个
targeted tests、frontend build、`./scripts/check.sh`、shared production cache-bust/restart、
...
## 2026-07-22 · INC-98.3l Markdown media 与 Gemini thought 边界

双侧真实回复同一 TH-03 media fixture：standalone external image 与 link-wrapped image 均保留
精确 src/alt，第二张 nearest link 为 `https://github.com/`。Codex complete `25` 与 AgentRunner
首次合并图 `27` 显示我方 inline image 仍受旧 420px cap 撑大正文；保留 lightbox 放大能力，只把
inline `.md-img` 收敛到 Codex 同态的 200px，修后 screenshot `28`、合并图 `29` 通过。

真 Gemini 修前 session 的 durable assistant message 还把 provider `Thought:true` summary 永久
放在回答前；在 Gemini adapter normalization 层丢弃 thought part 后，新 shared session/reload
只剩请求的 heading/images。普通 text、tool-call opaque `ThoughtSignature` 与 usage accounting
不变，两个 provider boundary tests 通过；未用前端文案擦除。`30..33` 保存 health、空 browser
logs、修前/修后 events 摘要与 DOM load/link/200×200 contract。G51 关闭、TH-03 升 PASS，矩阵为
`PASS 20 / GAP 9 / INTENTIONAL 4 / BLOCKED 1 / UNTESTED 45`；两条 session 与全部数据永久保留。

---

## 2026-07-22 · INC-98.3m long output 同态与低对比 OCR fallback

新 Codex thread 真跑 220 行、约 20.9KB harmless stdout，完成态保留 `Worked for 10s` 与
`TH02-LONG-DONE`；outer disclosure 里 long `Ran a command` 无 nested chevron/raw output。
AgentRunner 则在保留的 shared session 中完整显示 15,393 chars，以 240px inner scroll 约束，
末行可达、full Copy 既有证据不变且 body 横溢为零。按同逻辑 1952×1465 将 Codex `04` 与
AgentRunner `05` 合并成 `06` 后裁决：我方长输出可审计能力更强，不删功能向下对齐；TH-02
升 PASS，矩阵为 `PASS 21 / GAP 9 / INTENTIONAL 4 / BLOCKED 1 / UNTESTED 44`。

取证还暴露 Vision 在非 Retina 小字号/低对比 disclosure 上会漏字或把 `Ran a command` 合并为
单词。capture driver 保留 normal fast path，只在 target miss 后把 frame 2x 重识别；同时仅对
...
## 2026-07-22 · INC-98.3o Settings 次级页面与大 Worktrees

Codex profile menu 真机覆盖 Keyboard shortcuts、Configuration、Worktrees、Archived chats；普通
Vision 在未选中小字号灰 rail 上漏 Keyboard/Worktrees，driver 只在 miss 后裁左 16% 并 4× nearest，
仍以 exact rail label 与 main heading 双断言，四页 capture 后均 Escape 恢复原 task。AgentRunner
四页用 shared production 实测：22 shortcuts/4 groups 与 palette search、live daemon health、
Worktrees 全量数据、Archived empty/populated/search/Unarchive/re-Archive，六张合并图后再裁决。

Worktrees 修前一次渲染 304 workspace/641 session、scrollHeight 49,883；按 sidebar 既有 progressive
disclosure 改为首 40 + Show more，每次 40，全量 search 不截断。dirty production 首屏 40、展开
80、专用 workspace search=1、clear 回 40、body 横溢=0。Archived 专用 retained session
`20260723-033233-inc98-3o-archive-fixture-repl-272afa4115c9b6a3` 最终保持 archived，未删除/关闭/清理。

ST-03 从 UNTESTED 改 GAP；新增 G52 shortcut override/persistence、G53 worktree registry/GC/delete、
G54 permanent/bulk session delete。矩阵为 `PASS 22 / GAP 10 / INTENTIONAL 4 / BLOCKED 1 /
...
## 2026-07-22 · INC-98.4h complex commit/push：补齐失败状态并修闭环断点

在 `~/.local/share/agentrunner/qa-workspaces/2026-07-22-inc98-4h-commit-push`
永久保留 `primary`、`peer`、`no-remote`、`detached` 四个 workspace repo 与本地
`origin.git` bare remote；共享 session：
`20260723-062648-inc98-4h-first-push-218670121b5e1dac`、
`20260723-062659-inc98-4h-no-remote-b8ea1666152a872b`、
`20260723-062705-inc98-4h-detached-e67c07a274b815e1`。
不触外部 remote、不删除测试数据。

真实路径先证明 backend 状态：no remote/detached 分别稳定返回 HTTP 409 +
`kind=no-remote/detached`；首次 UI `Commit & push` 成功 commit、push 并设置
`main -> origin/main` upstream；clean tree 的 API push 返回 200
`Everything up-to-date`；peer clone 推进 remote 后 primary 本地分叉，push 确认
non-fast-forward rejection。

900×700 UI 复验暴露三项闭环 bug：clean tree 时整个 trigger disabled 导致
“Push existing commits” 永远点不到；API client 丢弃 backend 的 `kind`，rejected
只显示 `git push failed`；持久 error toast 跨 session 导航残留，上一 repo 的
no-remote 会与下一 repo 的成功 toast 并排。修后 clean tree 菜单仍可打开，仅 Commit
与 Commit & push disabled，Push enabled；`kind=rejected` 首句明确提示 remote 有新
commit 及 fetch/rebase/resolve/push 路径，raw stderr 仍折叠在 Details；select session、
run、page 都清空上一上下文的 toast。真实复验 status count 从 1 降为 0，detached 错误
仍保留具体动作提示；900×700 body/diff overflow=0，browser warn/error logs 空。
新增 API contract、DiffView action/error、Supervision shared error 与 navigation
toast 隔离回归；`./scripts/check.sh` 全绿。CH-06 仍标 UNTESTED，只因 Codex 同态尚缺。

---

## 2026-07-22 · INC-98.4i merge conflict / multi-session：阻止 marker 误提交

沿用 retained local bare remote，`peer` 与 `primary` 在同一 `README.md` 行提交不同内容；
`primary fetch + merge origin/main` 真实进入 `UU README.md`、ahead 3/behind 2，保留
unresolved 状态。修前 900×700 Review 只给普通 `M +7 -1`，Commit or push 全部可用；
实际 `handleCommit` 第一动作是 `git add -A`，会把仍含 conflict marker 的文件当作 resolved
提交。这是复杂路径才会出现的数据完整性缺陷，不是视觉小修。

修复增加 `unmergedPaths` 的 NUL-safe Git index 查询。Working Tree 与 Last Turn API 都投
`conflicts=["README.md"]`；Review 增加红色 blocking note、文件头与 file index 的 conflict
badge，Commit/Commit & push 禁用并说明先 resolve，Push existing commits 仍可用。backend
在任何 stage 动作前返回 HTTP 409、`kind=conflict` 与 path list。真实 API 前后比较确认
HEAD 完全相同、porcelain 仍为 `UU README.md` / staged peer file，不曾自动解决或提交。

第二 retained session
`20260723-064535-inc98-4i-shared-conflict-d758ef47097b3d07` 直接复用同 workspace：
第一 session 的 Last Turn 包含 918-byte conflict diff，第二 session 因 baseline 已含
marker 而 Last Turn diff=0；两者仍都投 conflict，第二 session Working Tree 真 Review
显示 `+5 -0`、同一 blocking note/badge/commit guard。由此钉死 “turn 无变化”不得遮住
...
## 2026-07-23 · INC-98.4q child typed attention 全链

Goal Governor 独立复核指出两个复杂边界：Web UI 每 4 秒只刷新最近 40 个 root，
旧 parent 的 child 新等待会陈旧；child 自己调用 structured `ask_user` 时，inspect
虽有 questions/answer_with，session projection 与 sub-agent UI 却丢失回答路径。

修复以 session tree 最新 descendant journal mtime 决定 recent page，并递归投
`attention.answers`。Environment/Agents 点名 `Needs answer` 且直达 child；sub-agent
仍无普通 composer，只在真实 structured wait 时开放 AskForm。shared 真 Gemini parent
置于首屏外后仍实时浮出，UI 选择 Beta 产生 typed `AskResolved` 并继续；resolve 与普通
daemon/webui restart 后 attention 均不复活。证据与 retained session 见 QA-88 98.4q；
Codex 同态复杂 child 场景仍缺，EV-05 保持 UNTESTED。

---

## 2026-07-23 · INC-98.4r goal/progress hierarchy

1100×700 同态 active goal 合并图暴露主层级问题：Codex 仅在 composer 上方放紧凑
`Pursuing goal` 控制条，我方全宽 banner 又重复 Environment 的 objective、checks 与动作。
现将 active/paused bar 收敛为状态、elapsed、edit、pause/resume、cancel 与详情入口；完整
goal/checklist/progress 留在 Environment，终态投影与 backend 不变。

shared 真模型 fixture 覆盖 active→pause→resume→budget stop、四步 progress、reload 与
390×844。真实浏览器同时发现 Environment 关闭后焦点落到 body，修为返回 compact opener；
移动端零横溢，72 files / 752 frontend tests、production build、full gate 与空 browser
...
## 2026-07-23 · INC-98.4v Home 首屏层级

1100×700 新鲜合并图暴露 hero 被单独居中，cards 上浮且把空白堆在 composer 前。现把
...
## 2026-07-23 · INC-98.4x running Review 与 Scheduled 失败闭环

先在 1100×700 真跑双方 running / populated Review：AgentRunner 361 字 draft、390/390
split 与 7-file complex diff 均无横溢，故不做结构性改动。随后 Scheduled 同屏对比暴露
failed driver 同时显示“不接受 follow-up”与普通 Retry，并把 provider 协议错误直接铺进
iteration chip。

现以 driver metadata/journal 双来源提前隐藏 Retry；已知 failure class 折成 bounded human
summary，driver terminal copy 改为查看详情后启动 replacement。完整
`provider_invalid / INVALID_ARGUMENT` 仍在 `Run details → Raw run data`，关闭回焦通过。
...
## 2026-07-23 · INC-98.2e 全分支审计：恢复 Web UI 原子开场附件

审计全部本地/远端 branch tip 后，旧 `893bb11f` 的大部分底层能力已由当前
durable opening 实现替代，但 Web UI 仍退化为“先发纯文本、再补一条附件消息”。
这会把同一 opening 拆成两个 turn，且 attachment-only 从 Home 根本无法创建。

现改为复用既有 `ar new --image/--file` 契约：Home 的 create request 一次携带
text/images/files，backend 在写 spec/启动 session 前验证附件可读；attachment-only
使用中性 caption，不再产生第二条 `send`。Go handler test、frontend attachment-only
component test、production build 与全量检查作为 A 闸；不改变 daemon/CAS/journal
不变量，也不把宿主 Codex 文件选择尚未自动化的 NS-06 误判为完整真浏览器 PASS。

全量 frontend gate 同时抓出最新 `main` 的 agent catalog 重构在 `AppShell` 静态挂载时
提前发起 `agents` I/O，违反 INC-99 已定的独立挂载边界。catalog 改为首次打开 Add
菜单或真正提交时惰性加载；isolated `AppShell` 恢复零 I/O，真实创建/Agent 菜单仍走
同一 catalog。另修正 3 处被 terminology lint 拒绝的旧 `task` 注释/Story fixture。

审计范围为 fetch 后 229 个 `refs/heads` / `refs/remotes/origin` 引用：203 个 tip 已是
`origin/main` 祖先；其余 26 个逐一跑 reachability + `git cherry` + 当前代码/历史对照。
其中本地 `main`、`a1ef0818`、`a674b413`、`a757ca84`、`aa11a5a5`、`ac192459`、
`ad573dc7`、`aeeffe4d`、`af18d412` 及远端 `codex-blackbox-testing`、`zen-colden`
共 11 个仅保留 patch-equivalent 历史。`save-my-1245`、`a0058b0d`、`a24fbf60`、
`a30c6f5b`、`a3d73770`、`a6dcb510`、`a7d2aa6f`、`a968279c`、`abcf80fd`、
`ac85db78`、`ac922202`、`afbd2345` 及远端 `continue-session`、`webui-tailwind-migration`
...
## 2026-07-21 · G44 会话全文搜索后端(`ar search`)

此前"哪次会话里提过这事"是**无法回答**的:command palette / sidebar 只在
**已加载**会话的 title/id 上做客户端子串过滤,历史一超出一页就失效。

落 `ar search`(`internal/cli/search.go`)。四条裁决,每条都有个诱人的错答案:

1. **扫描,不建索引。** 常驻索引拿速度换正确性——它需要失效策略,而一毫秒
   前刚写的会话恰恰是用户要搜的那个。扫描**天然新鲜**,于是"新 journal 可
   见性"根本不是一条要做对的策略:它不可能做错。成本改由显式边界约束。
   (与 G57 同一手法:能读时算的,就别缓存。)
2. **子串,不做分词。** `internal/index` 的 BM25 按词边界分词,对中文/日文
   **一个都匹配不上**——没有空格。子串匹配在英文排序上确实更差,但它是 CJK
   唯一能工作的选择,而本仓库的会话是双语的。
3. **tool payload 默认不搜。** 工具参数与结果携带文件内容、命令输出、以及
   workspace 里当时的一切。默认搜它,等于把一次随手查询变成对用户并没在看
   的数据的披露。`--include-tools` 是显式 opt-in。
4. **按最近排序,不编相关度分。** 子串匹配给不出诚实的相关度信号;编一个会
   在"看起来权威"的同时任意排序。"最近的会话在前"是能辩护的断言。

**边界即答案的一部分**:`sessions_scanned`/`sessions_skipped`/`truncated`
随结果返回,人类输出里也打印。没报出来的上限会被读成"东西不在那儿"——静默
截断在搜索里尤其有害,因为空结果看起来像个结论。
