...
| 4 | 对步骤 3 通过 `/api/sessions/:sid/retry` 执行 Scheduled Retry | 新建 driver series，首事件为 `driver_started`，没有 `input_received`；inspect 汇总 raw/cache/billed usage |
...
- **S7 审批语义对账**(QA-0719 第十四轮远程驱动首验,待并入 check.mjs
  自动化):ask 模式会话请求 bash 写文件。卡出现 ⇔ status=
  waiting:approval + ATTENTION 计数;Approve ⇒ 文件入两 scope diff、
  "Approved · bash" 审计线、ATTENTION 清零;Deny ⇒ 文件不存在于任何
  diff、"Denied · bash" chip 在 turn 折叠内、状态回 waiting:input。
  Review 入口 scope 配对由 ChangesOutcome 单测锚(scope pairing)常绿。
