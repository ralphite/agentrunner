...
| 2 | live Changes 默认打开 Working tree，再开范围 menu 切 Last turn | 两档都是真 API；menu 描述/active check；`Commit or push` 两档常驻且说明作用全 workspace，Apply/Remove 只在 Working tree |
...
呈 Add/Advanced 两组，root 精确为 Files and folders / Goal / Plan mode / Automation；
Automation 子页含 Loop / Best-of-N / Background run / Agent，Agent 子页可达 YAML spec。
...
| ATTENTION "keeps spending tokens" | child journal usage 增量 > 0 | S5 |
| child 审批可见性 | child journal `waiting_entered{approval}` ⇒ 父面 approval card + ATTENTION + Subagents Needs approval 必须呈现 | S5b(G39 常设绿门) |
| PROGRESS x/y settled-child 完成性 | parent `SubagentCompleted` 后 child 未完成项必须投影 failed；conversation `waiting:input` 不是终态，仍如实显示模型最后整表 | S5 |
...
- **S5b(G39 常设绿门)**:ask 模式 spawn,child 卡
  `waiting_entered{approval}`——断言 fresh/reload 后父面持久显示具名审批卡、
  Attention 与 Subagents `Needs approval`；卡使用 delegation 的真实 child workspace；
  approve/deny 精确路由后 child 与 parent 均继续。QA-88 TH-09 shared-store 闸门 B 已绿，
  G39 关闭；parent sidebar 的全局 attention 单列 G49。
...
## QA-86 Web UI QA 契约与可访问性收口（INC-95，UJ-24）

**状态**：PASS（2026-07-22；真实 `http://127.0.0.1:8809/` + 共享
`~/.local/share/agentrunner/`；候选 `fc1332a3-dirty-103034`，health
`ok/daemonUp/versionMatch=true`；测试数据未关闭、删除或清理）。

| # | 真实动作 | 硬断言 |
|---|---|---|
| 1 | 390×844 打开既有真实 diff session 的 Changes | `Show sidebar` 从视觉与 focus tree 消失；`Change diff scope` 命中自身；关闭 Changes 后 sidebar trigger 恢复且可打开 drawer |
| 2 | session 与 project row 分别以 `Shift+F10` 打开 context menu，再按 Escape | 首个 menuitem 获焦；菜单关闭后 active element 分别回原 session/project row |
| 3 | Last Turn 与 Home `+` menu | Last Turn 的 `Commit or push` 常驻且说明作用 workspace；Add root 精确四项，Automation/Agent 子页可达 Loop/Best-of-N/background/YAML |
| 4 | 检查既有 stale-progress session | parent 为 `waiting:input` 且 UI 如实显示模型最后整表；不改写 journal、不伪造 done |
| 5 | deep-link reload、health 与 browser logs | 原 thread/Changes 恢复；`daemonUp/versionMatch=true`；relevant warning/error=`[]` |

**证据要求**：归档到 `qa/runs/2026-07-22-QA86-webui-qa-corrections/`，保留
DOM/截图、health、版本与使用的共享 session id；不执行 Archive/Remove/Commit/Send。

**结果**：PASS。独立 CI agent 先确认 mobile hit-target、session/project Escape
focus-return 与 QA 契约漂移；确认 stale progress 是 `waiting:input` 下模型最后整表而非
runtime 终态 bug。修复后真实浏览器测得 `Show sidebar` display=`none`/rects=`0`、
scope center 命中 `Change diff scope`，关闭 Changes 后 trigger rects=`1`；两个 context
menu 首项分别为 Pin/Pin project，Escape 后 active element 回原 row。Home Add root 精确
四项，Automation 与 Agent/YAML 子页可达；Last Turn 的 Commit 常驻。stale session
...
| 98.2f 动作 | 硬断言 |
|---|---|
| Codex Goal/Plan driver | `--new-chat-control goal/plan` 从 Add root 二次 OCR 点击；Goal 以 composer `Goal` chip 验证，Plan 重开 Add 并验证 `Turn plan mode off` 后收回 menu；当批以 New chat 收尾，98.2g 发现 Plan preference sticky 后改为从 Add 真正 off 并反向验证；未 Send |
...
| 98.2g 动作 | 硬断言 |
|---|---|
| clean New chat 双侧比较 | Codex Retina 2× 与 AgentRunner 均换算为逻辑 1952×1465 light；先真实关闭 sticky Plan，再合并 `20+17→21`，两侧均为默认 placeholder/单 composer，不用不同 state 冒充同态 |
| responsive/theme | AgentRunner 真实 UI 覆盖 1840×1000、1280×720、390×844 的 home/sidebar/Appearance × light/dark；body `scrollWidth==clientWidth`，mobile composer、settings close/back 与主题卡均在 viewport |
| persistence/no-flash | Light、Dark、System 均经 Settings 选择后 reload；显式态恢复 `data-theme`，System 保持无 attribute 并跟随 media；head parser-blocking `theme-init.js` 在 body/main 首 paint 前恢复，`theme-color` 同步 |
...
| 98.3o 动作 | 硬断言 |
|---|---|
| Codex secondary Settings | profile menu→Keyboard shortcuts/Configuration/Worktrees/Archived chats；exact rail OCR、main heading 二次确认，capture 后 Escape 回原 thread；`04..07` accepted，debug 图拒收 |
| same-state compare | `12..15/17/19` 均先把 Codex 与 AgentRunner 同 viewport 状态合并后验图；Keyboard 真实 22 bindings/4 groups，Configuration 只投 live health，Archived 用专用 retained shared session 补 empty/populated/search/unarchive/rearchive |
| large Worktrees 红转绿 | 修前 304 workspace/641 session 一次渲染、scrollHeight=49,883；修后首 40、Show more→80，全量 query 精确命中 retained fixture workspace，clear 回 40，body 横溢=0 |
| backend 裁决 | shortcut override/conflict/persistence=G52；managed worktree registry/GC/delete=G53；permanent/bulk session delete=G54。只读文案保留，不画 edit/delete 假 control |
| Gate B | dirty shared production `7d2bdfbc-dirty-203538` health `daemonUp/versionMatch=true`、browser logs=`[]`；frontend 68 files/712 tests、production build、webui Go vet/test、capture contract/shellcheck 与 `./scripts/check.sh` 全绿；fixture/session/workspace/journal 永久保留 |
...
| cancel / compatibility | Skip→`SKIPPED` 且非错误；普通 composer Beta 兼容回答→`COMPAT=Beta`。修前后者在 answer 已消费、assistant 已完成后仍残留 `Beta / queued…`，reload 才消失；只加 `AskResolved.answer` poll matching 的 production 复拍仍命中 receipt race，且实际 `send --detach` receipt 是泛型 `delivered`；最终以 durable structured AskForm 可见 context + 成功 receipt 按 optimistic id 清除，journal matching 作 fallback；部署后必须以 `?v=<health.webuiVersion>` cache-bust 并确认 script hash 与 server `index.html` 一致，新加载 production bundle 后无需 reload 即零幽灵 |
...
**98.2g 证据**：`qa/runs/2026-07-22-QA88-98.2g-responsive-theme/` 保存 Codex
...
**98.3o 证据**：`qa/runs/2026-07-22-QA88-98.3o-settings-sections/` 保存 Codex 四个
secondary Settings 页面、OCR fail-closed fallback 校准、AgentRunner 四页修前/修后、六张同 viewport
comparison、Archived empty/populated/搜索/恢复/再归档、Worktrees 40/Show more/full search、DOM contract、
health 与 logs。专用 shared session/workspace/journal 保留且最终 archived；不 close/delete/cleanup。

| 98.3p 动作 | 硬断言 |
|---|---|
| Codex Scheduled | 先存当前 `Run TH02 long output command` restore anchor；真实 search `Weekly` 只留 Weekly status/suggestion，Paused 只留 `cloc`；每次 capture 后均经 palette 回原 thread |
| AgentRunner list/filter | shared production 的 populated 长列表、`INC66-INTERVAL-OK` search=2、Active 与 Finished=23 均真交互；browser logs=`[]`，没有伪造 empty/loading/error fixture |
| Create validation/cancel | 我方 Create menu 四 preset 可达；One-time 空 prompt 的 Start disabled，填入专用 marker 后 enabled，Cancel 后 dialog/marker 均消失且没有创建 run。Codex Create 四次校准未出现可验证 menu，`03..06` 拒收，SC-03 保持 UNTESTED |
| same-state compare | `14` 为双侧 All，`15` 为双侧 search；均先置于同一逻辑 1952×1465 后验图。`16` 只用于 Paused vs Finished capability-gap 裁决，不冒充同状态视觉 parity |
| backend 裁决 | Codex Paused 有真实 `cloc`，我方只有从 series facts 推导的 Active/Finished；全局 driver series 缺 typed pause/resume lifecycle 与 paused projection，登记 G55，禁止把 Finished 直接改名 Paused |
| driver / gate | 新增 `--surface scheduled --scheduled-search/--scheduled-filter`；placeholder/query 与 exact filter OCR fail-closed，未知状态拒绝；未保留不可验证的 Create driver。capture contract/shellcheck 与 `check.sh` 全绿；现有 shared 数据全部保留 |

**98.3p 证据**：`qa/runs/2026-07-22-QA88-98.3p-scheduled/` 保存 Codex restore anchor、
...
| 98.4m 动作 | 硬断言 |
|---|---|
| 125-step picker | retained 20-turn parent 的 126 个 checkpoint 默认不 mount；只显示 Latest，点击后才出现完整 picker；`bar-tN` 显示 `agent step`，不冒充 conversation turn |
| durable title | 从 `bar-t61` 真实 fork 后，child journal 最后一条 title 为 `SessionTitled{source:fork,title:"Initialize Taskledger CLI Skeleton"}`；CLI/sidebar/header/reload 一致 |
| recovery safety | stranded child 只显示 Resume，不显示 Retry；parent 仍为 20 turns / `max_generation_steps`，child 独立 workspace，未 Resume/Send |
| restart authority | 修前 `bar-t61` child 在普通 daemon restart 后未经操作新增 8 条事件并停在 bash approval；修后 retained `bar-t62` child `20260723-075202-fork-bar-t62-5059d9a1155aacf1` 跨 restart 始终 759 events / stranded，daemon log 无 resume，1100×700 UI 只给 Resume |

| 98.4o 动作 | 硬断言 |
|---|---|
| typed list projection | `ar sessions --json` 与 `/api/sessions` 保持 raw status 兼容，并分别投 active structured ask `attention.answers=1`、单 child parent `approvals=1`、真实三 child team parent `approvals=4`；ordinary idle 与 resolved child parent 无 attention |
| complex tree | scripted parent 同时含 structured ask、两个 child approval 与重复 `SpawnRequested`：结果 answers=1/approvals=2；resolve 一个 child 后 approvals=1，resolve ask 后 plain CLI 仍因剩余 child 显示 `waiting:approval` |
| global UI | 1100×700 fresh deep link 中 structured ask sidebar/header 为 `Needs answer`，Environment 为 `Answer requested`；child parent sidebar/header 为 `Needs approval` 且保留完整 child approval card/worktree；command palette 的三 child team row 为 amber attention |
| persistence / containment | dirty daemon/webui restart 后五个 retained shared session 的 typed projection 不变；body `scrollWidth=clientWidth=1100`，browser warning/error logs=`[]` |
| 数据保留 | 五个 shared session、全部 child journals/worktrees 与截图永久保留；不 answer/approve/deny/close/delete/cleanup |

**98.4o 证据**：`qa/runs/2026-07-23-QA88-98.4o-typed-attention/` 保存修前
sidebar Ready/Environment 空态反证、修后 structured ask、single-child approval 与
multi-child command palette 的 1100×700 截图，以及 CLI/API/restart/browser contract。

| 98.4q 动作 | 硬断言 |
|---|---|
| old parent refresh | shared parent 自身 journal 置于最近 40 条外、child journal 保持新活动；修前 production 首屏缺 parent，修后按 session-tree 最新 journal 排序并投 `attention.answers=1` |
| child structured ask | 真 Gemini child 发起 Stable/Beta/Canary 单选；parent sidebar/Environment/Agents 均为 `Needs answer`，Attention 点名 worker 且 Open 直达只开放 AskForm 的 child view |
| resolve/restart | UI 选择 Beta 后 journal 为 typed `AskResolved.Answers=[Beta]`，child 继续报告 Beta；parent CLI/API attention 归 nil，普通 daemon/webui restart 后保持 Ready、不复活 |
| evidence/gate | 1100×700 before/after/child/restart 与同 viewport comparison 在案；750 frontend tests、build、相关 Go suites 与 full gate 通过；shared session/workspace/child journal 全保留 |
...
| attachment only | Home 只选 file 时补中性 caption；accepted session 以 1 generation step、0 tool call 自然结束，唯一 `input_received` 同 event 携 file，未执行 interrupt，不出现创建后的第二次 send |
| reload / journal | 两条 accepted deep-link 可重新打开；完整原始 journal 各只有一次 `input_received`，`delivery_seq=1`；被 interrupt 污染的校准 session 明确拒收 |
