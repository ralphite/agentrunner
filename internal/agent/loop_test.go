...
func TestLoopMultiTurnEditsFile(t *testing.T) {
	...
	// GenStep 1: model reads, edits. GenStep 2: model confirms and stops.
...
func TestLoopMaxGenerationSteps(t *testing.T) {
	...
	l.Spec.MaxGenerationSteps = 3
	res, err := l.Run(context.Background(), "loop forever")
	if err != nil {
		t.Fatal(err)
	}
	if res.Reason != "max_generation_steps" || res.GenSteps != 3 {
		t.Errorf("result = %+v", res)
	}
}
