...
// Plan mode's edit/execute prohibition is the one floor ruling that stays an
// outright denial: it is the whole content of plan mode, and exit_plan_mode is
// the sanctioned way out.
func TestPlanFloorStillDenies(t *testing.T) {
	g := &PermissionGate{WS: newPermWS(t), Mode: ModePlan, Rules: []PermissionRule{{Action: "allow"}}}
	d := g.Check(context.Background(), toolEffect("edit_file", "edit", `{"path":"a.txt","old":"x","new":"y"}`))
	if d.Action != event.VerdictDeny {
		t.Errorf("plan-mode edit = %+v, want deny", d)
	}
}
