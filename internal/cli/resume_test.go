...
// INC-102: an in-session schedule (the /loop form) projects its rhythm onto
// the sessions row WITHOUT flipping kind — the Scheduled page lists the
// conversation, whose row click opens the thread. A cancelled schedule folds
// to nil, so a finished loop drops off the Scheduled page by construction.
func TestCLISessionsProjectsInSessionSchedule(t *testing.T) {
	xdg := t.TempDir()
	t.Setenv("XDG_DATA_HOME", xdg)
	dir := filepath.Join(xdg, "agentrunner", "sessions", "loop-1")
	es, err := store.OpenEventStore(dir)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, item := range []struct {
		...
		{event.TypeSessionStarted, &event.SessionStarted{SpecName: "dev", Prompt: "循环代码走查", SubStateVersions: state.SubStateVersions()}},
		{event.TypeScheduleAttached, &event.ScheduleAttached{ScheduleID: "schedule", Interval: "45s", Prompt: "loop prompt", MaxWakes: 2, Base: base}},
	} {
		if _, err := es.Append(mkEnv(t, item.typ, item.v)); err != nil {
			t.Fatal(err)
		}
	...
	list := func() row {
		...
		t.Fatal("loop-1 row missing")
		return row{}
	}

	r := list()
	if r.Kind != "session" {
		t.Fatalf("kind=%q, want session (a loop is a conversation, not a driver)", r.Kind)
	}
	if r.Schedule != "interval" || r.Cadence != "Every 45s" {
		t.Fatalf("schedule=%q cadence=%q, want interval / Every 45s", r.Schedule, r.Cadence)
	}
	if !r.ScheduleControl || !r.ScheduleDetail {
		t.Fatalf("schedule_control=%v schedule_detail=%v, want both true", r.ScheduleControl, r.ScheduleDetail)
	}
	if r.NextRunAt == "" {
		t.Fatal("next_run_at empty for a live interval schedule")
	}
	...
	if _, err := es2.Append(mkEnv(t, event.TypeScheduleCancelled, &event.ScheduleCancelled{ScheduleID: "schedule"})); err != nil {
		t.Fatal(err)
	}
	if err := es2.Close(); err != nil {
		t.Fatal(err)
	}
	if r := list(); r.Schedule != "" || r.NextRunAt != "" || r.ScheduleDetail || r.ScheduleControl {
		t.Fatalf("cancelled schedule must drop the projection, got %+v", r)
	}
}
